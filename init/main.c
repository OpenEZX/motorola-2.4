...
#ifdef MOT_A760
unsigned long loops_per_jiffy = 0xF2800;
 ...
 */
	...
	parse_options(command_line);
        ...
        /* add by linwq 2003-01-24, switch off USB pull-up resistor
	 * updated by Levis*/   
   ...
   /* added by lilij for hardware hand shake, begin */
#if HANDSHAKE_DEBUG
        printk("ezx_test: begin hk in start kernal %d\n", jiffies);
#endif
        while (GPLR0 & GPIO0_BP_RDY);
     /* config MCU_INT_SW as output */
        GAFR1_L &= 0xFFFFFFF3;
        GPDR1 |= GPIO33_MCU_INT_SW;
        GPCR1 |= GPIO33_MCU_INT_SW;
#if HANDSHAKE_DEBUG
        printk("ezx_test: end hk in start kernal %d\n", jiffies);
#endif
	if (!(GPLR0 & GPIO14_BP_WDI2)) {
		printk("start kernel: got BP_WDI2\n");
		pm_do_poweroff();
        }  
     /* added by lilij for hardware hand shake, end */
...
/**add by Levis for switch**/
	if(usb_flag == USB_FUNC_NET)
	{
		//printk("\n start_kernel: net_mod!\n");
		network_modinit();
		bi_modinit();
	}
/**end***/
...
/**add by Levis for switch**/
	if(usb_flag == USB_FUNC_ACM){
			//printk("\n start_kernel: acm_mod!\n");
			acm_modinit();
			bi_modinit();
		}else if(usb_flag == USB_FUNC_PST){
				//printk("\n start_kernel: pst_mod!\n");
				pst_modinit();
				bi_modinit();
			}
/**end**/
...
static int init(void * unused)
{
	...
	(void) dup(0);
	(void) dup(0);
        /*   add by linwq 2003-01-24, switch on USB pull-up resistor 
	updated by Levis*/
	if((usb_flag == USB_FUNC_ACM) || (usb_flag == USB_FUNC_PST))
	{
		pcap_switch_on_usb();
