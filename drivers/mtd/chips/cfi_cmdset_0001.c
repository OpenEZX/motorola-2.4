...
#endif
			kfree(extp);
			return cfi_cmdset_0001_mp(map, primary);
		}
...
int cfi_intelext_unlockdown_L18(struct mtd_info *mymtd)
{
	...
	for ( i = 0; i < mymtd->numeraseregions; i ++ )
	{
		...
		for (j = 0; j < ebnums; j ++)
		{
			if ( (the_offset >= L3_RW_REGION_START) && (the_offset < L3_RW_REGION_END) )
			{
				/* unlock it */
		...
		cfi_write(map, CMD(0xFF), (the_offset - the_ebsize));  /* clear status register when we leave this region */
	}
...
int cfi_intelext_read_roflash (unsigned long from, size_t len, size_t *retlen, u_char *buf)
{
	...
	int ret = 0;
#if (0)
	printk(KERN_NOTICE "cfi_intelext_read_roflash: from(%d),len(%d)\n",from,len);
#endif
