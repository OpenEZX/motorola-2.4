 ...
 * 04/21/2006
 *      - Modified for EzX, let pm lock chip when flash sleep
 * 06/15/2006
 *	- support Penglai product
 * 08/09/2006
 * 	- Modified memory map to support multi-product
 */
...
#ifdef FLASH_RW_ARRAY1_START
	{		
		rw_start: FLASH_RW_ARRAY1_START,
		rw_end:   FLASH_RW_ARRAY1_END,
	},
#endif

#ifdef FLASH_RW_ARRAY2_START
	{
		rw_start: FLASH_RW_ARRAY2_START,
		rw_end:   FLASH_RW_ARRAY2_END,
	},
#endif

#ifdef FLASH_RW_ARRAY3_START
	{		
		rw_start: FLASH_RW_ARRAY3_START,
		rw_end:   FLASH_RW_ARRAY3_END,
	}
#endif

/* Add new flash_rw_array definition here */
};
...
static struct mtd_info *cfi_intelext_setup(struct mtd_info *mtd)
{
	...
	/* Changed for TOP+BOTTOM geometry layout -- Barbados */	
	for (j=0; j<cfi->numchips; j++)
	{
...
static int get_chip(struct map_info *map, struct flchip *chip, unsigned long adr, int mode)
{
	...
	suspend_status = atomic_read(&(pm_flash.pm_flash_suspend));
	if(suspend_status) {
		printk(KERN_ERR "Enter flash operation after mtd suspend : suspend_status(%d)!\n", suspend_status);
		return -EBUSY;
	}
#endif
...
static int cfi_intelext_suspend(struct mtd_info *mtd)
{
	...
	if (!status) 
	{
		...
		/* Add mtd suspend lock for flash operation */
		//atomic_inc(&(pm_flash.pm_flash_suspend)); 
...
static void cfi_intelext_resume(struct mtd_info *mtd)
{
	...
	2:		mcr	p15, 0, r0, c1, c0, 0":::"r0","memory");
		...
		/* Unlock mtd suspend flag */
		//atomic_dec(&(pm_flash.pm_flash_suspend));	
	}
	return;
}
...
/* Added for Intel Flash UNLOCK/LOCKDOWN mechanism */
int cfi_intelext_unlockdown(struct mtd_info *mymtd)
{
	struct map_info *map = NULL;
...
int __init cfi_intelext_init(void)
{
	...
	atomic_set(&(pm_flash.pm_flash_suspend), 0);
#endif
	return 0;
}
