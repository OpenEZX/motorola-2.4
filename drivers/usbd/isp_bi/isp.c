/*
 * ISP1583 USB Device Controller driver.
 *
 ...
 *  Author     Date                Comment
 * ======== ===========   ===================================================== 
 * Motorola 2005-06-16  - Create file to add USB 2.0 driver for Sumatra
 * Motorola 2005-07-18  - Add DMA support
 * Motorola 2005-08-05  - Fix: NFS is not working over USB 2.0
 * Motorola 2005-08-11  - Update for compliance test
 * Motorola 2005-08-19  - Update USB 2 enumeration and currect consumption te based on P3B
 * Motorola 2005-08-16  - Add more endpoints support
 * Motorola 2005-09-09  - Update ACM driver for USB2 512 max packet size
 * Motorola 2005-09-15  - Mass storage read/write not via fs
 * Motorola 2005-10-14  - Sumatra P3 NFS workaround
 * Motorola 2005-10-20  - Setup MSC1 again after AP sleep/wakeup
 * Motorola 2005-12-28  - Fix for FLASH test command
 * Motorola 2006-Jan-15 - Usb video class driver for Web camera
 * Motorola 2006-02-06  - Fix: Phone hang in logo screen
 * Motorola 2006-02-16  - Fix: USB device doesn't disappear in Windows Device Manager after power off.
 * Motorola 2006-03-21  - Fix: Phone hang when pluging in charger.
 * Motorola 2006-Jul-03 - Merge OSS cleanup to macau in kernel.
 * Motorola 2006-Aug-10 - Resolve all the OSS related issues.
 * Motorola 2006-Sep-28 - Add two NULL-pointer check in isp_dev_dma_irq
 */
...
/* macau_iso */
static int udc_high_speed(void)
{
	if(!usbd_bus)
	{
		return -ENODEV;
	...
	return usbd_bus->HighSpeedFlag;
}

int udc_hs_iso_trans_per_uframe(void)
{
	return USB_HS_ISO_TRANS_PER_UFRAME;
}
	...
	/* high speed */	
		switch(endpoint->bmAttributes & USB_ENDPOINT_MASK)
		{
			case USB_ENDPOINT_ISOCHRONOUS:
				isp_regs->D14_ENDPT_MAXPKTSIZE.VALUE = endpoint->wMaxPacketSize | udc_hs_iso_trans_per_uframe();
				break;
			default:
...
/* macau_iso */
	case USB_ENDPOINT_ISOCHRONOUS:
		ep_type = ISP_EPTYPE_ISOCHRONOUS;
		break;
	default:
		break;
	}
...
/* macau_iso */
/* ATTENTION: mass storage function driver might be influenced */
#if 0
...
static void isp_int_hndlr (int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if (int_status_lsb.BITS.RESET)   { 
		...
		/* 
		 * If USB device is configured, it consumes 500MA from VBUS. The current from VBUS
		 * should be less than 100MA in default state after a Reset.
		 */
		if (usbd_bus->device_state == STATE_CONFIGURED) {	
	...
	if (int_status_lsb.BITS.SUSP && (!int_status_lsb.BITS.RESUME)) { 
		...
		if (usbd_bus->Raw_ConfigurationValue)  {
			...
			} else if(usbd_bus->device_state == STATE_CONFIGURED) {
				queue_motusbd_event(USB_CURRENT_SUSPEND);
			}
		}
#endif	// CONFIG_ARCH_EZXBASE
...
static void isp_hw_gpio_init(void)
{
	...
	/* Configure GPIO 79 as output high to prevent bus interference.  */
	set_GPIO(GPIO_USB20_NCS3);
	set_GPIO_mode(GPIO_USB20_NCS3 | GPIO_OUT);
	PGSR(GPIO_USB20_NCS3 ) |=  GPIO_bit(GPIO_USB20_NCS3 );
...
/* udc_reflash - reset the UDC for sumatra reflash command
 */
void udc_reflash (void)
{
	USB_MODE m;
	ADDRESS_REG add_reg;

	/* If USB cable is not connected, do nothing*/
	...
	/* disable ISP1583 device enable bit */
	add_reg.VALUE = isp_regs->D14_ADDRESS.VALUE;
	add_reg.BITS.DEVEN = 0;
	isp_regs->D14_ADDRESS.VALUE = add_reg.VALUE;
	mdelay(10);

	/* disconect the ISP1583 soft reset before switch to USB1.1 */
	m.VALUE = isp_regs->D14_MODE.VALUE;
	m.BITS.SFRESET = 1;
	isp_regs->D14_MODE.VALUE = m.VALUE;
	udelay(500);
...
int udc_request_udc_irq (void)
{
	...
	/* Some registers such as MSC1 may be changed after sleep/wakeup, re-init them again.*/
	isp_hw_gpio_init();					  
