...
typedef enum usb_device_event {
	...
	DEVICE_CABLE_DISCONNECTED,	// add by Levis
...
struct urb {
	...
	__u32 framenum;
	__u32 previous_errors;
	__u32 dropped_errors;
...
/*
 * MMM Function state (for use with proc fs function switch)
 *
 */
typedef enum usb_function_state {
	FN_STATE_RUNNING,	// Normal operation
	FN_STATE_SWITCHING,	// Device function switch in progress
} usb_function_state_t;
...
struct usb_device_instance {
	...
	struct usb_function_instance *active_function_instance;
	struct usb_function_instance *future_function_instance;
	// function state
	usb_function_state_t function_state;
...
struct urb *usbd_alloc_urb (struct usb_device_instance *, __u8, int length);
//struct urb *usbd_alloc_urb_pst (struct usb_device_instance *, struct usb_function_instance *, __u8, int length);
...
struct usb_device_descriptor *usbd_device_qualifier_descriptor (struct usb_device_instance *, int );
...
#if 0
void *ckmalloc (int n, int f);
char *lstrdup (char *str);
void lkfree (void *p);
#else
void *_ckmalloc (char *, int, int n, int f);
char *_lstrdup (char *, int, char *str);
void _lkfree (char *, int, void *p);
