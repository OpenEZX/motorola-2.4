 ...
 *     02/25/2004	create file
 *     10/20/2005	change for NM
 *     07/10/2006	change to 0xFA from 0x0a for charging
 */
...
#include <linux/ezxusbd.h> /*  MAR 2 - for queue_motusbd_event(USB_NM_READY); */
...
static __u8 nm_configuration_descriptor[sizeof(struct usb_configuration_descriptor)] = {
  ...
  0xFA           /* bMaxPower 0x0a*2 MA  */ /* 060710 - change to 0xFA from 0x0a for charging*/
};
...
static void nm_event_irq(struct usb_function_instance *function,usb_device_event_t event,int data){
    ...
    switch (event) {
        ...
        case DEVICE_CONFIGURED:
             ...
             queue_motusbd_event(USB_NM_READY); /*  MAR 2 - inform uplayer emumanager */
#endif
