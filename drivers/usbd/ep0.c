 ...
 *
 * 2004-Aug-04 - (Motorola) Added support for EZX
 * 
 */
...
extern struct tq_struct cable_hotplug_tq;
		...
		case USB_REQ_GET_STATUS:
					...
					(bus->device_state != DEVICE_CONFIGURED))
			{
				...
				if(usbcable_status == 0)
				{
					...
					usbcable_status = 1;
					usb_cable_event_handler(USB_CABLE_IN);
#ifdef CONFIG_HOTPLUG
					// cable_hotplug
					// wait till QT is ready???
					if(jiffies > SYS_INIT_RDY_JIFF)
					{
						*((ACCESSORY_DEVICE_STATUS *)(cable_hotplug_tq.data)) = 
							ACCESSORY_DEVICE_STATUS_ATTACHED;
						xxx = schedule_task(&cable_hotplug_tq);
