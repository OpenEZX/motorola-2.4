...
/*  Author     Date                Comment
 * ======== ===========   =====================================================
 * Motorola 2004-Dec-06 - Change for EZXBASE.
 * Motorola 2005-JUN-01 - Passed USB-IF Test.
 * Motorola 2006-Jan-15 - Usb video class driver for Web camera.
 * Motorola 2006-Feb-07 - Duplicate dialog is pop up and error of adjusting brightness.
 * Motorola 2006-Feb-20 - Webcamera bugfix.
 * Motorola 2006-Mar-20 - Upmerge Macau baseline to R532_G_11.00.44P.
...
static int copy_endpoint (struct usb_function_instance *function, u8 *cp,
                struct usb_endpoint_descriptor *endpoint, int endpoint_index, int actual_length, int max_buf, int hs, int zero_bandwidth)
{
	int available = max_buf - actual_length;
	...
	memcpy (&endpoint_copy, endpoint, endpoint->bLength);
       	usbd_endpoint_update(function, endpoint_index, &endpoint_copy, hs, zero_bandwidth);
	...
	case USB_DESCRIPTOR_TYPE_CONFIGURATION:
		{
			...
			for (interface = 0; interface < configuration_descriptor->bNumInterfaces; interface++) {
				...
				if(interface_instance->IAD_descriptor) {
					if(interface == interface_instance->IAD_descriptor->bFirstInterface) {
						actual_length += copy_config (buffer + actual_length, interface_instance->IAD_descriptor, actual_length, max);
					}
				}
#endif
				...
				for (alternate = 0; alternate < interface_instance->alternates; alternate++) {
					...
					//interface_descriptor = alternate_instance->interface_descriptor;
                                        ...
                                        for (endpoint = 0; endpoint < alternate_instance->endpoints ; endpoint++) {
                                                ...
                                                //                );
						struct usb_endpoint_descriptor *endpoint_p = *(( alternate_instance->endpoint_list) + endpoint);

						if(endpoint_p && 
						((endpoint_p->bmAttributes & USB_ENDPOINT_MASK) == USB_ENDPOINT_ISOCHRONOUS) &&
						(alternate == 0)) {
							zero_bandwidth = 1;
						}
						else {
							zero_bandwidth = 0;
                                                ...
                                                actual_length += copy_endpoint (bus->function_instance, 
                                                                ...
                                                                *(( alternate_instance->endpoint_list) + endpoint), 
                                                                alternate_instance->endpoint_indexes[endpoint],
                                                                actual_length, max, hs, zero_bandwidth);
                                        }
                                }
                        }
                }
