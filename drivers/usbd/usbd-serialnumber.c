/*
 * linux/drivers/usbd/usbd-serialnumber.c - USB Device serial number
 * Creates a procfs entry "usb-serial-number" for reading the
 * device serial number (if the device has one).
 *
...
USBD_MODULE_INFO ("usbd_serialnumber 0.2-alpha");
 ...
 * statistics for.
 */
static ssize_t proc_read_serial (struct file *file, char *buf, size_t count, loff_t * pos)
{
	unsigned long page;
	int len = 0;
	int index;
