...
storage_recv_urb (struct urb *urb, int rc)
{
    ...
    if (!new_urb)
    {
        ...
        //set high speed 0
/* 060411 - endpoint_halted() will cause panic once in a while and no need to be invoke */
#if 0
...
/* 060411 - endpoint_halted() will cause panic once in a while and no need to be invoke */
		...
		/* DEC 05*/ function = private->function;
...
do_inquiry (struct usb_storage_threaddata *tdata)
{
    ...
    data[0] = PERIPHERAL_DEVICE_TYPE;
    data[1] = 0x80;		/* Removal */
    /* Change RMB to 1 to be displayed as a removable device by Windows Explorer. */
    data[2] = 0x04;		/* SPC-2 */
    data[3] = 0x02;
    data[4] = 4 + 8 + 16 + 4;
    data[5] = 0x00;
