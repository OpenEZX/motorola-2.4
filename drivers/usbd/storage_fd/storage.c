 ...
 *
 * Changes copyright (C) 2003, 2005, 2006 - Motorola
 *
 ...
 * Date          Comment
 * ==========    ==========================
 * 10/24/2003    Added the interface number
 * 12/04/2005    add error protection in storage thread 
 * 12/21/2005    support Macau product
 * 04/28/2006    Release semephore before MPT
 * 06/15/2006	 support Penglai product
 * 08/11/2006    Fixed a bug of releasing urb buffer in storage thread 
 * 08/09/2006	 modify for support the multi-product
 */
...
#define SENCODE_LOGICAL_UNIT_NOT_READY          0x04
...
struct usb_device_description storage_device_description = {
    ...
    iProduct:EZX_PRODUCT_NAME_STORAGE,
...
storage_recv_urb (struct urb *urb, int rc)
{
    ...
    if (!new_urb)
    {
        ...
        //set high speed 0
	/*20060411 - endpoint_halted() will cause panic once in a while and no need to be invoke */
#if 0
	    ...
	    dbgPRINT("data_buf %p line %d", tdata->data_buf, __LINE__);
            ...
            if (!tdata->data_buf)   //Internal error, send csw and Stall all pipe 6.6.2
            {
                ...
                if(storage_send_CSW(private) < 0)
                {
                    private->devstate = STATE_INVALID;
                    //set high speed 0
/* endpoint_halted() will cause panic once in a while and no need to be invoke */
#if 0
                    usbd_endpoint_halted(function,
                            usbd_endpoint_bEndpointAddress(function, BULK_IN, 0));
#endif
...
extern struct semaphore usbd_bi_sem;
...
storage_thread (void *data)
{
    ...
    while(!storage_thread_terminating)
    {
	    ...
	    down(&usbd_bi_sem);
            spin_lock_bh(&storage_lock);	/* prevent event handler */
	    if(private->devstate == STATE_INVALID)
	    {
		tdata->busy = 0;
		spin_unlock_bh(&storage_lock);
		up(&usbd_bi_sem);
...
/* endpoint_halted() will cause panic once in a while and no need to be invoke */
		 ...
		 function = private->function;
	    	...
	    	if (tdata->cbw.CB[0] == READ_10)
		{
            		tdata->data_buf = g_read_dma_buf ? g_read_dma_buf->dma_buf_pool_ptr : NULL;
...
/*endpoint_halted() will cause panic once in a while and no need to be invoke */
                 ...
                 function = private->function;
                    ...
                    //continue;
#ifdef CONFIG_USBD_ISP_BUS
/* USBD ISP controller is used in MACAU product,the urb buffer of READ_10 command 
   is used for DMA transfer, so the release of it must no call lkfree(), 
   otherwise will cause kernel panic.*/
                    if(urb != g_in_urb) {
                        if (tdata->cbw.CB[0] == READ_10) {
                            if (urb->buffer) {
                                if (g_read_dma_buf != NULL) {
  	...
  	if (g_write_dma_buf != NULL)
	{
	    ...
	    g_write_dma_buf = NULL;
            ...
            if(scsi_switch_from_udisk != -1)
            {
                ...
                if(scsi_switch_from_udisk > USB_STATUS_UVC)
                      continue;
...
do_vpd (struct usb_storage_threaddata *tdata, int page, unsigned int alloclen)
{
  ...
  switch (page)
    ...
    case 0x00:			/* Supported vital product data pages */
      /* TODO: This response is inaccurate after add vendor specific pages */
...
#ifdef CONFIG_USBD_ISP_BUS
    /* 
     * If SD is not avaible, set to Local disk so that no "Removable Disk" is displayed by Windows;
     * Otherwise, set to Removable for Multimedia sync.
     */
    if (mmc_is_empty () || !mmc_slot_enable)
        data[1] = 0x0;		
    else 
        data[1] = 0x80;		
#else 
...
#ifdef CONFIG_USBD_ISP_BUS
    if (mmc_is_empty () || !mmc_slot_enable)
	data[15] |= 0x0C;       /* READD, WRITED */
    else if (mmc_check_locked ())
        data[15] |= 0x04;       /* WRITED */
#endif
    data[16] = 0;
    add_in_data (tdata, data, min (len, (unsigned int) CB[4]));
...
do_read (struct usb_storage_threaddata *tdata)
{
	...
	//tdata->sense.code = SENCODE_LOGICAL_UNIT_NOT_READY;
	return -EINVAL;
    }
#endif
...
#ifdef CONFIG_USBD_ISP_BUS
    init_dma_buf_pool(dma_write_buf_pool,&dma_write_buf_pool_head, MAX_WRITE_BUF_POOL_SIZE);
    init_dma_buf_pool(dma_read_buf_pool,&dma_read_buf_pool_head, MAX_READ_BUF_POOL_SIZE);
#endif
...
#ifdef CONFIG_USBD_ISP_BUS
    release_dma_buf_pool(dma_write_buf_pool,&dma_write_buf_pool_head, MAX_WRITE_BUF_POOL_SIZE);
    release_dma_buf_pool(dma_read_buf_pool,&dma_read_buf_pool_head, MAX_READ_BUF_POOL_SIZE);
#endif
}
