/*
 * linux/drivers/usbd/bi/sa1100.c
 *
 ...
 *
 * An overall limit of about 10uS is imposed for when the the host
 * is not actively polling.
 *
 */

static __inline__ void udc_inactive(void)
{
        u32 active_count = 0;
	...
	while ((*(UDCCR) & UDCCR_UDA) && ((OSCR - oscr) < 30)) {
        ...
        if (active_count || inactive_count) {
                TRACE_MSG8( "%02d:%02d %02d:%02d delayed till not active",
                                active_count, 
                                active_oscr - oscr,
                                inactive_count,
                                inactive_oscr - active_oscr
                                );
        }
}
...
static void int_hndlr_device (int irq, void *dev_id, struct pt_regs *regs)
{
	...
	*(UDCSR) = status;
	...
	udc_interrupts++;
        ...
        TRACE_MSG8("IRQ: CSR[%02x] CR[%02x]", status, *(UDCCR), 0, 0);
...
void udc_stall_ep0 (void)
{
	int ok = 0;
	// QQQ eh?
	dbg_udc (1, "stalling ep0 (UDCCS0_FST,UDCCS0_FST,UDCCS0_SST)");
...
{
	int ok;
	dbg_udc (1, "stalling ep %d (FST)", ep);
...
{
        /*
         * The address register should be done here, but because of timing
         * problems it is now done in the interrupt handler.
         */
...
int __init udc_serial_init (struct usb_bus_instance *bus)
{
	...
	dbg_init (1, "CPU ID #%08x stepping %s",id,stepping);
#else
	...
	if (machine_is_bitsy ()) {
		...
		if (bus->serial_number_str = ckmalloc (strlen (serial_number) + 1, GFP_KERNEL)) {
	...
	if (machine_is_calypso ()) {
		...
		if (bus->serial_number_str = ckmalloc (9, GFP_KERNEL)) {
...
#if 0
static void udc_disconnect_on_error(u32 reg, char *msg)
{
        struct usb_device_instance *device;
        TRACE_MSG(reg, msg);
        udc_disconnect ();
        if ((device = udc_device)) {
...
#endif
	dbg_udc (1, "udc_connected: %d", rc);
	TRACE_MSG8("udc_connected: %d", rc, 0, 0, 0);
	return rc;
}
...
{
	dbg_udc (1, "udc_connect:");
	TRACE_MSG("udc_connect:");
...
{
	dbg_udc (1, "udc_disconnect:");
	TRACE_MSG("udc_disconnect:");
...
{
	int ok;
	dbg_intr(1, "udc_set_interrupts: %02x", flags);
...
static void ep0_set_opr (void)
{
        ...
        if (!ok) {
                TRACE_MSG8( "CS0[%02x] Failed to reset OPR", *(UDCCS0), 0, 0, 0);
        }
        else {
                TRACE_MSG8( "CS0[%02x] reset OPR", *(UDCCS0), 0, 0, 0);
        }
}
...
static void ep0_set_opr_and_de (void)
{
        ...
        if (!ok) {
                TRACE_MSG8( "CS0[%02x] Failed to reset OPR and DE", *(UDCCS0), 0, 0, 0);
        }
        else {
                TRACE_MSG8( "CS0[%02x] reset OPR and DE", *(UDCCS0), 0, 0, 0);
        }
}
...
__inline__ static int ep0_read_data (struct usb_device_request *request, int max)
{
	...
	if (fifo_count != max) {
                if (fifo_count) {
                        TRACE_MSG8("ep0_read_data: have bytes; want max", fifo_count, 0, 0, 0);
                }
                //udc_disconnect_on_error(fifo_count, "ep0_read_data: have bytes; want max");
	}
	...
	for (reads = 0, bytes_read = 0; udcwc && fifo_count--;) {
		...
		if (i == 10) {
                        TRACE_MSG8("failed UDCWC", *(UDCWC), 0, 0, 0);
		}

                if (udcwc < (fifo_count - 1)) {
                        TRACE_MSG8("too low", fifo_count, 0, 0, 0);
                }
		...
		if ((bytes_read >= max) && (fifo_count > 0)) {
                        TRACE_MSG8("too much", fifo_count, 0, 0, 0);
			break;
		}
	}
...
static __inline__ void ep0_read_request (unsigned int udccs0, unsigned int udccr)
{
	...
	//if (!(udccr & UDCCR_UDA)) {
        //        TRACE_MSG(udccr, "warning, UDC not active");
        //        //udc_disconnect_on_error(udccr, "warning, UDC not active");
	//}
	...
	// unusual conditions?
        if (udccs0 & (UDCCS0_SST | UDCCS0_OPR)) {
                ...
                if (!(udccs0 & UDCCS0_OPR)) {
                        ...
                        TRACE_MSG8("wait for another interrupt", udccs0, 0, 0, 0);
                        return;
                }
        }
	...
	if ((i = ep0_read_data (&ep0_urb->device_request, 8)) < 8) {
                //TRACE_MSG(i, "not enough data");
                //udc_disconnect_on_error(i, "not enough data");
		ep0_set_opr ();
		return;
	}
	...
	// check direction of data transfer
        if ((ep0_urb->device_request.bmRequestType & USB_REQ_DIRECTION_MASK) == USB_REQ_HOST2DEVICE) {
                ...
                if ((ep0_urb->device_request.bmRequestType == 0) && (ep0_urb->device_request.bRequest == USB_REQ_SET_ADDRESS)) {
                        ...
                        TRACE_MSG8("set address", usb_address, 0, 0, 0);
                        ...
                        // XXX this should help, but UUT test program sometimes fails...
                        //udc_inactive();
                        ...
                        *(UDCAR) = usb_address;
                        ...
                        TRACE_MSG8("NO DATA", udccs0, 0, 0, 0);
                        //ep0_set_opr_and_de();
                ...
                if (!le16_to_cpu (ep0_urb->device_request.wLength)) {
                        TRACE_MSG8("wLength Zero", ep0_urb->device_request.wLength, 0, 0, 0);
                ...
                if (!ep0_urb->actual_length) {
                        TRACE_MSG8("no data to return", ep0_urb->device_request.wLength, 0, 0, 0);
...
static __inline__ void ep0_in (unsigned int udccs0)
{
	...
	//dbg_ep0 (2, "CS0: %02x", udccs0);
        TRACE_MSG8("ep0 in", udccs0, 0, 0, 0);

	// check for non empty FIFO
	if (*(UDCWC)) {
		//dbg_ep0 (0, "FIFO not empty");
                TRACE_MSG8("FIFO not empty", *(UDCWC), 0, 0, 0);
		return;
	}
	...
	if (udccs0 & (UDCCS0_SE | UDCCS0_SST)) {
		//dbg_ep0 (1, "ep0_in: SE or SST set\n");
                TRACE_MSG8("SE or SST set", udccs0, 0, 0, 0);
		return;
	}
...
void ep0_int_hndlr (unsigned int status)
{
	...
	//	 udccs0, *(UDCWC), *(UDCAR));
        ...
        if (udccs0 & (UDCCS0_IPR | UDCCS0_SST | UDCCS0_SE)) {
                ...
                if (udccs0 == UDCCS0_IPR) {
                        ...
                        TRACE_MSG8("IPR SET", udccs0, 0, 0, 0);
                        return;
                }
                ...
                if (udccs0 & UDCCS0_SST) {
                        ...
                        TRACE_MSG8("SST set", udccs0, 0, 0, 0);
                        if (udccs0 & UDCCS0_SST) {
                                SET_AND_TEST (*(UDCCS0) = UDCCS0_SST, *(UDCCS0) & UDCCS0_SST, ok);
                                if (!ok) {
                                        dbg_ep0 (0, "failed to reset SST CS0: %02x ok: %d", *(UDCCS0), ok);
                ...
                if (udccs0 & UDCCS0_SE) {
                        ...
                        if (send_zlp) {
                                ...
                                TRACE_MSG8("not sending ZLP CS0", *(UDCCS0), 0, 0, 0);
                        }
                        send_zlp = 0;
                }
        }
	...
	if ((ep0_state != EP0_STATE_IDLE) && (udccs0 & UDCCS0_OPR)) {
		//dbg_ep0 (3, "Forcing EP0_STATE_IDLE CS0: %02x", udccs0);
                TRACE_MSG8("forcing EP0_STATE_IDLE", udccs0, 0, 0, 0);
		ep0_state = EP0_STATE_IDLE;
	}
	...
	case EP0_STATE_IDLE:
		//dbg_ep0 (4, "state: EP0_STATE_IDLE CS0: %02x", udccs0);
                TRACE_MSG8("EP0_STATE_IDLE", udccs0, 0, 0, 0);
	...
	case EP0_STATE_IN_DATA_PHASE:
		//dbg_ep0 (4, "state: EP0_STATE_IN_DATA_PHASE %x", udccs0);
                TRACE_MSG8("EP0_STATE_IN_DATA_PHASE", udccs0, 0, 0, 0);
		ep0_in (udccs0);
		break;

	case EP0_STATE_END_IN:
		//dbg_ep0 (4, "state: EP0_STATE_END_IN %x", udccs0);
                TRACE_MSG8("EP0_STATE_END_IN", udccs0, 0, 0, 0);
		ep0_end_in (udccs0);
		break;
	}
		 ...
		 int chn)
{
	if (endpoint) {
		...
		SET_AND_TEST (*(UDCOMP) = ep1_endpoint->rcv_packetSize - 1,
