...
/***add by Levis for resume urb***/
static __inline__ void urb_resume_irq (urb_link * hd, struct urb *urb)
{
	if (hd && urb) {
		urb_link *new = &urb->link;
...
/**add by Levis for throttle***/
static __inline__ void usbd_resume_urb (struct urb *urb)
{
	if (urb) 
	{
		unsigned long flags;
		local_irq_save (flags);
		urb_resume_irq (&(urb->endpoint->rcv), urb);
		local_irq_restore (flags);
	}
}
/***end Levis***/
...
static __inline__ int usbd_recv_setup (struct urb *urb)
{
	...
	if (urb && !(urb->device->status == USBD_CLOSING)) {
		...
		//printk(KERN_INFO"usbd_recv_setup: %p request: %x request: %x mask: %x vendor: %x isvendor: %x\n", 
		//      urb, urb->device_request.bmRequestType, urb->device_request.bRequest, 
		//      USB_REQ_TYPE_MASK, USB_TYPE_VENDOR,
		//      (urb->device_request.bmRequestType&USB_REQ_TYPE_MASK)!=0);

		function_instance = 
			(USB_REQ_TYPE_STANDARD == (urb->device_request.bmRequestType & USB_REQ_TYPE_MASK)) ? 
			urb->device->ep0 : 
			urb->device->active_function_instance;
	...
	//	printk("\nrcv_urb->device=%p\n", rcv_urb->device);
				...
				(len <= endpoint->rcv_packetSize)) 
		{
						 ...
						 (rcv_urb->buffer_length - endpoint->rcv_packetSize))))
			{
				...
				rcv_urb->dropped_errors = endpoint->dropped_errors;
				endpoint->dropped_errors = 0;
...
#if 0
			//printk (KERN_ERR "usbd_rcv_complete: RECV_ERROR len: %d actual: %d buffer: %d urb_bad: %x\n", 
			//		len, rcv_urb->actual_length, rcv_urb->buffer_length, urb_bad);

			// XXX TEST rcv_urb->actual_length = 0;
			// XXX TEST printk(KERN_ERR"too large %d %d\n", len, endpoint->rcv_packetSize);

			if (rcv_urb->actual_length && (len < endpoint->rcv_packetSize )) {
				...
				printk(KERN_INFO"usbd_rcv_complete: cleared\n");
			}
#else
			rcv_urb->actual_length = 0;
			rcv_urb->previous_errors++;
		...
		endpoint->rcv_error = ((len == endpoint->rcv_packetSize) && urb_bad);
	}


	// if we don't have an urb see if we can get one
	if (!rcv_urb) {
		if ((endpoint->rcv_urb = first_urb_detached_irq (&endpoint->rdy))) {
			endpoint->rcv_urb->previous_errors = 0;
		}
	}
...
/**
 * usbd_rcv_next_irq - complete a receive
 * @endpoint:
 * @len:
 * @urb_bad:
 *
 * Called from rcv interrupt to complete.
 */
static __inline__ void usbd_rcv_next_irq (struct usb_endpoint_instance *endpoint)
{
	if ((endpoint->rcv_urb = first_urb_detached_irq (&endpoint->rdy))) {
		endpoint->rcv_urb->previous_errors = 0;
	}
 ...
 * Called from rcv interrupt to complete.
 */
static __inline__ void usbd_rcv_fast_complete_irq (struct usb_endpoint_instance *endpoint, struct urb *rcv_urb)
{
	rcv_urb->status = RECV_OK;
	rcv_urb->jiffies = jiffies;
	rcv_urb->dropped_errors = endpoint->dropped_errors;
