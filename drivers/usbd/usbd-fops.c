 ...
 *  Author     Date                Comment
 * ======== ===========   =====================================================
 * Motorola 2003-Dec-06 - Modified for EZXBASE 
 ...
 * Motorola 2006-Feb-22 - Webcamera bugfix.
 * Motorola 2006-MAR-13 - Add error protection.
...
extern int udc_hs_iso_trans_per_uframe(void);
#endif
...
void usbd_endpoint_update(struct usb_function_instance *function, int endpoint_index, 
                struct usb_endpoint_descriptor *endpoint, int hs, int zero_bandwidth)
{
...
/* macau_iso */
	...
	if(hs == 1) {
		switch(endpoint->bmAttributes & USB_ENDPOINT_MASK)
		{
			case USB_ENDPOINT_ISOCHRONOUS:
				endpoint->wMaxPacketSize |= udc_hs_iso_trans_per_uframe();
				break;
			default:
				break;
		}
	...
	if(zero_bandwidth == 1) {
		endpoint->wMaxPacketSize = 0x0000;
	}
#endif	
}
                ...
                struct usb_interface_description *interface_description_array)
{
	...
	for (interface = 0; interface < bNumInterfaces; interface++) {
		...
		interface_instance->IAD_descriptor = interface_description->IAD_descriptor;
#endif
	}
	...
	for (i = 0; i < bNumConfigurations; i++) {
		...
		for (j = 0; j < configuration_descriptor->bNumInterfaces; j++) {
			...
			if(interface_instance->IAD_descriptor) {
				if(j == interface_instance->IAD_descriptor->bFirstInterface) {
					length += sizeof (struct usb_interface_association_descriptor);
				}
			}
#endif
