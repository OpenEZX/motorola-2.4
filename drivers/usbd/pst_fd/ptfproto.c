 ...
 *26-02-2003    created, try to finish char device driver
 *26-08-2003	   first build pass, no read write function
 *26-02-2004       second pass, read and recv function added.
 *28-09-2006	fix memory leak about aplogger
 *25-10-2006	Update comments
 */
...
#define        PTF_CHAR_MAJOR         189	//random...???
...
#define FALSE 0
#endif
static int ptf_usb_func_ready=0; /* 1, current PTF USB interface is present, 0, no PTF usb interface,set/clear it by usb func driver*/
int ptf_cable_connected=0;	/*  1, pluged usb cable, 0, unplug usb cable */
int ptf_func_created=0;		/*  to indicate bottom available(=1),poll() needs check dev pointer,see ptf_poll()*/
DECLARE_WAIT_QUEUE_HEAD(ptf_write_queue); /* add wait queue for ptf/aplog write */
/*  for buttom, tell char driver that PTF interface ready */
void enable_ptf_func_driver(void){
  ptf_usb_func_ready=1;

}
/*  for buttom, tell char driver that PTF interface gone */
void disable_ptf_func_driver(void){
  ptf_usb_func_ready=0;

}
/*  for upper or outside, PTF interface current status
ret 0, not ready
ret 1 ready
 */
...
int ptf_release (struct inode * inode, struct file *filp)
{
	...
	if(ptfproto_device_array[dev_num]){ /* proto_device may  be destoryed in usbd event handler!!!*/
	  ...
	  /*  since keep all private and proto structs, we need clean recv buffer here */
...
int ptf_write(struct file *filp, const char *buf, size_t count,loff_t *f_pos) 
{
        ...
        if(!ptf_cable_connected)
                interruptible_sleep_on(&ptf_write_queue); /* sleep if cable unavailable */
        ...
        if(count%64){	/* normal case  */
			...
			kfree(buffer);/* - fix memory leak about aplogger */
			...
			kfree(buffer);/* - fix memory leak about aplogger */
                ...
                if (res){
			kfree(buffer1);/* - fix memory leak about aplogger */
...
unsigned int ptf_poll (struct file * filp, struct poll_table_struct * wait){
        ...
        int mask=0;
	unsigned long flags;	/*  we need secure competition conditions */
