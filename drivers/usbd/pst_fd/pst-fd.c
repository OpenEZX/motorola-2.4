...
struct usb_ptf_private {
	...
	atomic_t write_queued_bytes;  /* e12707 for aplog */
...
int pst_urb_sent_aplog (struct urb *urb, int rc); /* e12707  callback for aplog send urb*/
#if PST_BULKINTF
...
static int ap_log_xmit_data (int dev_index, unsigned char *data, int data_length)
{
        ...
        if(atomic_read(&ptf_private->write_queued_bytes) > MAX_APLOG_BUFSIZE)/* e12707 flow control for aplog */
        {
	...
	atomic_add(packet_length,&ptf_private->write_queued_bytes);/* e12707 051230 */
