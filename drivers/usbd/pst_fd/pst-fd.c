 ...
 * Author   Date           Comment
 * ======   ======         ==========================
 * Motorola 07/01/2003     create file for Motorola PST USB Function Driver
 * Motorola 07/28/2005     Add PST-BULK Driver
 * Motorola 03/03/2006     Modify aploger flow control
 * Motorola 08/09/2006     Modify bMaxPower value from 20MA to 500MA
 * Motorola 08/23/2006     Modified the race condition for intf8.out_msg_list in the 
 *                         usbc_interface8_read function
 */
...
#include <linux/sched.h>//use to sleep_on_timeout
...
#define VERSION "0.90"
#define VERSION25 1
// change code by the swith  move to pst-usb.h
...
extern int motusbd_status;  //added to Support CFG11 bplog
#define PST_DEBUG 0 
...
static spinlock_t intf8_out_msg_list_lock = SPIN_LOCK_UNLOCKED;

#define MAX_APLOG_BUFSIZE (1024*1024)

#if PST_DUOINTF
#define PST_MAX_BUF_CNT MAX_APLOG_BUFSIZE
int intf8_read_cnt;
...
#ifdef CONFIG_ARCH_EZX  // support cfg11
...
struct usb_ptf_private {
	...
	atomic_t write_queued_bytes;  /* for aplog */
...
/*add ptf and AP logger endpoints according to new excel*/
...
/*ptf and ap logger alternate description(or)s*/
...
int pst_urb_sent_aplog (struct urb *urb, int rc); /* callback for aplog send urb*/
#if PST_BULKINTF
...
static  struct usb_ptf_private *get_ptf_private (int device_index)
{
        ...
        for (i = 0; i < PTF_MAX_DEV_NUMBER; i++)
                ...
                if(ptf_private_array[i])       // fixed, null pointer should be check
                ...
                int tx_size)
{
	...
	atomic_set(&(ptf_private->write_queued_bytes),0);
...
static int ap_log_xmit_data (int dev_index, unsigned char *data, int data_length)
{
        ...
        if(atomic_read(&ptf_private->write_queued_bytes) > MAX_APLOG_BUFSIZE)/*flow control for aplog */
        {
                printk (KERN_WARNING "\n\n Can't write data to buf anymore \n\n");
                return -EINVAL; 
        }
        
	if (!(urb = usbd_alloc_urb (function, NDX_OF_AP_LOG_IN_EP, 0, pst_urb_sent_aplog ))) {
	...
	atomic_add(packet_length,&ptf_private->write_queued_bytes);/*051230 */
	PRINTKD(" urb allocate successfully\n");
        // push it down into the usb-device layer
        int ru =  usbd_send_urb (urb);	//??? finally bi_send_urb(), return 0 when finished
	PRINTKD("   sent urb successfully\n");
	return ru;	
...
Lread_again:	//bulk needed		
	
	spin_lock_irqsave(&intf8_out_msg_list_lock, flags);
        ptr = intf8.out_msg_list.next;
	spin_unlock_irqrestore(&intf8_out_msg_list_lock, flags);
        node = list_entry (ptr, msg_node_t, list);
        ret = node->size;
//PRINTKD("\nintf8_read2 buf[%x %x %x %x] stCount%d\ ret%d \n",pUserBuffer[0],pUserBuffer[1],pUserBuffer[2],pUserBuffer[3],stCount,ret);
	...
	if (stCount < ret){
		//WARNING!!!!! 
		// should keep remainder for next read op, but test command application doesn't think so
		printk(KERN_INFO "\n Cut Data %d from %d \n ",stCount,ret);
                ret = stCount;
        }
	...
	readed_cnt+=ret; //if host send data above a usb packet size, need read again, finish flag is short packet
...
//for flow control		
		...
		if(ret >= (usbd_high_speed(pst_function)?TEST_CMD_PKTSIZE_HS:TEST_CMD_PKTSIZE_FS)){
			...
			if(!list_empty (&intf8.out_msg_list)){
				...
				//If End Packet is a max size Packet, I don't know whether the transfer is end
				//so following Dan's saying, cal the length of the transfer
				copy_from_user((char*)(&rcvlen),&(pbufferStart[8]),4);
				rcvlen = swab32(rcvlen);
				PRINTKD("\n---- USB Count %d n %d \n",rcvlen,readed_cnt);
				if(readed_cnt < rcvlen+12){//wait a while for follow data. 12 is P2K05 TLCMD header length
	...
	//else is a control interface read, return 
	}
...
//maybe speed change	int MaxPktSize= usbd_high_speed(pst_function)?TEST_CMD_PKTSIZE_HS:TEST_CMD_PKTSIZE_FS;
...
//	if(stCount > PST_MAX_BUF_CNT)return 0;//flow control
	
        PRINTKD("\nThis is intf8_write1[%x %x %x %x] count=%d jiffies=%d\n",pUserBuffer[0],pUserBuffer[1],pUserBuffer[2],pUserBuffer[3],stCount,jiffies);

#if 0	
	//when last max packet transfered, pc driver need to add one dummy packet.
       ...
       { 
		...
		/*if two line below, PC driver will response first packet 
		count = normal_write2epx(pUserBuffer,         stCount-1        ,NDX_OF_TESTCMD_IN_EP);
...
#endif
       if(stCount% (usbd_high_speed(pst_function)?TEST_CMD_PKTSIZE_HS:TEST_CMD_PKTSIZE_FS))//MAXPACKET
       {
                PRINTKD("\n NO64\n");
                normal_write2epx(pUserBuffer, stCount, NDX_OF_TESTCMD_IN_EP);
       }else {
                PRINTKD("\n 64 \n");
                normal_write2epx(pUserBuffer, stCount, NDX_OF_TESTCMD_IN_EP);
                //sent 2 more 00 00 to completed the transfer
...
//	if(stCount > PST_MAX_BUF_CNT)return 0;//flow control
       ...
       if(stCount% (usbd_high_speed(pst_function)?TEST_CMD_PKTSIZE_HS:TEST_CMD_PKTSIZE_FS))//MAXPACKET
       {
		...
		//follow Dan says,sent 2 more 00 00 to completed the transfer 
...
/*+++++++++++++++++++++++++++++++support CFG11 bplog ++++++++++++++++++++===*/
	if(motusbd_status ==  0x2){   
		PRINTKD("cfg11:normal_write end03\n");
...
#if PST_BULKINTF
	//050824 penging a urb in ep of testcmd
...
/*  -------------------------------------------------------------------
		  struct usb_ptf_private *ptf_private;
		  ptf_private = get_ptf_private (ap_log_ndx);
		  printk("\n\n here buffer bytes:%d\n\n",atomic_read(&ptf_private->write_queued_bytes));		  
 -------------------------------------------------------------------------*/
		  ptfproto_send_fasync_to_all_devices(SIGIO);
#if 0	  /* when supporting usb switching, all private and proto dev structures will be keeped staticaly.*/
		  ptf_private_destroy();
#endif		  
...
static int callback4bulktcmd( struct urb *urb, int rc )
{
	...
	if(!interface->enable) goto bail;// interface have not been open, but pc sent data to it, ignore it.051114
...
//flow control
	...
	g_tcmd_bulkorcontrol = TCMD_BULKPORT;
	 ...
	 RETURN_ZERO_IF(!usbd_start_recv (urb));// a cycle finished!
...
int pst_urb_sent_aplog (struct urb *urb, int rc) /*  callback for aplog send urb*/
{
	...
	if ((ptf_private = get_ptf_private (ap_log_ndx)) == NULL) {
                printk (KERN_WARNING "Can't recover private\n");
                return -EINVAL;
        }
	...
	atomic_sub(urb->actual_length,&ptf_private->write_queued_bytes);
