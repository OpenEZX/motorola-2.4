 ...
 *
 *  USB Device And EMUD Device Support
 *
 ...
 *  Author     Date                Comment
 * ======== ===========   ===================================================
 * Motorola 2003-Nov-25 - Create file.
 * Motorola 2006-Jan-15 - Usb video class driver for Web camera.
 * Motorola 2006-MAR-06 - Modified for USB MODE Switch 
...
// 
	...
	switch (cmd)
	{
		case  MOTUSBD_IOCTL_SET_USBMODE: //EMUDSOCK_CMDTYPE_REQ_USBMODE
			...
			if((argi>=USB_STATUS_NONE)&&(argi<=USB_STATUS_UVC))//the Last USB status
			{
				motusbd_load(argi);
				return 0;
			}
        ...
        /*
         * For External USB2.0 chip,
         * need init usb device contrl,
         * enumerate usb device, after factory test switch b plus
         */
#ifdef CONFIG_ARCH_EZX_SUPPORT_EXT_USB20
        case MOTUSBD_IOCTL_SET_UDCINIT:
            udc_init();
            return 0;
        break;
#endif
