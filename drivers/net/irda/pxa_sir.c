...
/*digital camera control squence*/
...
static void pxa250_dma_stop(int ch)
{
   __ECHO_IN;
...
/*
 * Make irq avaliable for ICP unit and DMA controler
 */
static void pxa250_do_fir_IRQ_config(void)
{
   __ECHO_IN;
...
static void pxa250_do_fir_GPIO_config(void)
{
   ...
   /*Clear Falling edge detect register */
     GFER1 &= ~(3 << 14); 
...
static void pxa250_irda_rxdma_irq(int ch,void *id, struct pt_regs *regs)
{
   ...
   {
      DBG("PXA IrDA: Normal end of irq channel %d - packet to big\n", ch);
...
static void pxa250_irda_txdma_irq(int ch, void *id , struct pt_regs *regs)
{
   ...
   if (dcsr &  DCSR_ENDINTR )
   {
      DBG("PXA IrDA: Normal end of irq channel %d\n", ch);
      DCSR(ch) |= DCSR_ENDINTR;
      si->txskb = NULL;
   }
   ...
   if (skb)
   {
	...
	/*Disable transifer and anable receiver*/
...
static int pxa250_irda_start(struct net_device *dev)
{
	...
	/* Allocate DMA channel for recevier (not used) */
	err = pxa_request_dma("IrDA receive irq", DMA_PRIO_LOW, pxa250_irda_rxdma_irq , dev);
	if (err < 0 )
	   goto err_rx_dma;
	si->rxdma_ch=err;
	...
	err = pxa_request_dma("IrDA transmite irq", DMA_PRIO_LOW, pxa250_irda_txdma_irq , dev);
	if (err < 0 )
	   goto err_tx_dma;
	 ...
	 * buffers have to aligned and situated in dma capable memory region;
	 */
...
static
#endif
int __init pxa250_irda_init(void)
{
