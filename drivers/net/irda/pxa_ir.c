...
#define SET_SIR_MODE            STISR = STISR_RCVEIR | STISR_XMITIR | STISR_XMODE | STISR_RXPL
...
//	        STISR |= STISR_XMITIR;
		...
		len = si->tx_buff.len > 64 ? 64: si->tx_buff.len;
		for(i=0; i<len; i++)
		{
...
static void pxa250_irda_hpsir_irq(struct net_device *dev)
{
	...
	if(si->xmit_flag ==1)
	{
	       ...
	       len = si->tx_buff.len > 32? 32: si->tx_buff.len;
	       for(i=0 ;i<len ;i++)
	       {
      		      STTHR = *si->tx_buff.data++;
     		      si->tx_buff.len -= 1;
...
#if 0	
    printk("GPDR0 (21) = %x\n",GPDR0);
    printk("GPDR1 (46,47) = %x\n",GPDR1);
    printk("GPLR0 (21) = %x\n",GPLR0);
    printk("GPLR1 (46,47) = %x\n",GPLR1);
    printk("GAFR0_U (21) = %x\n",GAFR0_U);
    printk("GAFR1_L (46,47) = %x\n",GAFR1_L);
#endif
	return 0;
}
...
//    request_mem_region(__PREG(STRBR), 0x24, "IrDA") ? 0 : -EBUSY;
	MOD_INC_USE_COUNT;
...
static int __init pxa250_irda_init(void)
{
    ...
    GPSR0=(1<<21);
    PGSR0|=(1<<21);
    /*move to start for confliction with stuart dma*/
#if 0
	/* STUART */
