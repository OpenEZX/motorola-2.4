...
void MacEventDisconnected(wlan_private * priv)
{
    ...
    printk("Before send Disconnect EVENT\n");
...
//					sizeof(SupportedRates_B));
	...
	if (i >= MRVDRV_MAX_REGION_CODE) {
		...
		PRINTK("unidentified region code, use the default (USA)\n");
	}
...
static void discard_bad_ssid(wlan_adapter *a)
{
  ...
  memset(&a->BSSIDList[a->ulNumOfBSSIDs], 0, bad_count * sizeof(WLAN_802_11_BSSID));
#ifndef NEW_ASSOCIATION
  memset(&a->IEBuffer[a->ulNumOfBSSIDs], 0, bad_count * sizeof(MRV_BSSID_IE_LIST));
#endif
  return;
}
						...
						HostCmd_DS_COMMAND * resp)
{
	...
	PRINTK("SNMP_RESP: value of the OID = %x, QueryType=%x\n", OID,QueryType);
	PRINTK("SNMP_RESP: Buf size  = %x\n", wlan_le16_to_cpu(smib->BufSize));
	
	if(QueryType == HostCmd_ACT_GEN_GET){
		switch(OID){
			case FragThresh_i:
				priv->adapter->FragThsd =  wlan_le16_to_cpu(*((PUSHORT) (smib->Value)));
				PRINTK("SNMP_RESP: FragThsd =%u\n", priv->adapter->FragThsd);
				break;
			case RtsThresh_i:			
				priv->adapter->RTSThsd = wlan_le16_to_cpu(*((PUSHORT) (smib->Value)));
				PRINTK("SNMP_RESP: RTSThsd =%u\n", priv->adapter->RTSThsd);
				break;
			case ShortRetryLim_i:	
				priv->adapter->TxRetryCount = wlan_le16_to_cpu(*((PUSHORT) (smib->Value)));
				PRINTK("SNMP_RESP: TxRetryCount =%u\n", priv->adapter->RTSThsd);
				break;
			default:
				break;			
		}		
...
int wlan_process_rx_command(wlan_private * priv)
{
	...
	if (Adapter->CurCmd->CmdFlags & CMD_F_HOSTCMD) {
		...
		memcpy(Adapter->CurCmd->InformationBuffer,
				resp, MIN(resp->Size, MRVDRV_SIZE_OF_CMD_BUFFER));
