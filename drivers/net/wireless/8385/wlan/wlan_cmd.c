...
int DownloadCommandToStation(wlan_private * priv, CmdCtrlNode * CmdNode)
{
	...
	if (ret != 0) {
		...
		ModTimer(&Adapter->MrvDrvCommandTimer, MRVDRV_TIMER_2S);
...
#if (defined PROC_READ_FIRMWARE) || (defined FATAL_ERROR_HANDLE)	
	if(Adapter->fwstate != FW_STATE_READY){
		PRINTK1("firmware is not ready\n");
		return -ENODEV;
	}
#endif	
...
#ifdef HOST_WAKEUP
				if (Adapter->bHostWakeupConfigured && (Adapter->sleep_period.period == 0)) {
					Adapter->bHostWakeupDevRequired = TRUE;
#if WIRELESS_EXT > 14
					send_iwevcustom_event(priv, "HWM_CFG_DONE.indication ");
#ifdef WPRM_DRV
                                        printk("iw custom event: HWM_CFG_DONE.indication.\n");
                                        printk("WiFi Driver triggered Bulverde sleep.\n");
                                        apm_req_sleep(0);
#endif /* WPRM_DRV */
