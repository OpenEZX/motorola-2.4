...
#ifdef DEEP_SLEEP
	if ((Adapter->IsDeepSleep == TRUE)) {
		printk("<1>SIOCGIWRETRY: IOCTLS called when station is"
							" in DeepSleep\n");
		return -EBUSY;
	}
#endif
...
static int wlan_hostcmd_ioctl(struct net_device *dev, struct ifreq *req, int cmd)
{
	...
	if(wrq->u.data.length > MRVDRV_SIZE_OF_CMD_BUFFER){
		PRINTK1("Invalid length=%d\n",wrq->u.data.length);
		return -EINVAL;
	}
	 ...
	 */
	copy_from_user(pCmdPtr, wrq->u.data.pointer, wrq->u.data.length);
	...
	PRINTK1("HOSTCMD Command: 0x%04x Size: %d SeqNum: %d\n",
...
static int wlan_extscan_ioctl(wlan_private *priv, struct ifreq *req)
{
	...
	if(SendSpecificScan(priv, &Ext_Scan_SSID))
		return -1;
...
static int wlan_deepsleep_ioctl(wlan_private *priv, struct ifreq *req)
{
	...
	if (Adapter->MediaConnectStatus == WlanMediaStateConnected) {
		printk("Cannot enter Deep Sleep mode in connected state.\n");
		return -EINVAL;
	}
...
#ifdef WPRM_DRV
                        /* sessiontype setting should not be abandoned. There is a case:
                           enter deepsleep comes before de-register a sessiontype. If sessiontype
                           de-register is abandoned, then, next time, when associated, driver will
                           enter the special modes according to this sessiontype immediatly. */
                        && (cmd != WLAN_SESSIONTYPE)
#endif
