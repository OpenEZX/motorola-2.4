/*
	File	: msd25_sdio.c
	$Id: //depot/MRVL/main/driver/WLan/Drv/MSI/Linux/src/if/if_sdio/msd25_sdio.c#2 $
*/
...
mmc_notifier_rec_t msd25_sdio_notifier;
...
void sbi_interrupt(int irq, void *dev_id, struct pt_regs *fp)
{
	...
	PRINTK("sbi_interrupt called IntCounter="
			"%d\n",priv->adapter->IntCounter);
#endif

	TXRX_DEBUG_GET_ALL(0x00, 0xff, 0xff);
	wlan_interrupt(dev, fp);

	LEAVE1();
}

int *sbi_register(wlan_notifier_fn_t add, wlan_notifier_fn_t remove, void *arg)
{
	int *sdio_ret;

	msd25_sdio_notifier.add = (mmc_notifier_fn_t)add;
	msd25_sdio_notifier.remove = (mmc_notifier_fn_t)remove;
	isr_function = sbi_interrupt;
	sdio_ret = sdio_register(MMC_REG_TYPE_USER, 
				&msd25_sdio_notifier, 0);
...
void sbi_unregister()
{
//	sdio_write_ioreg((mmc_card_t)priv->wlan_dev.card, FN0, 0x06, 0x08);
	sdio_unregister(MMC_REG_TYPE_USER, &msd25_sdio_notifier);
}
...
{
	return sdio_read_ioreg((mmc_card_t)priv->wlan_dev.card, FN1, HOST_INTSTATUS_REG, ireg);
}

int sbi_read_card_reg(wlan_private *priv, u8 *cs)
{
	int ret;
	ret = sdio_read_ioreg((mmc_card_t)priv->wlan_dev.card, FN1, CARD_STATUS_REG, cs);
	return ret;
}
...
{
	int ret;
	ret = sdio_write_ioreg((mmc_card_t)priv->wlan_dev.card, FN1, HOST_INTSTATUS_REG, 0x0);
	return ret;
}
...
int sbi_get_int_status(wlan_private *priv , u8 *ireg)
{
	int ret;
	u8	cs;
	wlan_dev_rec_t	*wlan_dev = &priv->wlan_dev;
	...
	if ((ret=sdio_read_ioreg( card, FN1, HOST_INTSTATUS_REG, ireg)) < 0) {
		printk("sdio_read_ioreg: reading interrupt status register failed\n");
		goto end;
	}
	...
	if(*ireg != 0) {		//DN_LD_HOST_INT_STATUS and/or UP_LD_HOST_INT_STATUS
		...
		if ((ret=sdio_write_ioreg(card, FN1, HOST_INTSTATUS_REG, ~(*ireg) & 0x03)) < 0) {
			printk("sdio_write_ioreg: clear interrupt status register failed\n");
			goto end;
		}
	}
...
*/
	...
	if(trd_p != 0) {
		trd_p = 0;
		printk("************************************ INT ireg=0x%x  CARD cs=0x%x dnld_sent=%d\n",*ireg,cs,priv->wlan_dev.dnld_sent);
	}
#endif

	if((card->chiprev != 0x00)					//support hw tx_done
//	   && (priv->wlan_dev.dnld_sent != 0)				//waiting for tx_done INT
	   && (*ireg & DN_LD_HOST_INT_STATUS)				//tx_done INT
//	   && (!priv->adapter->HisRegCpy)
	   ) {
		*ireg |= HIS_TxDnLdRdy;
		if(!priv->wlan_dev.dnld_sent) {	// tx_done already received
			PRINTK("warning: tx_done already received: dnld_sent=0x%x ireg=0x%x cs=0x%x\n",priv->wlan_dev.dnld_sent,*ireg,cs);
			TXRX_DEBUG_GET_ALL(0x05, *ireg, cs);
		}
		else {
			PRINTK("tx_done received: dnld_sent=0x%x ireg=0x%x cs=0x%x\n",priv->wlan_dev.dnld_sent,*ireg,cs);
			priv->wlan_dev.dnld_sent = 0;
...
#ifdef THROUGHPUT_TEST
		||  !priv->adapter->NumTransfersRx) && (priv->adapter->ThruputTest & 0x02)
#endif /* THROUGHPUT_TEST */
	) {
...
int mv_sdio_read_event_cause(wlan_private *priv)
{
	...
	if ((ret = sdio_read_ioreg(priv->wlan_dev.card, FN0, 0x80fc, &scr2)) < 0) {
...
int msd25_dnld_ready(wlan_private *priv)
{
	int rval;
	u8 cs;
	wlan_dev_rec_t	*sdiodev = &priv->wlan_dev;
...
int sbi_disable_host_int(wlan_private *priv, u8 mask)
{
	...
	ret = sdio_read_ioreg(priv->wlan_dev.card, FN1, HOST_INT_MASK_REG, &host_int_mask);
	if (ret < 0)
		goto done;

	/* Update with the mask and write back to the register */
	host_int_mask &= ~mask;
	ret = sdio_write_ioreg(priv->wlan_dev.card, FN1, HOST_INT_MASK_REG, host_int_mask);
	if(ret<0)
...
#if 0
static int get_block_size(int nb, int *blklen)
{
/*
        int num, blksz, templen;
        ...
        blksz = 32 + (32 * num);
        *blklen = (nb + blksz - 1) / blksz;

        if(num == 15) {
                templen = nb / *blklen;
                templen = ((templen - 1) / 32) + 1;
                blksz = templen * 32;
        	*blklen = (nb + blksz - 1) / blksz;
        }
*/
	int blksz = SD_BLOCK_SIZE;
        *blklen = (nb + blksz - 1) / blksz;
        return blksz;
}
#endif
 ...
 */
int sbi_host_to_card(wlan_private *priv, u8 type, u8 *payload, int nb)
{
	int	ret;
	int	buf_block_len;
	int	blksz;
	...
	//enter1("mv_sdio_host_to_card");
	PRINTK("sbi_host_to_card\n");

	TXRX_DEBUG_GET_ALL(0x80, 0xff, 0xff);
	/* The host polls for the DN_LD_CARD_RDY and IO_READY bits */
//#ifndef DUMMY_PACKET
#if 0	/* When we get tx_done, we set the HIS_TxDnLdRdy bit in HisRegCpy in mv_sdio_read_scratch().
	   And in service_pending_int thread we check for HIS_TxDnLdRdy and call 
	   wlan_process_tx(). So 'priv->wlan_dev.dnld_sent' is not required to be checked */
	
	if(card->chiprev != 0x00) {
		if(priv->wlan_dev.dnld_sent) { 	// (DAT|CMD) tx_done not received yet
			printk("previous dnld_sent = 0x%x %s, now %s\n",priv->wlan_dev.dnld_sent,(priv->wlan_dev.dnld_sent&1)?"DAT":"CMD",(type==MVSD_DAT)?"DAT":"CMD");
{
#ifdef TXRX_DEBUG
			print_txrx_debug();
#endif
		...
		if(priv->wlan_dev.dnld_sent & 1) { 	// (DAT) tx_done not received yet
			printk("previous dnld_sent = 0x%x %s, now %s\n",priv->wlan_dev.dnld_sent,(priv->wlan_dev.dnld_sent&1)?"DAT":"CMD",(type==MVSD_DAT)?"DAT":"CMD");
			ret = -1;
			goto done;
		}
...
//		blksz = get_block_size(nb + 4, &buf_block_len);
	...
	if(type == MVSD_DAT)
		priv->wlan_dev.dnld_sent = 0x01;
	else
		priv->wlan_dev.dnld_sent = 0x02;
			 ...
			 u32 *type, int *nb, u8 *payload, int npayload)
{
	...
	/* The host polls for the UP_LD_CARD_RDY and IO_READY bits */
//#ifndef DUMMY_PACKET
#if 0
	if(card->chiprev != 0x00) {
		ret = mv_sdio_poll_card_status(priv, IO_READY | UP_LD_CARD_RDY);
		if (ret < 0)
			return ret;
	}
#endif
...
//		return -EINVAL;
	...
	/* Allocate buffer */
	if(card->chiprev != 0x00) {
//		blksz = get_block_size(buf_len, &buf_block_len);
		blksz = SD_BLOCK_SIZE_320;
 ...
 */
//int mv_sdio_probe(mmc_card_t card)
...
/*
	cis[256]=0;
	for(i=0;i<256;i++) {
		if(sdio_read_ioreg(card, FN0, 0x8000+i, &tmp) < 0) {
			printk("cannot read %d\n",i);
		}
		printk("%02x ",tmp);
		if(i>=4) cis[i-4]=tmp;
	}
...
*/
	...
	if(sdio_read_ioreg(card, FN1, CARD_REVISION_REG, &card->chiprev) < 0) {
		...
		PRINTK("manfid=0x%x  cardid=0x%x  revision=0x%x\n",parse.manfid.manf,parse.manfid.card,card->chiprev);
		switch(card->chiprev) {
#ifdef 		SD8385
		case 0x00:
			fmimage = fmimage_A0;
		...
		case 0x10:
			...
			sdio_write_ioreg(card, FN0, BUS_INTERFACE_CONTROL_REG, bic | 0x20);
			fmimage = fmimage_B0;
...
static int sbi_download_wlan_fw_image(wlan_private *priv, const u8 *firmware, int firmwarelen)
{
	...
	do {
		if((ret = sdio_read_ioreg(priv->wlan_dev.card, FN1, HOST_F1_RD_BASE_0, &base0)) < 0) {
			if(timeout>4995 || timeout<5)
				printk("Dev BASE0 register read failed: base0=0x%04X(%d)\n",base0,base0);
			ret = -1;
			goto done;
		}
		if((ret = sdio_read_ioreg(priv->wlan_dev.card, FN1, HOST_F1_RD_BASE_1, &base1)) < 0) {
			if(timeout>4995 || timeout<5)
				printk("Dev BASE1 register read failed: base1=0x%04X(%d)\n",base1,base1);
			ret = -1;
			goto done;
		}
	...
	for (offset = 0; offset < firmwarelen; offset += txlen) {
		...
		// The host polls for the DN_LD_CARD_RDY and IO_READY bits //
		ret = mv_sdio_poll_card_status(priv,
		...
		do {
			mdelay(1);
			if((ret = sdio_read_ioreg(priv->wlan_dev.card, FN1, HOST_F1_CARD_RDY, &base0)) < 0) {
				printk("Dev CARD_RDY register read failed: base0=0x%04X(%d)\n",base0,base0);
				ret = -1;
				goto done;
			}
			PRINTK("offset=0x%08X len=0x%04X: FN1, HOST_F1_CARD_RDY: 0x%04X\n",offset,txlen,base0);
		...
		if (len & B_BIT_0) {
			printk("CRC32 Error indicated by the helper: len=0x%04X(%d)\n",len,len);
			len &= ~B_BIT_0;
...
#ifdef FW_DOWNLOAD_SPEED
	tv2 = get_utimeofday();
	printk("firmware: %ld.%03ld.%03ld ",tv1/1000000,(tv1%1000000)/1000,tv1%1000);
	printk(" -> %ld.%03ld.%03ld ",tv2/1000000,(tv2%1000000)/1000,tv2%1000);
	tv2 -= tv1;
...
static int sbi_prog_firmware_image(wlan_private *priv, const u8 *firmware, int firmwarelen)
{
	int	ret;
	u16	firmwarestat;
	int	tries;
...
//	printk("Downloading firmware (%d bytes) ", firmwarelen);
	printk("Downloading "
#ifdef HELPER_IMAGE
		"helper"
#else
		"firmware"
#endif
		" image (%d bytes), block size %d bytes ", firmwarelen, SD_BLOCK_SIZE);
#ifdef FW_DOWNLOAD_SPEED
	tv1 = get_utimeofday();
#endif
	/* Perform firmware data transfer */
	...
	for (fwblknow = 0; fwblknow < firmwarelen;
		...
		ret = sdio_write_iomem(priv->wlan_dev.card, FN1,
			priv->wlan_dev.ioport, BLOCK_MODE, FIXED_ADDRESS,
				FIRMWARE_TRANSFER_NBLOCK, SD_BLOCK_SIZE, fwbuf);
...
#ifdef FW_DOWNLOAD_SPEED
	tv2 = get_utimeofday();
	printk("helper: %ld.%03ld.%03ld ",tv1/1000000,(tv1%1000000)/1000,tv1%1000);
	printk(" -> %ld.%03ld.%03ld ",tv2/1000000,(tv2%1000000)/1000,tv2%1000);
	tv2 -= tv1;
	printk(" == %ld.%03ld.%03ld\n",tv2/1000000,(tv2%1000000)/1000,tv2%1000);
#endif
	...
	ret = sdio_write_iomem((mmc_card_t) priv->wlan_dev.card, FN1, priv->wlan_dev.ioport,
				BLOCK_MODE, FIXED_ADDRESS, 1, SD_BLOCK_SIZE, fwbuf);
...
int sbi_verify_fw_download(wlan_private *priv)
{
...
#if 0
	ret = sdio_write_ioreg(priv->wlan_dev.card, FN1, HOST_INT_RSR_REG, 3);
	if (ret < 0) {
		printk("sdio_write_ioreg: writing interrupt RSR register failed\n");
		return -1;
	}
#endif
	...
	if (ret < 0) {
		printk("sdio_read_ioreg: reading interrupt RSR register failed\n");
		return -1;
	}
	else
...
#if 0
{
	mmc_card_t	card = (mmc_card_t) priv->wlan_dev.card;
	u8 ireg;
	printk("get int status\n");
	if (sdio_read_ioreg(card, FN1, HOST_INTSTATUS_REG, &ireg) < 0)
		printk("sdio_read_ioreg: get interrupt status register failed\n");
	else
...
inline int sbi_enter_deep_sleep(wlan_private *priv)
{
	...
	mdelay(2);
	ret = sdio_write_ioreg(priv->wlan_dev.card, FN1, CONFIGURATION_REG, HOST_POWER_DOWN);
...
#else
	ret = sdio_write_ioreg(priv->wlan_dev.card, FN1, CONFIGURATION_REG, HOST_POWER_UP);
#endif
