						...
						int index, int mode)
{
	...
	if (Adapter->BSSIDList[index].InfrastructureMode == mode) {
		if (Adapter->SecInfo.WEPStatus == Wlan802_11WEPDisabled
...
#endif	//WPA2
			&& Adapter->wpa_supplicant[index].Wpa_ie[0] != WPA_IE
#ifdef WPA2
			&& Adapter->wpa2_supplicant[index].Wpa_ie[0] != WPA2_IE
#endif	//WPA2
#endif	//WPA
#ifdef ADHOCAES
			...
			/* WPA enabled */
#ifdef WPA2
PRINTK("IsNetworkCompatible() WPA: index=%d wpa_ie=%#x wpa2_ie=%#x WEP=%s WPA=%s WPA2=%s EncMode=%#x privacy=%#x\n",index,Adapter->wpa_supplicant[index].Wpa_ie[0],Adapter->wpa2_supplicant[index].Wpa_ie[0],(Adapter->SecInfo.WEPStatus==Wlan802_11WEPEnabled)?"e":"d",(Adapter->SecInfo.WPAEnabled)?"e":"d",(Adapter->SecInfo.WPA2Enabled)?"e":"d",Adapter->SecInfo.EncryptionMode,Adapter->BSSIDList[index].Privacy);
#endif
			LEAVE();
			return index;
		}
...
#ifdef WPA
#ifdef WPA2
PRINTK("IsNetworkCompatible() dynamic WEP: index=%d wpa_ie=%#x wpa2_ie=%#x EncMode=%#x privacy=%#x\n",index,Adapter->wpa_supplicant[index].Wpa_ie[0],Adapter->wpa2_supplicant[index].Wpa_ie[0],Adapter->SecInfo.EncryptionMode,Adapter->BSSIDList[index].Privacy);
#else
PRINTK("IsNetworkCompatible() dynamic WEP: index=%d wpa_ie=%#x EncMode=%#x privacy=%#x\n",index,Adapter->wpa_supplicant[index].Wpa_ie[0],Adapter->SecInfo.EncryptionMode,Adapter->BSSIDList[index].Privacy);
#endif
#endif
			LEAVE();
			return index;
...
#else
PRINTK("IsNetworkCompatible() FAILED: index=%d wpa_ie=%#x WEP=%s WPA=%s EncMode=%#x privacy=%#x\n",index,Adapter->wpa_supplicant[index].Wpa_ie[0],(Adapter->SecInfo.WEPStatus==Wlan802_11WEPEnabled)?"e":"d",(Adapter->SecInfo.WPAEnabled)?"e":"d",Adapter->SecInfo.EncryptionMode,Adapter->BSSIDList[index].Privacy);
#endif
#endif
		LEAVE();
		return -ECONNREFUSED;
...
/*Action could be put to 11D state Machine*/
typedef enum _wlan_802_11d_action {
	CMD_INIT_11D = 0,
	CMD_RESET_11D,
	CMD_SET_DOMAIN_11D,
	CMD_DISABLE_11D,
	CMD_ENABLE_11D,
	CMD_ALLOW_USR_REGION_11D,
	CMD_DISALLOW_USR_REGION_11D,
	CMD_SET_REGION_11D,
	CMD_SCAN_RESP_COMPLIANT_11D,
	CMD_SCAN_RESP_NON_COMPLIANT_11D,
	CMD_DISCONNECT_11D,
	CMD_MAX_ACTION_11D,
	CMD_YIELD_11D = CMD_MAX_ACTION_11D, /*used to stop state-transfer*/
} wlan_802_11d_action; 
