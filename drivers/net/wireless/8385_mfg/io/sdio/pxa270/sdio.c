...
static inline void *register_user(mmc_notifier_t notifier)
{
  ...
  if(ret && ret->add) {
    ...
    for(i = 0; i < mmc_ncontrollers; i++) {
      ...
      down_read(&ctrlr->update_sem);    /* FIXME */
      __card_stack_foreach(&ctrlr->stack, ret->add, FALSE);
...
error:
  /* RPACE adder */
//    sdio_write_ioreg(ctrlr->stack.selected, FN0, 0x06, 0x08);
...
static int __init core_module_init(void)
{
  ...
  clr_GPIO(GPIO_WLAN_RESETB);        // do not reset WLAN
  ...
  ret = 0;
  direct_init();
  printk(KERN_INFO "SDIO subsystem (Marvell Semiconductor, Inc.)\n");
  drv_module_init();
  /* RPACE adder */
...
static int check_card_stack(mmc_controller_t ctrlr)
{
  ...
  if(!ctrlr){
	  printk("<1>ctrlr failed - check_card_stack\n");
    goto error;
  }
  if(ctrlr->stack.ncards > 0) {
	  printk("Entered\n");
    ...
    for(card = ctrlr->stack.first; card; card = card->next) {
      ...
      MMC_DEBUG(MMC_DEBUG_LEVEL2, "CMD13(0x%04x%04x)\n", argh, argl);
      ret = complete_cmd(ctrlr, MMC_R1, FALSE);
      if(ret){
	      printk("Card status is unpluggged\n");
	      printk("state = %x\n",hostdata->state);
...
//#if CONFIG_MMC_DEBUG_IRQ
  ...
  if(save_ireg & MMC_I_REG_RES_ERR) {
	...
	MMC_I_MASK = new_ireg;
	wmb();
        printk("<1>irq(%ld) mmc_i_reg=%x(%x), int=%d, MMC_STAT=%x, MASK=%x->%x(%x), ICMR=%x ICLR=%x ICIP=%x ICPR=%x ICCR=%x %d\n", irqnum,hostdata->mmc_i_reg,save_ireg,sdio_interrupt_enabled, sreg, old_ireg,new_ireg,MMC_I_MASK, icmr,iclr,icip,icpr,iccr,hostdata->irqcnt);
  }
...
static int suspend(mmc_controller_t ctrlr)
{
  ...
  if(hostdata->state == PXA_MMC_FSM_IDLE) {
    ...
    if((ret = complete_cmd(ctrlr, MMC_NORESPONSE, FALSE))) {
      ret = -EIO;
      goto error;
    }
#endif
...
static int __devinit drv_module_init(void)
{
  ...
  printk(KERN_INFO "%sMMC controller -SDIO protocol(Marvell Semiconductor, Inc.)\n",
...
MODULE_DESCRIPTION("SD83xx SDIO Driver");
MODULE_SUPPORTED_DEVICE("Xscale");
