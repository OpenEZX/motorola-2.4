 ...
 *  Date          Author                Comment
 * ===========   ========   =====================================================
 * 2004-Jan-12   Motorola   add queue apm event
 * 2005-Jan-18   Motorola   add handling of RTC Stopwatch
 * 2005-Mar-18   Motorola   add handling of RTC Periodic
 * 2006-Sep-19   Motorola   fixed delete request bug from sw request list 
 * 2006-Oct-09   Motorola   close debugging info  
 * 2006-Oct-12   Motorola   correct wrong writting value code to RTSR and remain time compare bug 
 * 2006-Oct-26   Motorola   remove debugging loges 	
 * 2006-Dec-05   Motorola   close PIAR and SWAR after using to resolve potential bug and update comments
 */
...
#ifdef DEBUG
                        printk("del request<interval:%ld remain:%ld>.\n", time, req->remain);
			if(req->process != NULL)
				printk("del request pid:%d \n", req->process->pid);
#endif
...
static void update_one_request(struct rtc_sw_request *req, unsigned long hun)
{
	if(req == NULL)
		return;
#ifdef DEBUG
        printk("<%s>: hundredths: %ld remain: %ld\n", __FUNCTION__, hun, req->remain);
	if(req->process != NULL)
		printk("update request pid:%d \n", req->process->pid);
#endif
        if (req->remain <= hun) {
                /* wake up the process */
#ifdef DEBUG
                if (req->process)
                  printk("wake up the process<%d>.\n",req->process->pid);
#endif
...
static void rtc_sw_app_exit(void)
{
        ...
        unsigned long slice;
	unsigned long flags;
        struct rtc_sw_request *del_req_list = NULL;
        int ret = -1;
	...
	if ((RTSR & RTSR_SWCE) && (RTSR & RTSR_SWALE1)) {
		...
		RTSR = (RTSR & RTSR_STATUS_BITS_MASK) & ~RTSR_SWALE1;
	} else if ((RTSR & RTSR_PICE) && (RTSR & RTSR_PIALE)) {
		RTSR = (RTSR & RTSR_STATUS_BITS_MASK) & ~RTSR_PICE;
		slice = get_slice_past(INT_PERIODIC, CALLER_IOCTL);
		RTSR = (RTSR & RTSR_STATUS_BITS_MASK) & ~RTSR_PIALE;
	} else 
		slice = 0;
	...
	while ((req = request_del_process(current)) != NULL) {
		req->next = del_req_list;
		del_req_list = req;
		ret = 0;
	}

	/*
	 * We will update the list after delete the request from the list, 
	 * in case of current request also arrives time to be waked up.
	 */
	if (slice > 0)
	...
	if (req != NULL) {
		...
		if ((req->remain * 10) > RTCPICR_MAX) {
			...
			udelay(1);
			RTSR = (RTSR & RTSR_STATUS_BITS_MASK) | (RTSR_SWALE1 | RTSR_SWCE);
		} else {
			ret = set_rtc_pi_reg(req->remain);
			udelay(1);
			RTSR = (RTSR & RTSR_STATUS_BITS_MASK) | (RTSR_PIALE | RTSR_PICE);
		}
	...
	while(del_req_list != NULL)
	{
		req = del_req_list->next;
		request_free(del_req_list);
		del_req_list = req;	
	}
	
	if( ret < 0 )
		printk("%s: current process no request delete or reschedule sw erro ", __FUNCTION__);
...
static void rtc_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
        ...
        printk("Con. RCNR: %08x, RTAR: %08x, PIAR: %08x, SWAR1: %08x, SWAR2: %08x\n", 
                RCNR, RTAR, PIAR, SWAR1, SWAR2);
#endif
        ...
        if ((rtsr & RTSR_PIAL) || (rtsr & RTSR_SWAL1)) {
                ...
                if (rtsr & RTSR_PIAL) {
			RTSR = (RTSR & RTSR_STATUS_BITS_MASK) | RTSR_PIAL;
			RTSR = (RTSR & RTSR_STATUS_BITS_MASK) & ~RTSR_PICE;
                } else {
			RTSR = (RTSR & RTSR_STATUS_BITS_MASK) | RTSR_SWAL1;
			RTSR = (RTSR & RTSR_STATUS_BITS_MASK) & ~RTSR_SWCE;
                }
...
#endif
			RTSR = (RTSR & RTSR_STATUS_BITS_MASK) &
				 ~(RTSR_PICE | RTSR_PIALE | RTSR_SWCE | RTSR_SWALE1);
                } else {
                        if ((req->remain * 10) > RTCPICR_MAX) {
                                ret = set_rtc_sw_reg(req->remain);
                                udelay(1);
		...
		RTSR = (RTSR & RTSR_STATUS_BITS_MASK) | (RTSR_AL | RTSR_HZ);

                /* clear alarm interrupt if it has occurred */
                if (rtsr & RTSR_AL)
			RTSR = (RTSR & RTSR_STATUS_BITS_MASK) & ~RTSR_ALE;
		     ...
		     unsigned int cmd, unsigned long arg)
{
	...
	case RTC_UIE_OFF:
		spin_lock_irq(&rtc_lock);
		RTSR = (RTSR & RTSR_STATUS_BITS_MASK) & ~RTSR_HZE;
		rtc_irq_data = 0;
		spin_unlock_irq(&rtc_lock);
		return 0;
	case RTC_UIE_ON:
...
#ifdef DEBUG
	printk("%s:pm callback %d RTSR=%08x ICPR=%08x\n", __FUNCTION__ , req, RTSR, ICPR );
#endif
