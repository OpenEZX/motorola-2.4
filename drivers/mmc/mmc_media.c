 ...
 * Author            Date             Comment
 * =======         =========          ======================  
 * Motorola        05/27/2004         change for sync fs when close device
 * Motorola        04/15/2005         change for ioctl 
 * Motorola        12/21/2005         add Macau support
 * Motorola        06/15/2006         add PengLai support
 * Motorola        01/10/2007         RF influence SD issue
 * Motorola        02/03/2007         SD card format issue
 * Motorola        03/06/2007         Application hang issue
*/ 
...
extern int mmc_crc_error;
extern u32 sd_status;

static struct mmc_io_request  g_io_request_tmp;
static int retry_cnt = 0;
static int reinit_cnt =0;
static int end_error_mmc =0;
int re_insert_flag = 0;
int error_lock_flag = 0;
int haspwd = -1;

struct timer_list mmc_error_timer;
void mmc_error_timer_callback( unsigned long data );
void mmc_error_reset( void );

void mmc_error_reset( void )
{
	DEBUG(2, "ERROR - enter mmc_error_reset!\n");
	retry_cnt = 0;
	reinit_cnt = 0;
	end_error_mmc = 0;
	re_insert_flag = 0;
...
static void error_process(int res)
{
	int cardexist;
	...
	if( (0==haspwd) || (1==haspwd) || (1==error_lock_flag))
	{//SD card has pwd or locked
		DEBUG(2, "ERROR - Locked or PWD, res:%d\n", res);
		mmc_error_reset();
		end_request(res);
		g_busy = 0;
		return;
	}
	if( sd_status&R1_OUT_OF_RANGE )
	{
		printk("OUT_OF_RANGE, will return error, %08x\n", sd_status);
		sd_status = 0;
		mmc_error_reset();
		end_request(res);
		g_busy = 0;
        ...
        cardexist = (~GPLR(GPIO_MMC_DETECT)) & GPIO_bit(GPIO_MMC_DETECT);
	if( !cardexist )
	{
		mmc_error_reset();
		end_request(0);
	...
	printk("MMC ERROR, card status:%08x\n", sd_status);
	if(end_error_mmc)
	{//At last, should send error to upper!
		DEBUG(2, "ERROR - Error latest, retry_cnt:%d, reinit_cnt:%d!\n", retry_cnt, reinit_cnt);
		mmc_error_reset();
                end_request(res);
		g_busy = 0;
		return;
	...
	if( retry_cnt >4)	//5 times
	{//Re-init
		DEBUG(2, "ERROR - Re-init. reinit_cnt:%d, Retry:%d!\n", reinit_cnt, retry_cnt);
		retry_cnt = 0;
		mmc_crc_error = 0;	//Before re-try, clear the CRC
		re_insert_flag = 1;
		reinit_cnt++;
		enable_irq(IRQ_MMC);
		mmc_insert(0);
		mmc_error_timer.expires = jiffies + 2*HZ;
		add_timer(&mmc_error_timer);
		return;
	}
	else
	{//re-try
		DEBUG(2, "ERROR - Retry:%d!\n", retry_cnt);
		mmc_crc_error = 0;	//Before re-try, clear the CRC
		enable_irq(IRQ_MMC);
		mmc_handle_io_request(&g_io_request_tmp);
		retry_cnt++;
		return;
	}
}
...
void mmc_error_timer_callback( unsigned long data )
{
	if (reinit_cnt>3)	//3 times
	{
		DEBUG(2, "ERROR - Timer in, finnal error!\n");
		retry_cnt = 0;
		reinit_cnt = 0;
		end_error_mmc = 1;
		enable_irq(IRQ_MMC);
	...
	if ( re_insert_flag==1 )
	{
		DEBUG(2, "ERROR - Timer in, will re-init!\n");
		mmc_crc_error = 0;	//Before re-try, clear the CRC
		reinit_cnt++;
		enable_irq(IRQ_MMC);
		mmc_insert(0);
	...
	{
		DEBUG(2, "ERROR - Timer in, will send CMD again!\n");
		retry_cnt = 0;
		mmc_crc_error = 0;	//Before re-try, clear the CRC
		enable_irq(IRQ_MMC);
		mmc_handle_io_request(&g_io_request_tmp);
...
//#ifdef CONFIG_ARCH_EZX_E680		
		/* add  to prevent from flushing data when card is removed */
		if (!mmc_slot_enable)
		{
		    if (READ == CURRENT->cmd)
		    {
...
#else
#if defined(CONFIG_ARCH_EZX_MACAU) || defined(CONFIG_ARCH_EZX_PENGLAI)
...
//#endif
	...
	init_timer(&mmc_error_timer);
	mmc_error_timer.function = mmc_error_timer_callback;
	mmc_error_timer.data = (unsigned long)11;
	mmc_error_timer.expires = jiffies + 2*HZ;
	
	DEBUG(2, "ERROR - Timer init over!\n");
