...
static int uart_open(struct tty_struct *tty, struct file *filp)
{
	...
	int stuart_state;
#ifdef DEBUG
	printk("uart_open(%d) called, MAJOR is %d, MINOR is %d\n", line, drv->normal_major, drv->minor);
#endif
...
/* To enable clock WUL */
	...
	if(stuart_state != 0) {
		return -EIO;
		printk("uart_open failed, stuart_state = %d\n",  stuart_state);
	}
//	else
//		printk("open STUART and dc_enable \n");
#ifdef DEBUG	
	printk("open STUART and dc_enable \n");
	printk("uart_open success\n");
#endif
	return 0;
...
extern void pxa_rs_console_init(void);
