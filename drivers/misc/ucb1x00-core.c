...
static int ucb1x00_pm (struct pm_dev *dev, pm_request_t rqst, void *data)
{
	...
	static int      ts_cr,  adc_cr, pcsr,  fcsr;

	if (rqst == PM_RESUME)
	{
	/*
	 *  resume still have problems if we enable the code here..
	 *  but simply it works when there is no code at all.
	 */
#if 0
	ucb1x00_enable(ucb);
	...
	ucb1x00_reg_write(ucb, UCB_ADC_CR,pcsr);
	ucb1x00_reg_write(ucb, UCB_ADC_CR,0);   /* first set PR1 =0 */
	fcsr |= UCB_FCSR_HPEN;
	ucb1x00_reg_write(ucb, UCB_ADC_CR,fcsr);    /* then HPEN =1 */
	...
	{
	/*
	 *  Should disable ADC, AC97 link, touchscreen, etc.
	 */
	ucb1x00_enable(ucb);

	ts_cr  = ucb1x00_reg_read(ucb, UCB_TS_CR);
	adc_cr = ucb1x00_reg_read(ucb, UCB_ADC_CR);
	pcsr   = ucb1x00_reg_read(ucb, UCB_PCSR);
	fcsr   = ucb1x00_reg_read(ucb, UCB_FCSR);

	ucb1x00_reg_write(ucb, UCB_TS_CR, 0);   /*  touchscreen power down.*/
	ucb1x00_reg_write(ucb, UCB_ADC_CR, 0);  /*  ADC power down.*/
	ucb1x00_reg_write(ucb, UCB_PCSR, 0);
	ucb1x00_reg_write(ucb, UCB_FCSR, 0);
...
static int __init ucb1x00_init(void)
{
	...
	if (my_ucb->pmdev == NULL)
		printk("ucb1x00-core: unable to register in PM.\n");
	else
