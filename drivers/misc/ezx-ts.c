 ...
 * Author   Date            Comment
 * ======   ======          ==========================
 * Motorola 09/05/2005      new touch pannel sample points logic base on the SPI bus interruptable
 * Motorola 09/22/2005      retrun last point if the new sample point is not too far
 * Motorola 11/23/2005      fix the jump point handle logic bug
 * Motorola 12/01/2005      add wakeup protection for AtoD finish interrupt
 * Motorola 30/03/2006      when backlight off, ts driver should not send coordinates events for
 *                          first tap on touch screen, except to arose pm to turn on backlight
 *                          Notice: pxafb.c will also be modifed for the issue.
 * Motorola 15/05/2006      add a flag to control whether touch screen send ts events to user space
 *                          when bklight is turned off.
...
/* 
 * used to indicate whether ts is to  send ts events to user space
 * when bklight is off but ap does not enter into sleep mode.
 * 1 = bklightoffsentevents_flag means open the mode, 
 * e.g. TCMD bklight test case.
 * 0 = bklightoffsentevents_flag means close the mode, default mode
 * for mostly common users.
 */
static int bklightoffsentevents_flag = BKLIGHTOFFSENDEVENTS_OFF;
...
//		touchscreen_ioctl(POWER_IC_IOCTL_TS_SET_INTERVAL, 1);
	...
	case TS_BKLIGHTOFF_SENDEVENTSMODE:
	    	TS_DPRINTK("ezx_ts_ioctl_test: This is TS_BKLIGHTOFF_SENDEVENTSMODE.\n");
		/* we only accept those values now.*/
                if(BKLIGHTOFFSENDEVENTS_ON == arg || BKLIGHTOFFSENDEVENTS_OFF == arg)
			bklightoffsentevents_flag = arg;
		else
			res = -EINVAL;
		break;
...
/*
 * used to indicate whether backlight is on or off.
 * 1 = bklight_onoff_flag means backlight on
 * 0 = bklight_onoff_flag means backlight off
 * the flag is set/clear in pxafb_ioctl() FBIOSETBKLIGHT BKLIGHT_OFF/BKLIGHT_ON
 * through ezx_ts_set_backlight_flag()
 */
static int bklight_onoff_flag = 1;

void ezx_ts_set_backlight_flag(int onoff )
{
        bklight_onoff_flag = onoff;
}
...
{
        if((bklight_onoff_flag == 0 && bklightoffsentevents_flag == BKLIGHTOFFSENDEVENTS_OFF))
        {
                touchScreen_send_pmMessage();
                del_timer(&tsi_keep_timer);
                enable_tsi_again(0);
