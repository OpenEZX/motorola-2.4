 ...
 * ----------------   ------------------------
 *     09/05/2005      new touch pannel sample points logic base on the SPI bus interruptable
 *
 *     09/22/2005      retrun last point if the new sample point is not too far
 *
 *     11/23/2005      fix the jump point handle logic bug
 *
 *     12/01/2005      add wakeup protection for AtoD finish interrupt
 *
 *     03/20/2006      remove SPI support
 *
 */
...
void touchScreen_wakeUpCallBack(void)
{
    ...
    */
    ...
    {
        int val;
        power_ic_read_reg(POWER_IC_REG_PCAP_ISR,&val);
        if(val&PCAP_ISR_TSI)
        {
