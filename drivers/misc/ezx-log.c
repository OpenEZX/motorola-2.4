 ...
 * 0.03 2005-12-21  add Macau support
 * 0.04	2006-03-10  change the panic log design and use the hidden part of mtd 
 * 0.05 2006-06-15  add Penglai support
 * 0.06 2006-06-29  support APR phase II
 * 0.07 2006-08-18  show alert text on lcd as interprocessor communication paralysis
 */
...
static unsigned char g_ezxlog__panic_type = APR_AP_KERNELPANIC_TYPE_DEFAULT_EV; // =default panic value


/* notice! type should be less than 256 */
void log_set_panic_type(unsigned char type)
{
	g_ezxlog__panic_type = type;
}
...
static struct panic_log_header* init_log_header(void)
{
	...
	sprintf(log_header->panic_type, " %03d", g_ezxlog__panic_type);
...
void panic_enable_pxafb_bklight(void);
void panic_enable_pxafb_controller(void);
#endif
...
static ssize_t log_write(const char *buf, size_t count)
{
	...
	{
		panic_enable_pxafb_controller();
		printout_bp808a5_string();
		panic_enable_pxafb_bklight();
	}
	else { 
#endif
