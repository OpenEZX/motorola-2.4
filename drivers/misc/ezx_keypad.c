 ...
 *  Author     Date                Comment
 * ======== ===========   =====================================================
 * Motorola 2004-Nov-25   Fix some bugs and re-organize the file.
 * Motorola 2005-Oct-10   Add kernel panic triggger.
 * Motorola 2005-Dec-21   Add MACAU support. 
 * Motorola 2006-Mar-15   Make supporting of multiple keypress a option.
 * Motorola 2006-Mar-30   Disable the mutiple keypress for sumatra.
 * Motorola 2006-Jun-15   Add PENGLAI support.
 * Motorola 2006-Jun-29   Add APR phase II support.
 * Motorola 2006-Sep-04   Fix: call wrong function in a interrupt context.
 * Motorola 2006-Oct-17   Disable 3keypad panic tirgger for macau.
...
/* Disable multiple keypress for Sumatra */
#ifdef CONFIG_KEYPAD_SUMATRA
#undef KEYPAD_MULTIPLE_KEYPRESS_SUPPORT
#undef KEYPAD_TRIGGER
#endif /* CONFIG_KEYPAD_SUMATRA */

#if defined(CONFIG_KEYPAD_BARBADOS) || defined(CONFIG_KEYPAD_MARTINIQUE) || \
               defined(CONFIG_KEYPAD_HAINAN) || defined(CONFIG_KEYPAD_PENGLAI)
#define KEYPAD_MULTIPLE_KEYPRESS_SUPPORT	/* multiple keypress support */
/* 
 * panic log trigger
 * NOTE: must also enable KEYPAD_MULTIPLE_KEYPRESS_SUPPORT
 */
#define KEYPAD_TRIGGER
#endif 

#ifdef CONFIG_KEYPAD_MACAU
#define KEYPAD_MULTIPLE_KEYPRESS_SUPPORT
#undef KEYPAD_TRIGGER
#endif /* CONFIG_KEYPAD_MACAU */ 

/*
 * If we disabled the KEYPAD_MULTIPLE_KEYPRESS_SUPPORT feature, and now 
 * we press multiple keys to wake up Bulverde, the key driver won't send out
 * any key events. But from the key server's point of view, it does not 
 * know whether this is a "multiple keys' wakeup" or "single key press wakeup",
 * it will eliminate the "wakeup key event" as before. 
 * 
 * When the key server doing this, user will observe an interesting scenario
 * that I pressed multiple keys to wake up phone, then I press one key to 
 * begin my input, but the key input does't have any effect. (be eliminated
 * by key server)
 *
 * So when we disable KEYPAD_MULTIPLE_KEYPRESS_SUPPORT, we must send key
 * server a fake key event. This is how below macro going.
 */
#ifndef KEYPAD_MULTIPLE_KEYPRESS_SUPPORT
#define KEYPAD_MULTIPLE_KEYPRESS_FAKE
#endif
...
/* Product specific defines */
...
#elif defined(CONFIG_KEYPAD_MACAU)
    #define SWITCH_STATUS_E             SCREEN_LOCK_STATUS_E
    #define SWITCH_LOCKED               SCREEN_LOCK_LOCKED
    #define SWITCH_UNLOCKED             SCREEN_LOCK_UNLOCKED
    ...
    #define KEYPAD_IOC_GET_SWITCH_STATUS  KEYPAD_IOC_GET_SCREEN_LOCK_STATUS
    #define KEYPAD_IOC_SET_SWITCH_STATUS  KEYPAD_IOC_SET_SCREEN_LOCK_STATUS
#elif defined(CONFIG_KEYPAD_PENGLAI)
    #define SWITCH_STATUS_E             SCREEN_LOCK_STATUS_E
    #define SWITCH_LOCKED               SCREEN_LOCK_LOCKED
    #define SWITCH_UNLOCKED             SCREEN_LOCK_UNLOCKED
    #define EVENT_DEV_SWITCH            EVENT_DEV_FLIP
...
#ifdef KEYPAD_MULTIPLE_KEYPRESS_SUPPORT
...
static int scanbit_to_keycode[] = {
        ...
        KEYPAD_NAV_LEFT, KEYPAD_NONE, KEYPAD_NONE, KEYPAD_NONE,
        /* col 2 */
        KEYPAD_SIDE_DOWN, KEYPAD_LAUNCH, KEYPAD_CAMERA, KEYPAD_SEND,
        KEYPAD_SIDE_UP, KEYPAD_NONE, KEYPAD_NONE, KEYPAD_NONE,
        /* col 3 */
        KEYPAD_NONE, KEYPAD_NONE, KEYPAD_NONE, KEYPAD_NONE,
        KEYPAD_NONE, KEYPAD_NONE, KEYPAD_NONE, KEYPAD_NONE,
        ...
        KEYPAD_NONE, KEYPAD_NONE, KEYPAD_NONE, KEYPAD_NONE,
        /* col 5 */
        KEYPAD_AUD_PRV, KEYPAD_NONE, KEYPAD_CARRIER, KEYPAD_PLAY_PAUSE, KEYPAD_AUD_NEXT,
        KEYPAD_NONE, KEYPAD_NONE, KEYPAD_NONE, KEYPAD_NONE,
};
#elif CONFIG_KEYPAD_PENGLAI
static int scanbit_to_keycode[] = {
...
#else
scan_to_bitmap(unsigned long kpas)
#endif
{
        struct keypad_s *p = &ezx_keypad;
         ...
         * for macau, this means:
         *             SIDE_UP, SIDE_DOWN and SEND
         */
        if (kpasmkp1 == 0x19) {
		// set panic type
		log_set_panic_type(APR_AP_KERNELPANIC_TYPE_HANG_EV);
        	/* trigger panic log */
        	printk("PANIC LOG TRIGGERED.\n");
		BUG();
	}
#endif /* KEYPAD_TRIGGER */
...
#endif /* KEYPAD_MULTIPLE_KEYPRESS_SUPPORT */

#ifdef KEYPAD_MULTIPLE_KEYPRESS_FAKE
	/* KEYPAD_ACTIVATE is a fake key event which is not really used */
	ret = turn_on_bit(p->keybitmap, KEYPAD_ACTIVATE);
#endif
        return ret;
}
...
#if defined(CONFIG_KEYPAD_MACAU) || defined(CONFIG_KEYPAD_PENGLAI)
                /* Mask the touchscreen interrupt */
                spi_int_set_reg_bit(POWER_IC_REG_PCAP_IMR, POWER_IC_EVENT_PCAP_TSI, 1);
                /* Disable TS_REF */
                spi_int_set_reg_bit(POWER_IC_REG_PCAP_ADC1, 21, 1);
		/* Set touchscreen mode */
		spi_int_get_reg_value(POWER_IC_REG_PCAP_ADC1, 17, &ts_mode_save, 3);
		spi_int_set_reg_value(POWER_IC_REG_PCAP_ADC1, 17, 0x6, 3);
#endif
#ifdef KEYPAD_DEBUG
...
#if defined(CONFIG_KEYPAD_MACAU) || defined(CONFIG_KEYPAD_PENGLAI)
		if (ts_mode_save != -1) {
			/* Restore touchscreen mode */
			spi_int_set_reg_value(POWER_IC_REG_PCAP_ADC1, 17, ts_mode_save, 3);
			/* Enable TS_REF */
			spi_int_set_reg_bit(POWER_IC_REG_PCAP_ADC1, 21, 0);
			/* Unmask the touchscreen interrupt */
			spi_int_set_reg_bit(POWER_IC_REG_PCAP_ISR, POWER_IC_EVENT_PCAP_TSI, 1);
			spi_int_set_reg_bit(POWER_IC_REG_PCAP_IMR, POWER_IC_EVENT_PCAP_TSI, 0);
		}
#endif
#ifdef KEYPAD_DEBUG 
...
{
        unsigned long data;
        static unsigned long pre_data = -1;


        data = GPLR(GPIO_FLIP_PIN) & GPIO_bit(GPIO_FLIP_PIN);
	
	if (data == pre_data)  /* switch status is not changed. */
		return;
...
static void kp_interrupt(int irq, void *ptr, struct pt_regs *regs)
{
        ...
        if (kpc_val & KPC_MI) {
                ...
                p->kpas = KPAS;
#ifdef KEYPAD_MULTIPLE_KEYPRESS_SUPPORT
		while (KPASMKP0 & KPASMKPx_SO)
			;
                p->kpasmkp0 = KPASMKP0;
		while (KPASMKP1 & KPASMKPx_SO)
			;
                p->kpasmkp1 = KPASMKP1;
		while (KPASMKP2 & KPASMKPx_SO)
			;
                p->kpasmkp2 = KPASMKP2;
#endif
