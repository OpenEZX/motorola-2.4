...
static int __init pxa_ac97_init(void)
{
	...
	dummy->pmdev=pm_register(PM_SYS_DEV, PM_SYS_UNKNOWN, pxa_ac97_pm_callback);
	printk(KERN_INFO "In file %s, pm_registered.\n", __FILE__);
	if (dummy->pmdev == NULL)
		printk("PXA AC97: unable to register in PM.\n");
	else    {
		dummy->pmdev->data = dummy;
	}
#endif
	return 0;
}
...
#ifdef CONFIG_PM
	if ( ac_pm_use_point->pmdev)
		pm_unregister( ac_pm_use_point->pmdev);
#endif
	pxa_ac97_put();
}
