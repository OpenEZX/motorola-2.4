 ...
 *  History:
 *  Jun 20,2002             created
 *  Sep 19,2002             according code review meeting minutes.
 *  Oct 30,2002             according new requirement for VA.ASSP interface split to
 *                                             /dev/dsp (support stereo playback) and /dev/dsp16 (support
 *                                             mono playback and record) this file is for stereo playback.
 *  Nov 05,2002             according code review meeting minutes.
 *  Jan 13,2003             (1) add audio panic return value
 *                                             (2) modify sample frequency to standard
 *  Mar 03,2003             (1) open headset interrupt
 *                                             (2) change gain when headset is in
 *                                             (3) add ioctl to get headset status
 *  Apr 17,2003             (1) according codec_dac_rate init pcap
 *  Apr 18,2003             (1) change output gain according output path
 *  Apr 24,2003             (1) no switch when headset insert and remove
 *  May 21,2003             (1) modify loudspk gain max 0db, for audio-shaping
 *  Sep 23,2003             (1)Port from EZX; (2)Modify the ASSP port inital
 *  Feb 04,2004             haptics feature added
 *  Feb 23,2004             haptics GPIO initialization change
 *  Feb 26,2004             Add DAI test
 *                          Add control to switch PCAP CODEC mode from master to slave mode
 *  Mar.15,2004             mixer bug fix
 *  Mar 17,2004             GPIO change for haptics filter & boomer mute while setting haptics.
 *  Apr.13,2004             close dsp protection,and add 3d control interface for app
 *  Apr.23.2004             Add EMU Carkit 
 *  May.23.2004             Add the EMU audio test
 *  Jun.08,2004             record noise bug fix.
 *  Jun.22,2004             mixer power save
 *  Aug.11,2004             audio pcap LOW_POWER bit initialize
 *  Mar.10,2005             Change the debug log for barbados 
 *  Oct.30,2007             Removing changes from MP3 noise fix
 *
 */
