 ...
 * 2005/12/21
 *	 - support Macau product
 * 2006/06/06 
 * 	- change memory layout for macau (Add 1M space to resource area, and reduce 1M from language space)
 * 2006/06/15
 *	- support Penglai product
 * 2006/08/09
 * 	- modify memory map to support multi-product
 * 2006/08/18
 *      - Add interface to show panic text for interprocessor communication paralysis
 */
...
/* since barbados, logo source image file is always 24bpp with 3 bytes */
#ifdef CONFIG_ARCH_EZXBASE
/* 
	NOTE: In EZXBASE, Logo is shown by boot loader, not kernel now.
 */

#define MOT_LOGO_LINE		LOGO_W*3
#define IMAGE_SIZE		LOGO_H*MOT_LOGO_LINE
#define FILE_SIZE		IMAGE_SIZE+0x100
#define LOGO_ADDR               (MMAP_LOGO_START + MMAP_SECURE_HEADER_2KB)
...
/*---------------------------------------------------------------------------
 Font table for "BP 808A5"
---------------------------------------------------------------------------*/

static unsigned char bp808a5_fontcode16[]=	
        {
                /* font code of 'B' */

                0x00,0x00,0x7e,0x7f,0x63,0x63,0x63,0x7e,
                0x7e,0x63,0x63,0x63,0x63,0x7e,0x7c,0x00,

                /* font code of 'P' */

                0x00,0x00,0x7e,0x7f,0x63,0x63,0x63,0x7e,
                0x7c,0x60,0x60,0x60,0x60,0x60,0x60,0x00,
                ...
                0x00,0x00,0x3c,0x66,0xc3,0xc3,0xc3,0x66,
                ...
                0x00,0x00,0x18,0x3c,0x66,0xc3,0xc3,0xc3,
                0xc3,0xc3,0xff,0xff,0xc3,0xc3,0xc3,0x00,

                /* font code of '5' */

                0x00,0x00,0x7e,0x7e,0x60,0x60,0x60,0x6c,
                0x7e,0x03,0x03,0x03,0x43,0x7e,0x3c,0x00,
        };

static void printout_bp808a5_char(unsigned char *font ,unsigned int offset_row, unsigned int offset_col)
{
        struct display *p = &fb_display[fg_console]; /* draw to vt in foreground */
        unsigned char *pix;
        unsigned int f;
        ...
        offset = (offset_row *240 + offset_col) * 3;
...
/*Used to printout string:"kernel panic BP 808A5" on main panel*/
void printout_bp808a5_string(void)
{
    struct display *p = &fb_display[fg_console]; /* draw to vt in foreground */
    unsigned char *fb = p->screen_base;
    unsigned long i, pixel_count;
    ...
    unsigned char *font;
    unsigned int offset_row = 0,offset_col = 0;
    ...
    /*Print out string "kernel panic" on left top */	
    for(i=0; i<STR_LEN; i++) {
          printout_char(i,lcd_row,lcd_col++);
    }

    /*Print out string "BP 808A5" on central position*/
    offset_row = 151;
    offset_col = 80;	
    font = &bp808a5_fontcode16[0];
		
    for(i=0; i<8; i++) {
          printout_bp808a5_char(font,offset_row,offset_col);	
	  font += 16;
	  offset_col += 11;	
    }
}
...
static int __init fbcon_show_logo( void )
{
	...
	#endif
/*
 	We add decision of Power cut here, but we have to make sure
...
#ifdef CONFIG_ARCH_EZXBASE
//	    image_data = image + 14 + 2 + 40;  /*discard bitmap header and infoheader structures */
	    image_data = image + 14 + 40;  /*discard bitmap header and infoheader structures */
#else
	    image_data = image + 72;
#endif
