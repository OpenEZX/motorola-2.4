...
/* 
 *  Camera driver states
 */
typedef enum
{
	CAMERA_STATE_INVALID = 0,
	CAMERA_STATE_IDLE,	/* camera driver is not used */
	CAMERA_STATE_APP,	/* camera driver is opened by application */
	CAMERA_STATE_UVC,	/* camera driver is opened by UVC driver, 
				   system in web camera mode */
} camera_state_t;
...
typedef struct {
    ...
    int (*camera4uvc_command)(p_camera_context_t, unsigned int cmd, void *param);
