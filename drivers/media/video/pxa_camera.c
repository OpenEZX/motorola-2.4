 ...
 * Motorola     06/10/2004    add mt9v111 support
 * Motorola     01/06/2006    Add camera4uvc interface for web camera
 * Motorola     01/23/2006    Handle fifo overrun 
 * Motorola     04/21/2006    fix comments
...
camera_context_t  *g_camera_context = NULL;
#ifdef  CONFIG_ARCH_EZX_E680
extern camera_function_t  e680_camera_func;
#endif
  #ifdef CONFIG_CAMERA_SENSOR13M
  extern camera_function_t  sensor13m_func;
  extern int camera_func_sensor13m_standby(void); /*work around, resign later*/
  #endif
  ...
  #endif
  #ifdef CONFIG_CAMERA_SENSOR2M
  extern camera_function_t  sensor2m_func;
  extern int camera_func_sensor2m_s9(void); /*work around, resign later*/
  #endif
...
spinlock_t camera_state_lock;
camera_state_t camera_state;
static int captureed_count;
...
int camera_init(p_camera_context_t camera_context)
{
  ...
  #ifdef CONFIG_CAMERA_SENSOR13M
    ddbg_print("detect SENSOR13M...");
    camera_context->camera_functions = &sensor13m_func;
...
#ifdef CONFIG_USBD_UVC
    /* If camera device is acquired by UVC, the data buffer must be reorganize to
     * comply with UVC requirement. 
     * the buffer split to USB frames (section), and every section include a header
     * and a payload, the section size is (header_size + payload_size), every page
     * can store sections less than PAGE_SIZE/(header_size+payload_size).
     * we assume max capture size 640*480, and the buffer page size must large 
     * than
     *  ((((640*480) * 2) * 3 + payload_size -1) / payload_size) / 
     *  		((int)(PAGE_SIZE/(header_size + payload_size))
     */ 
    if ( camera_state == CAMERA_STATE_UVC ) {
	    int y_descrips, cb_descrips;
	    int y_size = camera4uvc_calc_size(
			    CAMERA4UVC_MAX_FRAME_SIZE/2, NULL, 
			    &y_descrips, NULL);
	    int cb_size = camera4uvc_calc_size(
			    CAMERA4UVC_MAX_FRAME_SIZE/4, NULL,
			    &cb_descrips, NULL);
	    int buffer_size = (y_size + cb_size*2 ) * FRAMES_IN_BUFFER;
	    int descriptor_size = (y_descrips + cb_descrips*2) * FRAMES_IN_BUFFER;
	    
	    if ( camera_context->buf_size < buffer_size) {
		    camera_context->buf_size = buffer_size;
		    camera_context->dma_descriptors_size = descriptor_size+10;
	    }

	    //set the width and height to default
	    camera_context->capture_width = CAMERA4UVC_DEF_WIDTH;
	    camera_context->capture_height= CAMERA4UVC_DEF_HEIGHT;
    }
#endif
...
int camera_ring_buf_init(p_camera_context_t camera_context)
{
    ...
    if(camera_context->still_image_mode)
    {
        ...
        {
            width = camera_context->camera_width;
            height = camera_context->camera_height;
        }
        else
        {
...
#ifdef CONFIG_USBD_UVC
    if ( camera_state == CAMERA_STATE_UVC )
	    ret = camera4uvc_ring_buf_init(camera_context);
    else
	    ret = camera_ring_buf_init(camera_context);
#else
    ret = camera_ring_buf_init(camera_context);
#endif
...
#ifdef LOG_FPS
	for(i=0; i<15; i++)
		dbg_print("interval %d, \tcount %d", i, frame_interval[i]);
#endif

	//disable GPIO_CIF_FV when capture stop.
...
void start_capture_data( p_camera_context_t camera_context)
{
    ...
    int jpeg_mode = camera_context->still_image_mode && (camera_context->still_input_format==CAMERA_IMAGE_FORMAT_JPEG
                                                        || camera_context->still_input_format==CAMERA_IMAGE_FORMAT_JPEG_SENSOR);
...
***********************************************************************************/
int pxa_camera_open_device( void )
{
    dbg_print("start...");
...
{
    /*
     *	Check if camera driver used by UVC
     */
    ...
    if ( camera_state == CAMERA_STATE_UVC )
    {
	    ...
	    /* As camera application required to notify user when
	     * camera device is acquired by UVC, and the safe way 
	     * for application get the camera driver state is open
	     * camera device, and determin camera driver state by
	     * return code of open and errno, so it return special
	     * error code 125 to application to indicate that camera
	     * driver is acquired by UVC
	     */
	    return -125;
    }
    camera_state = CAMERA_STATE_APP;
    spin_unlock ( &camera_state_lock );

    return pxa_camera_open_device();
}

void pxa_camera_close_device( void )
{
    camera_deinit(g_camera_context);
    pxa_camera_mem_deinit();
    camera_free_dma_irq();
...
int pxa_camera_mem_deinit(void)
{
    if(g_camera_context)
    {
       ...
       if (g_camera_context->capture_time != NULL)
       {
           kfree( g_camera_context->capture_time );
	   g_camera_context->capture_time = NULL;
       }
...
int pxa_camera_mem_init(void)
{
   g_camera_context = kmalloc(sizeof(struct camera_context_s), GFP_KERNEL);
    ...
    /*
     * alloc memory for picture capture time
     */
    g_camera_context->capture_time = 
	    (int*)kmalloc(sizeof(int)*VIDEO_MAX_FRAME, GFP_KERNEL);
    dbg_print("alloc capture time array %d\n", VIDEO_MAX_FRAME);
    if ( g_camera_context->capture_time == NULL)
    {
        err_print("PXA_CAMERA:Cann't allocate buffer for capture time\n");
        kfree( g_camera_context );
	g_camera_context = NULL;
	return -ENOMEM;
    }
...
static int __init pxa_camera_init(void)
{
    ...
    camera_state = CAMERA_STATE_IDLE;
    spin_lock_init( &camera_state_lock );
...
void pxa_ci_dma_irq_y(int channel, void *data, struct pt_regs *regs)
{
    ...
    /* when capture video in large resolution like 640*480,
     * sometime FIFO will overrun, handle it by skip one
     * frame of video */ 
    cisr = CISR;
    if(cisr & (CI_CISR_IFO_0|CI_CISR_IFO_1|CI_CISR_IFO_2))
    {
        if ( (camera_state == CAMERA_STATE_UVC) && (captureed_count >1) ) {
		CISR |= (CI_CISR_IFO_0|CI_CISR_IFO_1|CI_CISR_IFO_2);  
		camera_skip_frame(cam_ctx, 0);
	}
    } 
...
#endif
    cam_ctx->capture_time[cam_ctx->block_header] = captureed_count;
    captureed_count++;
