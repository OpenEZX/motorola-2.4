 ...
 *
 ...
 ------------  -------------------------
  12/19/2003    Created
  01/01/2004    Modified
  02/05/2004    Set frame rate in video mode
  02/26/2004    New chip id support
  ...
  03/08/2004    Photo effects setting
                Fix segmentation fault in rmmod
                Adjust default image buffer size  
  04/26/2004    Power Management added
                Photo effects setting bug fix
  05/28/2004    add two new interface.
                1 get ready frames
                2 set frame buffer count
  06/10/2004    add mt9v111 support 
  04/21/2006    fix comments
  08/20/2006    change capture frame number
...
int camera_capture_still_image(p_camera_context_t camera_context, unsigned int block_id)
{
    ...
    do_gettimeofday(&tv0);
#endif
	return  start_capture(camera_context, block_id, 2);
