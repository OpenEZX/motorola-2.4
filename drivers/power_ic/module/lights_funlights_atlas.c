 ...
 * Motorola 2006-Jan-12 - Add in Main Display, EL Keypad, and Camera 
 *   functionality for Ascension
 */
...
static bool lights_fl_region_bt_led(LIGHTS_FL_LED_CTL_T *pCtlData,LIGHTS_FL_COLOR_T nColor);
...
LIGHTS_FL_REGION_CFG_T LIGHTS_FL_region_ctl_tb[LIGHTS_FL_MAX_REGIONS] =
{
    ...
    { lights_fl_region_bt_led,      {0,1, 0}}  /*Region 9*/
};
...
)
{
    ...
    if(nColor == LIGHTS_FL_CAMERA_FLASH )
    {
        ...
        gpio_set_data(GPIO_SP_A_PORT, LIGHTS_FL_CAMERA_EN, 1);
    ...
    else 
    {
        ...
        gpio_set_data(GPIO_SP_A_PORT, LIGHTS_FL_CAMERA_EN, 0);
        gpio_set_data(GPIO_SP_A_PORT, LIGHTS_FL_TORCH_FLASH, 0);
    }
#endif
    return (error != 0);                 
...
/*!
  * @brief Update the bluetooth led
  *       Returns the number of the highest priority app which has control the region requested.
  *
  * @param  pCtlData     Pointer to the control data for the region. 
  * @param  nColor       The color to set the region to.
  ...
  * @return     The application priority
  */
static bool lights_fl_region_bt_led
(
    LIGHTS_FL_LED_CTL_T *pCtlData,
    LIGHTS_FL_COLOR_T nColor
)
{
    return (lights_fl_region_tri_color(pCtlData, nColor));
}
