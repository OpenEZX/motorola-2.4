...
{
/* These GPIOs need to be set for SCM-A11 since the main display lighting is supported off of GPIOs,
   but for Ascension, even though it is SCM-A11, the main display is going to be controlled by ATLAS. */
    ...
    if(boardrev() < BOARDREV_P1A) 
    {
        ...
        gpio_config(GPIO_AP_B_PORT, 17, true, GPIO_INT_NONE);
        gpio_set_data(GPIO_AP_B_PORT, 17, 1);    
    }
#endif /* CONFIG_MOT_FEAT_BRDREV */
    iomux_config_mux(AP_IPU_D3_PS, OUTPUTCONFIG_DEFAULT, INPUTCONFIG_NONE);
    gpio_config(GPIO_AP_A_PORT, 30, true, GPIO_INT_NONE);
    gpio_set_data(GPIO_AP_A_PORT, 30, 1);  
#elif defined(CONFIG_MACH_ASCENSION)
    /* EL Keypad GPIOs */
    iomux_config_mux(SP_SPI1_CLK, OUTPUTCONFIG_DEFAULT, INPUTCONFIG_NONE);
    gpio_set_data(GPIO_AP_A_PORT, 27, 0);
    gpio_config(GPIO_SP_A_PORT, 27, true, GPIO_INT_NONE);

    iomux_config_mux(SP_SPI1_MISO, OUTPUTCONFIG_DEFAULT, INPUTCONFIG_NONE);
    gpio_set_data(GPIO_AP_A_PORT, 29, 0);
    gpio_config(GPIO_SP_A_PORT, 29, true, GPIO_INT_NONE);
    
    /* Camera Flash GPIOs */
    iomux_config_mux(SP_SPI1_SS0, OUTPUTCONFIG_DEFAULT, INPUTCONFIG_NONE);
    gpio_set_data(GPIO_AP_A_PORT, 30, 0);
    gpio_config(GPIO_SP_A_PORT, 30, true, GPIO_INT_NONE);

    iomux_config_mux(SP_UH2_SUSPEND, OUTPUTCONFIG_DEFAULT, INPUTCONFIG_NONE);
    gpio_set_data(GPIO_AP_A_PORT, 10, 0);
    gpio_config(GPIO_SP_A_PORT, 10, true, GPIO_INT_NONE);
