...
int power_ic_audio_codec_en(bool val)
{
    ...
    /* CODEC Startup...Must be done in this order to minimize undesirable speaker noise and ensure
...
{ 
    ...
    /* ST DAC Startup...Must be done in this order to minimize undesirable speaker noise and ensure
       proper start-up of digital audio converters. */
    error = power_ic_set_reg_mask(POWER_IC_REG_ATLAS_AUDIO_RX_0, VAUDIOON_MASK, VAUDIOON_MASK);
    ...
    error |= power_ic_set_reg_mask(POWER_IC_REG_ATLAS_AUDIO_RX_0, 
                                   (ADDSTDC_MASK | ADDCDC_MASK | AHSSEL_MASK | ASPSEL_MASK | ALSPSEL_MASK | ARXOUTSEL_MASK),
...
int power_ic_audio_set_codec_sm(bool val)
{
    ...
    return (power_ic_set_reg_mask(AUDIO_CODEC, 
...
int power_ic_audio_set_st_dac_sr(POWER_IC_ST_DAC_SR_T val)
{
    ...
    return(power_ic_set_reg_value(ST_DAC,
                                  ...
                                  ST_DAC_SR_NUM_BITS));
#else
    int error = power_ic_set_reg_value(ST_DAC,
                                       ST_DAC_SR_INDEX,
                                       val,
                                       ST_DAC_SR_NUM_BITS); 
    mdelay (50);
