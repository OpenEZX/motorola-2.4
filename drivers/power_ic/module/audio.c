 ...
 * Motorola 2004-Feb-17 - Initial SIERRA_AP_1.0.0000
 */
...
#include <asm-arm/arch-scma11/platform.h>
#endif
...
#define ST_DAC_FS0_MASK              1<<5  /* Bus protocol selection */
#define ST_DAC_FS1_MASK              1<<6
...
#define ST_DAC_EN_MASK               1<<11 /* Stdac enable bit */
...
static const AUDIO_DATA_T audio_in_path[POWER_IC_NUM_OF_IN_PATH]={
    ...
    {POWER_IC_EXTERNAL_MIC, EXT_MIC_MUX_MASK, EXT_MIC_MUX_MASK}                                /*External MIC*/
#endif /* ATLAS */
};
...
EXPORT_SYMBOL(power_ic_audio_vaudio_on);
#endif
...
==================================================================================================*/
/*!
 * @brief Turn on VAUDIO
 * This function will turn on VAUDIO and BIASEN for ATLAS
 *
 * @return 0 if successful
 *
 * @note  This function is only used for ATLAS, not PCAP
 */
int power_ic_audio_vaudio_on(void)
{
    int error = 0;
#ifdef CONFIG_MOT_POWER_IC_ATLAS
    /* CODEC/STDAC Startup...Must be done in this order to minimize undesirable speaker noise and ensure
       proper start-up of digital audio converters. */
    error = power_ic_set_reg_mask(POWER_IC_REG_ATLAS_AUDIO_RX_0, VAUDIOON_MASK, VAUDIOON_MASK);   
    
    error |= power_ic_set_reg_mask(POWER_IC_REG_ATLAS_AUDIO_RX_0, (BIASEN_MASK | BIASSPEED_MASK), BIASEN_MASK);
    
    /* Wait for AUDIO BIAS to reach full value */
    mdelay(250);
#endif
    return(error);
}
...
int power_ic_audio_codec_en(bool val)
{
    ...
    /* Enable VAUDIO and BIASEN */
    error = power_ic_audio_vaudio_on();
    ...
    /* CDCRESET must be set to 1 */
    error |= power_ic_audio_codec_reset();
 
    /* Wait for data converter to settle */
    mdelay(15);
    ...
    /* For RX0, turn on bits 12,13,14,21 */
    error |= power_ic_set_reg_mask(POWER_IC_REG_ATLAS_AUDIO_RX_0, 
                                   (HSPGDIS_MASK | HSDETEN_MASK | HSDETAUTOB_MASK | ADDCDC_MASK), 
                                   (HSPGDIS_MASK | HSDETEN_MASK | HSDETAUTOB_MASK | ADDCDC_MASK));
...
{ 
    ...
    /* Enable ST DAC */
    stdac_mask = ST_DAC_EN_MASK | ST_DAC_RESET_MASK | ST_DAC_SSISEL_MASK | ST_DAC_CLKSEL_MASK | ST_DAC_CLK0_MASK |
                 ST_DAC_CLK1_MASK | ST_DAC_CLK2_MASK | ST_DAC_CLKEN_MASK | ST_DAC_SMB_MASK | ST_DAC_SR_MASK |
                 ST_DAC_FS0_MASK | ST_DAC_FS1_MASK;
    
    stdac_setup = ST_DAC_EN_MASK | ST_DAC_SSISEL_MASK | ST_DAC_CLKEN_MASK | ST_DAC_RESET_MASK | ST_DAC_FS0_MASK;
    
#ifdef CONFIG_ARCH_SCMA11    
    /* For SCM-A11 PASS2, the CLK is set to 13MHz whereas for PASS1 the CLK is set to 26MHz */
    if(system_rev == CHIP_REV_1_0)
    {
        stdac_setup |= ST_DAC_CLK2_MASK;
    }
#elif defined(CONFIG_MACH_ARGONPLUS_BUTE) || defined(CONFIG_MACH_ARGONLV_BUTE)
    stdac_setup |= ST_DAC_CLK2_MASK;
#endif 

    error |= power_ic_set_reg_mask(ST_DAC,stdac_mask, stdac_setup);
    ...
    /* For RX0, turn on bits 4, 7, 11, 17, 22 */
    error |= power_ic_set_reg_mask(POWER_IC_REG_ATLAS_AUDIO_RX_0, 
                                   (ADDSTDC_MASK | AHSSEL_MASK | ASPSEL_MASK | ALSPSEL_MASK | ARXOUTSEL_MASK),
                                   (ADDSTDC_MASK | AHSSEL_MASK | ASPSEL_MASK | ALSPSEL_MASK | ARXOUTSEL_MASK));
    
    /* For RX0, turn on bits 12, 13, 14 */
    error |= power_ic_set_reg_mask(POWER_IC_REG_ATLAS_AUDIO_RX_0,
                                   (HSPGDIS_MASK | HSDETEN_MASK | HSDETAUTOB_MASK), 
                                   (HSPGDIS_MASK | HSDETEN_MASK | HSDETAUTOB_MASK));
    tracemsg(_k_d("\nSTDAC is set up"));
...
int power_ic_audio_set_codec_sm(bool val)
{
    ...
    if (val == false)
    {    
        codec_setup = CDC_FS_MASK | CDC_CLKEN_MASK | CDC_RESET_MASK;
    }
    else /* Codec is slave */
    {
        codec_setup = CDC_FS_MASK | CDC_SMB_MASK | CDC_RESET_MASK;
    }
    ...
    if(system_rev == CHIP_REV_1_0)
    {
        codec_setup |= CDC_CLK2_MASK;
    }
#elif defined(CONFIG_MACH_ARGONPLUS_BUTE) || defined(CONFIG_MACH_ARGONLV_BUTE)
    codec_setup |= CDC_CLK2_MASK;
#endif

    return (error | (power_ic_set_reg_mask(AUDIO_CODEC,
                       (CDC_CLKSEL_MASK | CDC_FS_MASK | CDC_CLK0_MASK | CDC_CLK1_MASK | CDC_CLK2_MASK | 
                          CDC_SMB_MASK | CDC_CLKEN_MASK | CDC_RESET_MASK), codec_setup)));
#endif
}
...
int power_ic_audio_set_st_dac_sm(bool val)
{
    ...
    if (val == false)
    {
        stdac_setup = ST_DAC_SSISEL_MASK | ST_DAC_CLKEN_MASK | ST_DAC_RESET_MASK; 
    }
    else /* STDAC is slave */
    {
        stdac_setup = ST_DAC_SSISEL_MASK | ST_DAC_CLK0_MASK | ST_DAC_CLK1_MASK | 
    ...
    return (power_ic_set_reg_mask(ST_DAC, (ST_DAC_SSISEL_MASK |ST_DAC_CLKSEL_MASK | ST_DAC_CLK0_MASK | 
                                  ST_DAC_CLK1_MASK | ST_DAC_CLK2_MASK | ST_DAC_CLKEN_MASK | 
                                  ST_DAC_RESET_MASK | ST_DAC_SMB_MASK | ST_DAC_SR_MASK), stdac_setup));
#endif
}
...
int power_ic_audio_set_st_dac_sr(POWER_IC_ST_DAC_SR_T val)
{
    ...
    return(power_ic_set_reg_value(ST_DAC,
                                  ...
                                  ST_DAC_SR_NUM_BITS));
#else
    error = power_ic_set_reg_value(ST_DAC,
                                   ST_DAC_SR_INDEX,
                                   val,
                                   ST_DAC_SR_NUM_BITS); 
    mdelay (50);
    error |= power_ic_audio_st_dac_reset ();
...
int audio_ioctl(unsigned int cmd, unsigned long arg)
{
        ...
        switch (cmd) 
        {
            ...
            case POWER_IC_IOCTL_AUDIO_VAUDIO_ON:
                tracemsg(_k_d("Setting VAUDIO\n"));
                ret_val = power_ic_audio_vaudio_on();
                break;
