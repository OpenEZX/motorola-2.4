 ...
 * 02111-1307, USA
 *
 * Motorola 2006-Sep-06 - Disable atod converter when no hardware trig signal
 * Motorola 2006-Aug-19 - Fix for PCAP2 AtoD issue. 
 * Motorola 2006-May-15 - Moved some variable declarations to start of functions.
 * Motorola 2006-Feb-07 - Updated convert_to_milliamps for ATLAS.
...
/* When verifing that bad results have been returned by the PCAP2 measurement this limit 
 * will be used to compare the returned voltage and current results. */
#define REJECT_READINGS_CURRENT       6

/* When verifing that bad results have been returned by the PCAP2 measurement this limit
 * will be used to compare the returned voltage to other voltage results. */
#define REJECT_READINGS_VOLTAGE       50
...
/* These values are used by AUL for battery current alginment and conversion */
...
#define BATTI_READING_RESOLUTION     (unsigned int) (5.6 * BATTI_READING_DIVIDER)   /* For AUL, a/d on batti is 5.6mA per count */
#define BATTI_READING_ROUNDUP_OFFSET (BATTI_READING_DIVIDER/2) /* For calculation of roundup */
...
/*! Holds the results from the last battery current conversion in case it is needed to replace
 *  a future set
 */
#ifdef CONFIG_MOT_POWER_IC_PCAP2
static int previous_voltage_result;
static int previous_current_result;
#endif
...
#ifdef CONFIG_MOT_POWER_IC_PCAP2
static void filter_pcap2_atod(int * samples);
#endif  /* PCAP2 */
/******************************************************************************
* Local functions
******************************************************************************/
...
#ifdef CONFIG_MOT_POWER_IC_PCAP2
/*!
 * @brief Filters invalid AtoD reading for PCAP2
 *
 * This function filters off incorrect samples taken by PCAP2 and replaces them
 * with either copies of the first pair or previous averages.   
 *
...
static void filter_pcap2_atod(int * samples)
{
    ...
    if((abs(samples[0] - samples[1]) < 2) && 
       (abs(samples[0] - previous_voltage_result) > REJECT_READINGS_VOLTAGE))   
    {
        for(i=0; i < (SAMPLES_PER_BANK - 1); i+=2)
        {
	    samples[i]   = previous_voltage_result;
            samples[i+1] = previous_current_result;
	 }   
     }
     else         
     {
        for(i=2; i < (SAMPLES_PER_BANK - 1); i+=2)
        {
            if((abs(samples[0] - samples[i]) > REJECT_READINGS_VOLTAGE)
	        && (abs(samples[i] - samples[i+1]) < REJECT_READINGS_CURRENT))
	    {
                tracemsg(_k_d("sample %d of 0x%X has been changed"), i, samples[i]);
	        samples[i] = samples[0];
                tracemsg(_k_d("sample %d of 0x%X has been changed"),(i+1),samples[i+1]);
	        samples[i+1] = samples[1];
	     }   
        }
     }
}
...
#elif defined(CONFIG_MOT_POWER_IC_ATLAS)
    /* Set negative current readings to negative integers. */
    if (sample >= BATTI_ZERO)
    {
        /* Set upper bits to 1's */
        curr = curr | NEGATIVE_BATTI_MASK;
    }
    
    curr = ((curr * BATTI_READING_RESOLUTION + BATTI_READING_ROUNDUP_OFFSET)
            / BATTI_READING_DIVIDER);
#endif
...
static int exclusive_conversion(POWER_IC_ATOD_REQUEST_T * request, int * samples)
{
    ...
    if (request->type == POWER_IC_ATOD_TYPE_BATT_AND_CURR)
    {
        error_in_cancel = power_ic_atod_nonblock_cancel_conversion();
        if (error_in_cancel != 0)
        {
            tracemsg(_k_d("AtoD - error %d canceling postponed AtoD."), error_in_cancel);
        }
    }
...
#ifdef CONFIG_MOT_POWER_IC_PCAP2
        filter_pcap2_atod(postponable_results);  
#endif  /* PCAP2 */
...
#ifdef CONFIG_MOT_POWER_IC_PCAP2
            previous_voltage_result = average / (SAMPLES_PER_BANK/2);
#endif  /* PCAP2 */
...
#ifdef CONFIG_MOT_POWER_IC_PCAP2
            previous_current_result = average / (SAMPLES_PER_BANK/2);
#endif  /* PCAP2 */
