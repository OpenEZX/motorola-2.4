 ...
 * Motorola 2005-Dec-12 - Add ATLAS support.
...
{
    /* keep track of the current setting of reverse mode in case the device
       wasn't actually removed */
    emu_reverse_mode = EMU_GET_REVERSE_MODE();
...
EMU_STATE_T disconnecting_handler(int *polling_interval)
{
    ...
    *polling_interval = EMU_POLL_CONTINUE;
    ...
    if (curr_device_type == emu_current_device_type)
    {
        ...
        {
            /* Set reverse mode if it was set before the glitch */
            if (emu_reverse_mode != 0)
            {
                /* Enable reverse current */
...
void disconnecting_exit(EMU_STATE_T next_state, int *polling_interval)
{
        ...
        (next_state != EMU_STATE_CONNECTED__HEADSET))
    {
        ...
        /* Disable basic charging, THIS IS TEMPORARY FOR BUTE!!! */
