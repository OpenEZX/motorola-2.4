 ...
 * Motorola 2006-Mar-22 - Upmerge Macau baseline to R532_G_11.00.44P.
 * Motorola 2006-Apr-11 - Merge OS component to macau release. 
 * Motorola 2006-Apr-12 - Added 3.5mm headset support.
 * Motorola 2006-Apr-25 - Work arround of headset fully insertion.
 * Motorola 2006-May-30 - Delay the phone to sleep state. 
 * Motorola 2006-Jul-03 - Merge OSS cleanup to macau in kernel. 
 * Motorola 2006-Aug-10 - Resolve all the OSS related issues.
 * Motorola 2006-Sep-27 - Fixed the 3.5MM headset debounce issue 
 * Motorola 2006-Oct-25 - Update comments
 * Motorola 2006-Nov-01 - Add AD7 usage for 3.5mm headset
 */
...
static int debounce_interrupt_handler (POWER_IC_EVENT_T event)
{
    ...
    if (event == POWER_IC_EVENT_HEADSETI)
	    pm_delay_sleep(2*HZ);
#endif
    return 1;
}
...
static void headset_3mm5_debounced(DEBOUNCE_EVENT_T *event, int status)
{
	...
	if (status != debounce_events[DEBOUNCE_EVENT_HEADSET].polarity ) {
		...
		power_ic_set_reg_bit(POWER_IC_REG_PCAP_RX_AUD_AMPS, A1ID_RX_INDEX, 0);
	...
	/* Else, stereo headset is attached */
...
#ifdef CONFIG_MOT_POWER_IC_BARREL_HEADSET_3MM5_USES_AD7
		if (power_ic_atod_single_channel(POWER_IC_ATOD_CHANNEL_AD7, &ad6_val)) {
#else
		...
		if (ad6_val > HEADSET_DET_HIGH || ad6_val < HEADSET_DET_LOW) {
			...
			power_ic_set_reg_bit(POWER_IC_REG_PCAP_RX_AUD_AMPS, A1ID_RX_INDEX, 1);
...
#else
        power_ic_set_reg_bit(POWER_IC_REG_PCAP_RX_AUD_AMPS,A1ID_RX_INDEX,0);  /* resolve A1 interrupt issue */ 
...
#else			 
        power_ic_set_reg_bit(POWER_IC_REG_PCAP_RX_AUD_AMPS,A1ID_RX_INDEX,1);  /* resolve A1 interrupt issue */                
...
/*!
 * @brief timer timeout function to fix the hardware bug of 2.5mm and 3.5mm headset jack
 *
 * Workaround:
 * ===========
 * In the current hardware design of 4-pole headset, an unexpected send/end
 * key pressed interrupt will be raised before the fully removal of Moto 4-pole headset.
 * And the unexpected send/end key pressed interrupt occurs before the removal interrupt of
 * headset.
 * Besides, while consecutively removing and replugging the headset, an unexpected send/end
 * key released interrupt will be raised.
 * So, in order to workaround the hardware bug, a timer solution is adopted to eliminate the
 * unexpected pressed and released event. That is, when a pressed event happens, it is not passed
 * upper layer immediately, a one second timer is started instead. The pressed event is discarded
 * if the headset removed before the timer times out; The pressed event is passed to upper layer
 * as soon as a released event happens before timer's timing out.
 *
 * @param unused not used
 */
static atomic_t hs_timer_started = ATOMIC_INIT(0);
static struct timer_list hs_fix;

static void hs_fix_timeout(unsigned long unused)
{
        if (atomic_read(&hs_timer_started) == 0)
                return;
        atomic_set(&hs_timer_started, 0);
        if (debounce_events[DEBOUNCE_EVENT_MB2I].enabled &&
            debounce_events[DEBOUNCE_EVENT_HEADSET].poll_current == 0) {
                tracemsg(_k_d("hs_fix_timeout: headset send/end is pressed"));
                headset_key_handler(KEYDOWN);
        } else {
                tracemsg(_k_d("hs_fix_timeout: Spurious send/end key pressed event\n"));
        }
}
/*!
...
{
        if (debounce_events[DEBOUNCE_EVENT_HEADSET].poll_current > 0) {
                if (!atomic_add_negative(-1, &hs_timer_started)) {
                        del_timer_sync(&hs_fix);
                }
                tracemsg(_k_d("headset_key_debounced: Spurious send/end key: status:%d\n"), status);
                return;
        }
        if (status != debounce_events[DEBOUNCE_EVENT_MB2I].polarity) {
                ...
                if (atomic_read(&hs_timer_started) > 0) {
                        mod_timer(&hs_fix, jiffies + HZ);
                } else {
                        init_timer(&hs_fix);
                        hs_fix.function = hs_fix_timeout;
                        hs_fix.data = 0;
                        hs_fix.expires = jiffies + HZ;  /* one seconds */
                        add_timer(&hs_fix);
                        atomic_set(&hs_timer_started, 1);
                }
        }
}
...
#else     
#ifdef CONFIG_MOT_POWER_IC_BARREL_HEADSET_STEREO_3MM5
    { POWER_IC_EVENT_HEADSETI,	 1,	  75,	20,    0,      headset_3mm5_debounced },
#else
