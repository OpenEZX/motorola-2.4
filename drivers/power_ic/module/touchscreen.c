 ...
 * Motorola 2005-Jul-13 - Added robustness to the touchscreen module
 * Motorola 2005-Nov-23 - touchscreen_init add support of PCAP2_SPI_BUS
 ...
 * calculated and passed to wait_event_interruptible_timeout() will be zero jiffies. This is deliberate.
...
static int touchscreen_thread_loop (void *unused)
{
    ...
    while(!signal_pending(current))
    {
        ...
        if (wait_event_interruptible(touchscreen_wait_queue, (touchscreen_thread_state != POWER_IC_TS_DISABLED)) < 0)
        {
            break;
        }
        ...
        if (wait_event_interruptible(touchscreen_wait_queue, (touchscreen_thread_state != POWER_IC_TS_IDLE)) < 0)
        {
            break;
        }
        ...
        {        
            ...
            do
            {
                ...
                /* Wait for polling period or an abort signal. */
                if (wait_event_interruptible_timeout(touchscreen_wait_queue,
...
int power_ic_touchscreen_set_repeat_interval(int period)
{
     ...
     * remaining jiffies. This is to do with the wait_event_interruptible_timeout() behaviour
     * described in the notes for this file. */
