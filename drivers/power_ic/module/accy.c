...
void moto_accy_notify_insert (MOTO_ACCY_TYPE_T accy)
{
    ...
    else if((accy == MOTO_ACCY_TYPE_HEADSET_MONO)||
            (accy == MOTO_ACCY_TYPE_HEADSET_STEREO))
    {
       /*If any of the barrel headset connected already, we need to reset the path to reenable the 
         audio path which muted by PCAP.*/
       power_ic_audio_write_input_path(power_ic_audio_read_input_path());
       power_ic_audio_write_output_path(power_ic_audio_read_output_path());
    }
