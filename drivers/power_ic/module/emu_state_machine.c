/*
 * drivers/power_ic/module/emu_state_machine.c
 *
 ...
 *
 * Motorola 2006-May-15 - Conditionally compile name tables for tracemsgs.
 * Motorola 2006-Apr-06 - Move GPIO support to gpio.c
...
static int emu_state_machine_thread(void *unused)
{
    ...
    while(!signal_pending(current))
    {
        ...
        if (polling_interval >= EMU_POLL_MIN_DELAY)
        {
            ...
            error = wait_event_interruptible_timeout(emu_thread_wq, emu_event_flag, polling_interval);

            tracemsg(_k_d("EMU: emu_state_machine_thread: done sleeping or abort received.  error: %d"), error);
        ...
        else if (polling_interval == EMU_POLL_WAIT_EVENT)
        {
            ...
            error = wait_event_interruptible(emu_thread_wq, emu_event_flag);

            tracemsg(_k_d("EMU: emu_state_machine_thread: received an interrupt or signal.  error: %d"), error);
