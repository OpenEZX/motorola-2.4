 ...
 * Motorola 2006-Jan-31 - Changed temporary charging support to Bute-only.
 * Motorola 2006-Apr-17 - Work arround of headset fully insertion.
 * Motorola 2006-Apr-28 - ST:BIGX:Phonehang and Black screen appears without any operation.
 * Motorola 2006-May-12 - Fix detection of rapid insertion/removal issue with headsets
 * Motorola 2006-Jul-19 - fix the issue of writing EMU_ID error in factory mode 
...
void device_config_enter(EMU_STATE_T prev_state, int *polling_interval)
{
    ...
    if (emu_prev_state != EMU_STATE_DISCONNECTING)
    {
        ...
        if(emu_current_device == MOTO_ACCY_TYPE_CABLE_FACTORY)
        {
...
#if defined(CONFIG_MACH_ARGONPLUS_BUTE) || defined(CONFIG_MACH_ARGONLV_BUTE)
    /* Setup basic charging, THIS IS TEMPORARY FOR BUTE !!! */
    if ((emu_current_device == MOTO_ACCY_TYPE_CHARGER_MID)      ||
        (emu_current_device == MOTO_ACCY_TYPE_CHARGER_MID_MPX)  ||
        (emu_current_device == MOTO_ACCY_TYPE_CHARGER_FAST)     ||
        (emu_current_device == MOTO_ACCY_TYPE_CHARGER_FAST_MPX) ||
    ...
    {
        /* Set VCHRG to 3.8V (110)*/
        power_ic_charger_set_charge_voltage(6);
        
        /* Set ICHARGE to 1000mA (1010)*/
        power_ic_charger_set_charge_current(10);
    }
    else if(emu_current_device == MOTO_ACCY_TYPE_CABLE_USB)
    {
        /* Set VCHRG to 3.8V (110)*/
        power_ic_charger_set_charge_voltage(6);
        
        /* Set ICHARGE to 1000mA (0001)*/
        power_ic_charger_set_charge_current(1);
    }
#endif
    ...
    /* If we are in the poll SPD state then go directly to the handler
       and start polling for the removal, if we are in the headset state
       go directly to the handler to verify that the haedset is still connected,
       if we are in the Invalid device state then go to directly the handler to
       to verify whether it is connected or not ,otherwise just wait for an interrupt */
    if ((emu_state == EMU_STATE_CONNECTED__POLL_SPD_REMOVAL) || 
	((emu_state == EMU_STATE_CONNECTED__INVALID) && 
         (emu_current_device_type == EMU_DEV_TYPE_PPD)))
    {
        *polling_interval = EMU_POLL_CONTINUE;
    }
    else if (emu_state == EMU_STATE_CONNECTED__HEADSET)
    {
        *polling_interval = EMU_POLL_CONTINUE;

        emu_id_state = EMU_BUS_SIGNAL_STATE_ID_RESISTOR;
    }
    else
    {
...
EMU_STATE_T factory_handler(int *polling_interval)
{
    *polling_interval = EMU_POLL_CONTINUE;
    ...
    if((emu_id_state == EMU_BUS_SIGNAL_STATE_ID_RESISTOR) && 
       (emu_current_device_type == EMU_DEV_TYPE_PPD))
    {
...
EMU_STATE_T headset_handler(int *polling_interval)
{
    ...
    static const int headset_handler[EMU_BUS_SIGNAL_STATE_ID_FACTORY-EMU_BUS_SIGNAL_STATE_UNKNOWN][EMU_BUS_SIGNAL_STATE_ID_FACTORY-EMU_BUS_SIGNAL_STATE_UNKNOWN] =
        {
            ...
            
            {
                ...
                /*EMU_BUS_SIGNAL_STATE_ID_GROUNDED, EMU_BUS_SIGNAL_STATE_ID_FACTORY*/
                EMU_ID_HANDLER_DISCONNECTING|EMU_ID_HANDLER_SEND_END_UP
            },
            {  
                ...
                EMU_ID_HANDLER_NONE,
                /*EMU_BUS_SIGNAL_STATE_ID_RESISTOR, EMU_BUS_SIGNAL_STATE_ID_FACTORY*/
                EMU_ID_HANDLER_DISCONNECTING
            },
            {
                /*EMU_BUS_SIGNAL_STATE_ID_FACTORY, EMU_BUS_SIGNAL_STATE_ID_FLOATING*/
                EMU_ID_HANDLER_DISCONNECTING,
                /*EMU_BUS_SIGNAL_STATE_ID_FACTORY, EMU_BUS_SIGNAL_STATE_ID_GROUNDED*/
                EMU_ID_HANDLER_DISCONNECTING,
                /*EMU_BUS_SIGNAL_STATE_ID_FACTORY, EMU_BUS_SIGNAL_STATE_ID_RESISTOR*/
                EMU_ID_HANDLER_DISCONNECTING,
                /*EMU_BUS_SIGNAL_STATE_ID_FACTORY, EMU_BUS_SIGNAL_STATE_ID_FACTORY*/
                EMU_ID_HANDLER_DISCONNECTING
            }
        };
        ...
        (emu_current_device == MOTO_ACCY_TYPE_HEADSET_EMU_STEREO))
    {
        ...
        {
            tracemsg(_k_d("EMU: basic_device_handler: emu_id_state=%d, new_id_state=%d"), emu_id_state, new_id_state);
