...
EMU_STATE_T ppd_validate_handler(int *polling_interval)
{
    ...
    {
        tracemsg(_k_d("EMU: ppd_validate_handler: D+ is low, next_state = EMU_STATE_CONNECTING__PRE_PPD_IDENTIFY"));
...
{   
    *polling_interval = EMU_POLL_CONTINUE;
    ...
    if (get_bus_state(EMU_BUS_SIGNAL_ID) == EMU_BUS_SIGNAL_STATE_ID_FLOATING)
    {
        return EMU_STATE_CONNECTING__DEBOUNCE_DEV_TYPE;
    }
