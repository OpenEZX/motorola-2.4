 ...
 * Motorola 2006-Mar-30 - MMC low power mode support on SCMA11
 ...
 */
static struct pm_dev *sdhc_pm_dev;
#endif
 ...
 * It must return 0 if sleep is acceptable or -EBUSY if sleep cannot be entered.
 ...
 *
 * @return -EBUSY if the phone cannot go to sleep, 0 if it can.
 */
static int sdhc_pm_callback(struct pm_dev *pm_dev, pm_request_t req, void *data)
{
    unsigned int i;
...
int sdhc_poll(SDHC_MODULE_T module)
{
    ...
    /*
     * If edio_get_data returns 0, card is present.  If 1, card not present.  Because of
     * this, the return of edio_get_data is inverted to match what this function is
     * expected to return.
     */
    return !edio_get_data(ED_INT4);
#else
    /* Always return card not present */
    return 0;
#endif
}
...
#ifdef CONFIG_MACH_SCMA11BB
            sdhc_enable_int(module, SDHC_VAL_END_CMD_RES_EN | SDHC_VAL_DETECT_INT);
#else
            sdhc_enable_int(module, SDHC_VAL_END_CMD_RES_EN);
#endif
        }
...
int __init sdhc_initialize(void)
{
    ...
    sdhc_pm_dev = pm_register(PM_UNKNOWN_DEV, PM_SYS_UNKNOWN, sdhc_pm_callback);
#endif
