 ...
 * Motorola 2006-Jan-10 - Finalize the interface for Intel Bulverde
 * Motorola 2005-Apr-04 - Design of low level SPI interface for Intel Bulverde.
 *
 */
/*!
 * @file pxa_ssp.c
...
int pxa_ssp_initiate_tx(SPI_QUEUE_NODE_T *node_p, bool no_wait)
{
     ...
     */
    error = power_ic_gpio_spi_set_cs(node_p->params.cs, true);
    if (error < 0)
    {
        SSCR0 = 0;
        return (error);
...
static int pxa_ssp_rx_complete(SPI_QUEUE_NODE_T *node_p)
{
    ...
    do
    {
        ...
        {
            (void)power_ic_gpio_spi_set_cs(node_p->params.cs, false);
        }
