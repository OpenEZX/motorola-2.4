 ...
 * Motorola 2006-May-11 - Improve performance during periodic jobs(decrease idle time).
...
#endif /* SCM-A11 & ArgonLV & ArgonPlus */
...
void pcap2_power_ic_bh_handler(void)
{
...
#ifdef CONFIG_APM_ISP
    //Normally AtoD interrupts are collected at different time intervals
    //After AtoD interrupts there is one second delay to go to sleep
    //This function will ignore the delay to improve the performance
    //Here the AtoD interrupts will be recorded and comunicate with PM 
    unsigned int int_check_for_atod = 0;
#endif

    /* Loop while PCAP contGPIO_bit(1)GPIO_bit(1)GPIO_bit(1)inues to assert its interrupt line */
	...
	int_check_for_atod |= enabled_ints0;
#endif
...
#ifdef CONFIG_APM_ISP
    /*Make sure just atod interrupts*/
    if ( int_check_for_atod == (1<<(POWER_IC_EVENT_PCAP_ADCDONEI)) )
    {
	/*Comunicate with PM to ignore the AtoD interrupts*/
	apm_inc_ignore_int( PCAP_ATOD );
    }
#endif
}
...
int power_ic_event_sense_read (POWER_IC_EVENT_T event)
{
    ...
    if (event >= POWER_IC_EVENT_EOC_FIRST)
    {
         ...
         * enum lines up with the interrupt bit, some translation is needed. */
        if(event == POWER_IC_EVENT_EOC_VBUSOV)
        {
            event = POWER_IC_EVENT_EOC_FIRST + 9;
        }
