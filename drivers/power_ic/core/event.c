 ...
 *
 * Motorola 2006-May-27 - Fix GPL issues
...
******************************************************************************/
#ifdef CONFIG_MOT_POWER_IC_PCAP2
#define POWER_IC_REG_ISR0     POWER_IC_REG_PCAP_ISR
#define POWER_IC_EVENT_FIRST  POWER_IC_EVENT_PCAP_FIRST
#elif defined(CONFIG_MOT_POWER_IC_ATLAS)
#define POWER_IC_REG_ISR0     POWER_IC_REG_ATLAS_INT_STAT_0
#define POWER_IC_EVENT_FIRST  POWER_IC_EVENT_ATLAS_FIRST_REG
#endif
/******************************************************************************
* Local variables
******************************************************************************/
...
static void power_ic_bh_handler(unsigned long unused)
{
    ...
    while (power_ic_gpio_event_read_priint(PM_INT))
    {
        ...
        while (enabled_ints0 != 0)
        {
            ...
            execute_handlers(int_vec + POWER_IC_EVENT_FIRST);
        ...
        power_ic_write_reg_value (POWER_IC_REG_ISR0, isr0);
#if defined(CONFIG_MOT_POWER_IC_ATLAS)        
    ...
    while (power_ic_gpio_event_read_priint(10))
    {
        /* Read the interrupt status and mask registers */
...
{
    /* Clear the interrupt */
    power_ic_gpio_event_clear_priint();
