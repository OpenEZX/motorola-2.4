 ...
 *
 * yyyy-mmm-dd - Description of changes
 * -----------   ---------------------------------------------
 * 2006-Jan-11   Added conditionals for SCMA11.
 *
 */
/*!
 * @file usr_blk_dev.c
 ...
 */
void usr_blk_dev_attach_irq(int irq, void *device_msk, struct pt_regs *regs)
{
    tracemsg("\n");
    ...
    while ((req_p = elv_next_request(info_p->req_queue)) != NULL)
    {
        ...
        do
        {
            ...
            while ((bio_p != NULL) &&
                   ...
                   (bio_data(bio_p) == info_p->cmd[USR_BLK_DEV_CMD_SRC_REQ].data.cmd_data_to_usr.rw.buf+
                                       num_sectors*info_p->params.read_block_len))
            {
            ...
            tracemsg("sector: %010d nr_sectors: %010d buffer: %p\n",
                     info_p->cmd[USR_BLK_DEV_CMD_SRC_REQ].data.cmd_data_to_usr.rw.sector,
                     info_p->cmd[USR_BLK_DEV_CMD_SRC_REQ].data.cmd_data_to_usr.rw.num_sectors,
                     info_p->cmd[USR_BLK_DEV_CMD_SRC_REQ].data.cmd_data_to_usr.rw.buf);
            status = 0;
