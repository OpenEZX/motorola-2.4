/*
 *
 * drivers/power_ic/core/usr_blk_dev.c
 *
 ...
 * Motorola 2006-Jun-12   EZX: Do not turn off power to transflash when sleeping
 * Motorola 2006-May-12   Add Zeus support for SD
 * Motorola 2006-May-12   Linux 2.6: Do not call register_disk in revalidate function
...
/* For SCM-A11, this file is compiled by power ic, but in ezx, the file is compiled
 * by a different driver.  Therefore the paths to gpio.h have to change */
#ifdef CONFIG_CPU_BULVERDE
#include "../power_ic/core/gpio.h"
...
#if defined(CONFIG_PM) && (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0))
/*!
 * @brief Pointer to the power management device.
 *
 * Initialized when the power management callback is registered in init.
 */
static struct pm_dev *usr_blk_dev_pm_dev;
#endif
 ...
 *
 * @return  Always return IRQ_RETVAL(1) - Linux 2.6 only.
 */
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0))
void
#else
irqreturn_t
#endif
usr_blk_dev_attach_irq(int irq, void *device_msk, struct pt_regs *regs)
{
    tracemsg("\n");
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0))
    MOD_DEC_USE_COUNT;
    usr_blk_dev_info[USR_BLK_DEV_DEVICE_NR(inode_p->i_rdev)].device_opens--;
#else
 ...
 *
 * @param i_rdev A pointer to the kernel device information (Linux 2.4)
 * @param gd A pointer to the gendisk (Linux 2.6)
 *
 * @return Returns 0 when the device has not changed, or 1 when it has.
 */
static int usr_blk_dev_check_media_change(
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0))
  kdev_t i_rdev
#else
  struct gendisk *gd
 ...
 *
 * @param i_rdev Kernel device information (Linux 2.4)
 * @param gd A pointer to the gendisk (Linux 2.6)
 *
 * @return Always returns 0.
 */
static int usr_blk_dev_revalidate(
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0))
  kdev_t i_rdev
#else
  struct gendisk *gd
...
{
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0))
    unsigned int i = dev_num<<USR_BLK_DEV_SHIFT;
#endif
    ...
    /* Reset the sizes and partition data. */
    while (i < (dev_num+1)<<USR_BLK_DEV_SHIFT)
    {
        memset(&usr_blk_dev_partitions[i], 0, sizeof(usr_blk_dev_partitions[0]));
        usr_blk_dev_sizes[i] = 0;
    ...
    while (!list_empty(&info_p->req_queue.queue_head))
    {
        ...
        do
        {
            ...
            while ((bh_p != NULL) &&
                   ...
                   (bh_p->b_data == req_p->buffer+num_sectors*info_p->params.read_block_len))
            {
                ...
                (info_p->media_state > USR_BLK_DEV_MEDIA_STATE_ERROR))
            {
                do
                {
                    ...
                    tracemsg("minor: %02d start_sector: %010ld sector: %010d nr_sectors: %010d buffer: %p usr_blk_dev_blksize: %d try: %d\n",
                             minor,
                    ...
                    if (cmd_p->state == USR_BLK_DEV_CMD_STATE_NEW)
                    {
                        ...
                        switch (cmd_p->data.id)
                        {
                            ...
                            case USR_BLK_DEV_CMD_ID_INIT:
                                /*
                                 * If already initialized, do not perform new card init.  We call init
 ...
 * the phone will only be allowed to go to sleep between trans flash commands.
 *
 * Please note that if you make changes to this function, you will have to make
 * changes to the appropriate power management functions for Linux 2.6 located
 * below.
 *
 ...
 */
static int usr_blk_dev_pm_callback
(
 ...
 *
 * Please note that if you make changes to this function, you will have to make
 * changes to the usr_blk_dev_pm_callback power management function for Linux
 * 2.4 located above.
 *
...
#ifdef CONFIG_PM
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0))
    usr_blk_dev_pm_dev = pm_register(PM_UNKNOWN_DEV, PM_SYS_UNKNOWN, usr_blk_dev_pm_callback);
#else
...
#ifdef CONFIG_PM
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0))
    pm_unregister(usr_blk_dev_pm_dev);
#else
