...
extern int sdhc_poll(int module);
#endif
...
static void usr_blk_dev_set_irq_state(unsigned int dev_num, bool enable)
{
    ...
    if (enable)
    {
        enable_irq(INT_EXT_INT4);
    }
    else
    {
        disable_irq(INT_EXT_INT4);
    }
#endif
 ...
 */
static void usr_blk_dev_thread_send_init_to_usr(USR_BLK_DEV_INFO_T *info_p, unsigned int dev_num)
{
    /*
...
static void usr_blk_dev_thread_init_cmd(USR_BLK_DEV_INFO_T *info_p, unsigned int dev_num)
{
    ...
    info_p->cmd[info_p->cmd_src].data.cmd_data_to_usr.init.new_card = true;
    usr_blk_dev_thread_send_init_to_usr(info_p, dev_num);
        ...
        do
        {
                ...
                (info_p->media_state > USR_BLK_DEV_MEDIA_STATE_ERROR))
            {
                do
                {
                    ...
                    {
                        info_p->cmd[USR_BLK_DEV_CMD_SRC_REQ].data.cmd_data_to_usr.init.new_card = false;
                        usr_blk_dev_thread_send_init_to_usr(info_p, dev_num);
                    }
...
)
{
    ...
    else if (req == PM_RESUME)
    {
        for (i = 0; i < USR_BLK_DEV_NUM_DEVICES; i++)
        {
            if (usr_blk_dev_info[i].debounce_state == USR_BLK_DEV_DEBOUNCE_STATE_INSERTED)
            {
                usr_blk_dev_send_cmd_to_thread(USR_BLK_DEV_CMD_ID_INIT, i, USR_BLK_DEV_CMD_SRC_IRQ);
            }
        }
    }
...
int __init usr_blk_dev_init(void)
{
    ...
    if (request_irq(INT_EXT_INT4, usr_blk_dev_attach_irq,
    ...
    for (i = 0; i < USR_BLK_DEV_NUM_DEVICES; i++)
    {
        ...
        devfs_mk_bdev(MKDEV(USR_BLK_DEV_MAJOR_NUM,1), S_IFBLK|S_IRUSR|S_IWUSR, "mmc%s1", "a"+i);
    }
#endif
#ifdef CONFIG_PM
