 ...
 * Motorola 2006-Sep-04 - Add spi functions for interrupt mode and comments.
...
EXPORT_SYMBOL(spi_int_reg_read);
EXPORT_SYMBOL(spi_int_reg_write);
EXPORT_SYMBOL(spi_int_write_reg_value);
EXPORT_SYMBOL(spi_int_set_reg_value);
EXPORT_SYMBOL(spi_int_get_reg_value);
EXPORT_SYMBOL(spi_int_set_reg_bit);
EXPORT_SYMBOL(spi_int_set_reg_mask);
#endif
 ...
 *
 * @attention This function differs from spi_int_reg_read() in that it can't be
 ...
 *
 * @attention This function differs from spi_int_reg_write() in that it can't 
 * be called in a interrupt context.
 *
 * @param        reg        register number
 ...
 *
 * @attention This function differs from spi_int_write_reg_value() in that it 
 * can't be called in a interrupt context.
 *
 * @param        reg        register number
 ...
 *
 * @attention This function differs from spi_int_set_reg_bit() in that it can't
 * be called in a interrupt context.
 *
 * @param        reg        register number
 ...
 *
 * @attention This function differs from spi_int_set_reg_value() in that it 
 * can't be called in a interrupt context.
 *
 * @param        reg        register number
 ...
 *
 * @attention This function differs from spi_int_get_reg_value() in that it 
 * can't be called in a interrupt context.
 *
 * @param        reg        register number
 * @param        index      starting bit index (0 = least-significant bit)
 ...
 * call.
 *
 * @attention This function differs from spi_int_set_reg_mask() in that it can't be called
 ...
 *
 * @attention This function differs from power_ic_read_reg() in that it can be 
 * called in a interrupt context.
 *
 ...
 * spi_int_transceive function                                                              
 *                                                                                          
 * @attention This function differs from power_ic_write_reg() in that it can be             
 * called in a interrupt context.                                                           
 *                                                                                          
 * @param     reg        The PCAP register index.                                           
 ...
 * is written to the register.
 *
 * This function differs from spi_int_reg_write() in that it takes the new
 * register value directly instead of expecting a pointer to the new value.
 * The function is simply implemented as a call to spi_int_reg_write().
 *
 * @attention This function differs from power_ic_write_reg_value() in that it 
...
{
    return spi_int_reg_write (reg, &reg_value);
}
 ...
 *                                                                                          
 * @attention This function differs from power_ic_set_reg_value() in that it                
 * can be called in a interrupt context.                                                    
 *                                                                                          
 * @param        reg        register number                                                 
 * @param        index      starting bit index (0 = least-significant bit)                  
 ...
 * permissition. (modify_spi_bus_status)                                                    
 *                                                                                          
 * @attention This function differs from power_ic_set_reg_bit() in that it can              
 * be called in a interrupt context.                                                        
 *                                                                                          
 * @param        reg        register number                                                 
 * @param        index      bit index to set (0 = least-significant bit)                    
 ...
 * value to match the input parameters.
 *
 * @attention This function differs from power_ic_get_reg_value() in that it can 
 * be called in a interrupt context.
 *
 * @param        reg        register number
 * @param        index      starting bit index (0 = least-significant bit)
...
int spi_int_get_reg_value(POWER_IC_REGISTER_T reg, int index, int *value, int nb_bits) 
{
    ...
    /* Read the whole register first */
    retval = spi_int_reg_read (reg, &reg_value);
...
/*!
 * @brief Set the value of a set of (possibly) non-contiguous bits in a power IC
 * register, even in interrupt context.
 *
 * This function is used to set a possibly non-contiguous set of bits in a power IC
 * register.  This function performs a read-modify-write operation on a power IC register.
 * In the case of those registers that must be physically read before being written (as
 ...
 * call.
 *
 * @attention This function differs from power_ic_set_reg_mask() in that it can be called
 * in a interrupt context.
 *
 * @param        reg        register number
 * @param        mask       bitmask indicating which bits are to be modified
