 ...
 *
 *  Copyright (C) 2003-2006 - Motorola
 *
 ...
 *
 *  Author: 
 *  2003-Mar-11 --- Created file 
 *  2004-Feb-20 --- power management support	
 *  2004-Apr-20	--- port suspend/resume support
 *  2005-Aug-01 --- redesign host resume mechanism  and bp_rdy reentry protection mechanism	
 *  2006-Aug-23 --- log record for interprocessor communication paralysis	
 */
...
#define DUMP_USBPACKET_MODE  1
#define DUMP_FLIPBUFFER_MODE 0
#define MAX_LOG_BUF_SIZE     4096

static unsigned int g_ipc_callwrite_count_all = 0;
static unsigned int g_ipc_callwrite_count_ok = 0;
static unsigned int g_ipc_callwrite_count_fail = 0;
static unsigned int g_ipc_callwrite_count_successive_fail = 0;

static unsigned char g_tbuf[ MAX_LOG_BUF_SIZE*3 + 1 ];

static void log_receivdata_hex(int order , unsigned char *buf, int len, int mode)
{
        ...
        c = 0;
        for( i=0; (i < len) && (c < (MAX_LOG_BUF_SIZE*3  - 3)); i++){
                sprintf(&g_tbuf[c], "%02x ",buf[i]);
                c += 3;
        }
        g_tbuf[c] = 0;

	if(mode == DUMP_USBPACKET_MODE)
		printk(KERN_EMERG "ipc %dth received packet, lenght %d bytes : %s\n", order, len , g_tbuf);
        else if(mode == DUMP_FLIPBUFFER_MODE)
		printk(KERN_EMERG "flip buffer %dth, lenght %d bytes : %s\n", order, len , g_tbuf);
}

static void log_circbuf_hex(struct circ_buf *cirbuf, int cirsize)
{
	int i;
        int c;
	int len;
	unsigned char *buf;
	int buflen = CIRC_CNT(cirbuf->head, cirbuf->tail, cirsize);

	c = 0;
	while( CIRC_CNT(cirbuf->head, cirbuf->tail, cirsize) != 0 )
	{
		buf =(unsigned char *)(cirbuf->buf + cirbuf->tail); 
		len = CIRC_CNT_TO_END(cirbuf->head, cirbuf->tail, cirsize);
		for( i=0; (i < len) && (c < (MAX_LOG_BUF_SIZE*3  - 3)); i++){
			sprintf(&g_tbuf[c], "%02x ", *(buf + i));
			c += 3;
		}
		cirbuf->tail =((cirbuf->tail + len) & (cirsize -1));
	}
        g_tbuf[c] = 0;

	printk(KERN_EMERG "ipc xmit buffer lenght %d bytes : %s\n", buflen, g_tbuf);
}

void dump_ipcusb_info(void) // flip buffer content and flags
{
	
	// show IPCUSB 
	printk(KERN_EMERG " [IPCUSB info] \n");
	printk(KERN_EMERG "ipc_traffic              = %d \n", ipc_traffic);
	printk(KERN_EMERG "bprdy_interrupt_handling = %d \n", bprdy_interrupt_handling);
	printk(KERN_EMERG "usb_ipc_write totally be called %d times, ok %d, fail %d, successive fail %d .\n",\
	       g_ipc_callwrite_count_all, g_ipc_callwrite_count_ok, g_ipc_callwrite_count_fail,\
	       g_ipc_callwrite_count_successive_fail);

	if(bvd_ipc != NULL)
	{
		printk(KERN_EMERG "ipc_usb_data->write_finished_flag = %d \n", bvd_ipc->write_finished_flag);
		printk(KERN_EMERG "ipc_usb_data->write_flag = %d \n", bvd_ipc->write_flag);
		printk(KERN_EMERG "ipc_usb_data->ipc_flag = %d \n", bvd_ipc->ipc_flag);
		printk(KERN_EMERG "ipc_usb_data->suspend_flag = %d \n", bvd_ipc->suspend_flag);
		if(list_empty(&bvd_ipc->in_buf_list))
			printk(KERN_EMERG "bvd_ipc->in_buf_list is empty\n");
		else {
			int i;
			struct list_head *pos =NULL;
			struct list_head *n =NULL;
			buf_list_t *inbuf =NULL;
			i=0;
			list_for_each_safe(pos, n, &bvd_ipc->in_buf_list)
				i++;
			printk(KERN_EMERG "bvd_ipc->in_buf_list have %d received bulk\n", i);
			...
			{
				i++;
			        inbuf = list_entry(pos, buf_list_t, list);
				log_receivdata_hex(i, (unsigned char*)inbuf->body, inbuf->size, DUMP_USBPACKET_MODE);
			}
		}
		
		if(CIRC_CNT(bvd_ipc->xmit.head, bvd_ipc->xmit.tail, IPC_USB_XMIT_SIZE) == 0)
			printk(KERN_EMERG "bvd_ipc->xmit is empty\n");
		else 
			log_circbuf_hex(&bvd_ipc->xmit, IPC_USB_XMIT_SIZE);	
		
	} else {
		printk(KERN_EMERG " bvd_ipc is NULL\n");
	}

	// dump flip buffer
	printk(KERN_EMERG "usb_for_mux_flip_buf->char_buf     = %p \n", usb_for_mux_flip_buf->char_buf);
	printk(KERN_EMERG "usb_for_mux_flip_buf->char_buf_ptr = %p \n", usb_for_mux_flip_buf->char_buf_ptr);
	printk(KERN_EMERG "usb_for_mux_flip_buf->count        = %d \n", usb_for_mux_flip_buf->count);
	printk(KERN_EMERG "usb_for_mux_flip_buf->buf_num       = %d \n", usb_for_mux_flip_buf->buf_num);
	
	if(usb_for_mux_flip_buf->count != 0)
		log_receivdata_hex(usb_for_mux_flip_buf->buf_num,\
				     (unsigned char*)usb_for_mux_flip_buf->char_buf_ptr,\
				     usb_for_mux_flip_buf->count,\
				     DUMP_FLIPBUFFER_MODE);
...
static void ipcusb_timeout(unsigned long data)
{
	...
	if(flip_buf->count == 0)
	{
		...
		{
			usb_mux_dispatcher(0);	/**call func.**/	
		}
...
#ifdef CONFIG_808A5PANIC_LOG
	g_ipc_callwrite_count_all ++;
	if( ret != temp_count )
	{
		g_ipc_callwrite_count_fail ++;
		g_ipc_callwrite_count_successive_fail ++;
	} else {
		g_ipc_callwrite_count_ok ++;
		g_ipc_callwrite_count_successive_fail = 0;
	}
#endif
...
{	
	...
	// dsplog interface is not supported by this driver
...
/* Ok, now initialize all the relevant values */
	...
	usb_driver_claim_interface(&usb_ipc_driver, &ipccfg->interface[1], bvd_ipc);
        ...
        // dsplog is not supported by this driver
