 ...
 *  2006-Apr-24 --- largen the interval of ipcsuspend, and just return when waked up bp timeout
 *  2006-Jun-22 --- bug fix for usb panic
 */
...
//			printk("\n test\n");
	...
	{ 
		...
		/*send a IN token*/
		bvd_ipc->readurb_mux.dev = bvd_ipc->ipc_dev;
		ret = usb_submit_urb(&bvd_ipc->readurb_mux);
		if (ret) {
			printk("[bf_readurb]urb submit, status=%d!\n", ret);
		}
		printk("[bf_readurb] IN token sent----\n");
		bvd_ipc->suspend_flag = 0;
