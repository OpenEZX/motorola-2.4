 ...
 * 
 * History:
 * 2006/05/23  	add hw workaround   	
 * 2006/01/03  	change retry time   	
...
#ifdef HW_WORKROUND
			edi = dma_to_ed (ohci, le32_to_cpup (&ed->hwNextED));
			if (!edi) {
				ed->state = ED_UNLINK;
				return 0;
			}
				
			edi->ed_prev = ed->ed_prev;
#else
			(dma_to_ed (ohci, le32_to_cpup (&ed->hwNextED)))->ed_prev = ed->ed_prev;
#endif
		}
...
void get_halted_bit(void)
{
	...
	bhed_v = (ed_t *)phys_to_virt(uhcbhead);
