...
#define USBD_DEFAULT USBD_NETWORK
#elif defined(CONFIG_USBD_SERIAL) || defined(CONFIG_USBD_SERIAL_MODULE)
#define USBD_DEFAULT USBD_SERIAL
#else
#error USBD enabled, but none of ethernet or serial interfaces chosen
#endif

int usbd_function = USBD_DEFAULT;

static void usbd_setup_function (char *function)
{
	char *functions[] = {"network", "serial"};
#if defined(CONFIG_USBD_NET) || defined(CONFIG_USBD_NET_MODULE)
	if (function && !strcmp (function, functions[0]))
		usbd_function = USBD_NETWORK;
#endif
#if defined(CONFIG_USBD_SERIAL) || defined(CONFIG_USBD_SERIAL_MODULE)
	if (function && !strcmp (function, functions[1]))
		usbd_function = USBD_SERIAL;
#endif
	if (usbd_function == 0) {
		printk (KERN_WARNING "USBD: parameter `%s' for `usbd=' is "
			"invalid, assume `%s'\n", (function ? function : "<NULL>"),
			functions[USBD_DEFAULT - 1]);
		usbd_function = USBD_DEFAULT;
	}
	printk (KERN_INFO "USBD: use %s function\n", functions[usbd_function - 1]);
}
...
int usbd_alloc_urb_data (struct urb *urb, int len)
{
	...
	//printk("\nurb->endpoint=%x\n", &(urb->endpoint));	
	//printk("\nurb->endpoint->endpoint_address=%d\n", urb->endpoint->endpoint_address);	
	//printk("\nurb->function_instance=%x\n", &(urb->function_instance));	
	//printk("\nfunction_driver=%x\n", &(urb->function_instance->function_driver));	
	//printk("\nops=%x\n", &(urb->function_instance->function_driver->ops));	
#if 0	
	if (urb->endpoint){
		printk("\nusbd_alloc_urb_data-1!\n");	
		if(urb->endpoint->endpoint_address){
			printk("\nusbd_alloc_urb_data-2!\n");	
			if(urb->function_instance){
				printk("\nusbd_alloc_urb_data-i3!\n");	
				if(urb->function_instance->function_driver->ops->alloc_urb_data) 
        			{
					printk("\nusbd_alloc_urb_data-4!\n");	
...
static int usbd_setup (char *str)
{
	usbd_setup_function (str);
	return 1;
}
__setup("usbd=", usbd_setup);
