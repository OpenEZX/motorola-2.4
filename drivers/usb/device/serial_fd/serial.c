...
int dbgflg_usbdfd_ser_usbe;
int dbgflg_usbdfd_ser_tx;
int dbgflg_usbdfd_ser_rx;
int dbgflg_usbdfd_loopback;

static debug_option dbg_table[] = {
	{&dbgflg_usbdfd_init, NULL, "init", "initialization and termination"},
	{&dbgflg_usbdfd_ser_usbe, NULL, "usbe", "USB events"},
	{&dbgflg_usbdfd_ser_rx, NULL, "rx", "receive (from host)"},
	{&dbgflg_usbdfd_ser_tx, NULL, "tx", "transmit (to host)"},
...
#define dbg_usbe(lvl,fmt,args...) dbgPRINT(dbgflg_usbdfd_ser_usbe,lvl,fmt,##args)
#define dbg_rx(lvl,fmt,args...) dbgPRINT(dbgflg_usbdfd_ser_rx,lvl,fmt,##args)
#define dbg_tx(lvl,fmt,args...) dbgPRINT(dbgflg_usbdfd_ser_tx,lvl,fmt,##args)
...
static int serial_xmit_data (int interface, unsigned char *data, int data_length)
{
	...
	//dbgPRINTmem(dbgflg_usbdfd_ser_tx,3,urb->buffer,urb->actual_length);
	dbgPRINTmem (dbgflg_usbdfd_ser_tx, 3, urb->buffer, CONFIG_USBD_SERIAL_IN_PKTSIZE + 4);
...
int serial_recv_urb (struct urb *urb)
{
	...
	dbgPRINTmem (dbgflg_usbdfd_ser_rx, 3, urb->buffer, urb->actual_length);
...
static int __init serial_modinit (void)
{
	...
	if (usbd_function != USBD_SERIAL) {
		printk (KERN_WARNING "USBD: serial function driver is disabled\n");
		return 0;
	}
	op = find_debug_option (dbg_table, "ser");
