...
int dbgflg_usbdfd_net_usbe;
int dbgflg_usbdfd_net_rx;
int dbgflg_usbdfd_net_tx;
...
static debug_option dbg_table[] = {
	...
	{&dbgflg_usbdfd_net_rx, NULL, "rx", "USB RX (host->device) handling"},
	{&dbgflg_usbdfd_net_tx, NULL, "tx", "USB TX (device->host) handling"},
	{&dbgflg_usbdfd_net_usbe, NULL, "usbe", "USB events"},
...
#define dbg_rx(lvl,fmt,args...) dbgPRINT(dbgflg_usbdfd_net_rx,lvl,fmt,##args)
#define dbg_tx(lvl,fmt,args...) dbgPRINT(dbgflg_usbdfd_net_tx,lvl,fmt,##args)
#define dbg_usbe(lvl,fmt,args...) dbgPRINT(dbgflg_usbdfd_net_usbe,lvl,fmt,##args)
...
static struct usb_class_description mdlm_comm_class_descriptions[] = {
      ...
      {USB_ST_MDLMD, 2, {mobile_direct_detail:{
                                                      // XXX FIXME
                                bGuidDescriptorType:0x00,
                                bDetailData:{0x0, 0x3}}}
      },

      //{ USB_ST_MDLMD, 1, { mobile_direct_detail: {
	...
	//{ USB_ST_UF,      1, { union_function: { bMasterInterface: 0, bSlaveInterface: { 0 }, }}},
};
...
//		printk("\nThis is DEVICE_CONFIGURED event!\n");
...
//		printk("\nThis is DEVICE_SET_INTERFACE event!\n");
#ifndef CONFIG_USBD_NET_NFS_SUPPORT
		if (!alwaysup) {
			net_create (device, function);
		}
...
//		printk("\nThis is DEVICE_BUS_INACTIVE event!\n");
...
//		printk("\nThis is DEVICE_BUS_ACTIVITY event!\n");
...
//		printk("\nThis is DEVICE_DESTROY event!\n");
		net_destroy (device);
		break;

	case DEVICE_FUNCTION_PRIVATE:
//		printk("\nThis is DEVICE_PRIVATE event!\n");
		switch (data) {
		case NET_SET_CRC:
			break;
		}
...
int net_recv_urb (struct urb *urb)
{
	...
	dbgPRINTmem (dbgflg_usbdfd_net_rx, 4, urb->buffer, length);
			...
			if (((fcs =
				...
				if ((fcs_memcpy32
					...
					dbg_rx (1, "CRC32 failed on extra byte len: %d fcs: %08x",
						length, fcs);
					dbgPRINTmem (dbgflg_usbdfd_net_rx, 1, urb->buffer, length);
	...
	dbg_rx (3, "skb: %p head: %p data: %p tail: %p end: %p len: %d",
		skb, skb->head, skb->data, skb->tail, skb->end, skb->len);
	dbgPRINTmem (dbgflg_usbdfd_net_rx, 3, skb->data, skb->len);
#endif
...
//#warning NO ZLP SUPPORT DEFINED
	...
	dbgPRINTmem (dbgflg_usbdfd_net_tx, 4, urb->buffer, urb->actual_length);
...
static int __init net_modinit (void)
{
	...
	if (usbd_function != USBD_NETWORK) {
		printk (KERN_WARNING "USBD: network function driver is disabled\n");
		return 0;
	}
	op = find_debug_option (dbg_table, "net");
