 ...
 *   Date      Author    Comment
 * ==========  ========  ===============
 * 08/30/2004  Motorola  Intel pm workaround2
 * 07/21/2005  Motorola  Update xtime
 * 12/22/2005  Motorola  GPIO99 should be low	 
 * 03/20/2006  Motorola  bug fix for wake up
 * 07/19/2006  Motorola  support macau and penglai
 * 08/17/2006  Motorola  config GPIOs of IPC connection during wakeup for current drain issue
 * 09/19/2006  Motorola  Disable SWCR after wakeup if enable it before sleep
 * 10/11/2006  Motorola  Restore CLKCFG after wakeup, cpu will exit turbo-mode after wakeup
 * 10/12/2006  Motorola  Correct wrong writting value code to RTSR 
 * 12/05/2006  Motorola  workaround for call time inaccuracy 
 */
  ...
  */
	...
	switch(flag) {
		case SAVE_REG:
...
#endif 
			/* 
			 * workaround for call time inaccuracy, never touch 
                         * need_restore unless it is strictly necessary.	
			 */ 
			...
			RTSR = (RTSR & RTSR_STATUS_BITS_MASK) | RTSR_SWCE;  	
		...
		case RESTORE_REG:
			if (need_restore == 1) {
              ...
              */
                /*
                 * set SWCR to a large value to avoid match SWAR by accident
                 * SWAR will less than 120s, because the largest interval of emud time is 2min.
                 */
				SWCR = (1 |
					1 << 7 |
					1 << (7 + 6) |
 ...
 * sleep count. 
 *
 * Since the resolution of SWCR (we use to count the sleep time) is 10ms,
 * we must sync the xtime from PCAP RTC every 100 scount to limited the 
 * inaccuracy of xtime to < 1s,
 */ 
static unsigned short scount = 0;
#define XTIME_NEED_SYNC 100

extern long next_rtc_update;
extern int power_ic_rtc_get_time(struct timeval * power_ic_time);
...
static inline void time_restore(unsigned long s_rcnr, unsigned long s_swcr)
{
	...
	if (++scount >= XTIME_NEED_SYNC) {
		...
		/* time to sync RTC to xtime */
		next_rtc_update = xtime.tv_sec; 
...
#define REMOVE_IPC_CURRENT_DRAIN 1 
...
int pm_do_suspend(unsigned int mode)
{
    ...
    asm("mrc\tp14, 0, %0, c6, c0, 0" :"=r"(save_clkcfg));
    printk("pm_do_suspend : 3 : CLKCFG = 0x%8x\n",save_clkcfg&0x0f);
	/* *** go zzz *** */
	pxa_cpu_suspend(mode);
    save_clkcfg = (save_clkcfg&0x0f)|0x02;
    asm("mcr\tp14, 0, %0, c6, c0, 0" ::"r"(save_clkcfg));
...
#if (!defined(CONFIG_ARCH_EZX_SUMATRA) && !defined(CONFIG_ARCH_EZX_MACAU) && !defined(CONFIG_ARCH_EZX_PENGLAI))
	...
	GPCR(GPIO_USB_READY) = GPIO_bit(GPIO_USB_READY);/* clear gpio99 to low */
	GPSR(GPIO_BB_RESET) = GPIO_bit(GPIO_BB_RESET);
        ...
        GPCR(GPIO_MMC_CMD) = GPIO_bit(GPIO_MMC_CMD);
...
#if (defined(CONFIG_ARCH_EZX_SUMATRA) || defined(CONFIG_ARCH_EZX_MACAU) || defined(CONFIG_ARCH_EZX_PENGLAI))
	GPSR(GPIO_USB20_NCS3) = GPIO_bit(GPIO_USB20_NCS3);
...
#if REMOVE_IPC_CURRENT_DRAIN
    GPSR2    =  0x0c000000;   // gpio 90, 91 level high
    GPCR3    =  0x20000;     // gpio 113 level low
    GPDR0   &= ~0xc0000000;  // gpio 30, 31 input
    GPDR1   &= ~0x01000000;  // gpio 56 input
    GPDR2   |=  0x0c000000;  // gpio 90, 91 output
    GPDR3   |=  0x20000;     // gpio 113 output
#endif
...
#if REMOVE_IPC_CURRENT_DRAIN
    GAFR0_U &= ~0xf0000000;  // gpio 30, 31 general gpio function
    GAFR1_U &= ~0x30000;     // gpio 56 general gpio function
    GAFR2_U &= ~0x00f00000;  // gpio 90, 91 general gpio function
    GAFR3_U &= ~0xc;         // gpio 113 general gpio function
#endif
