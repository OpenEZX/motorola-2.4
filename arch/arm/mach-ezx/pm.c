 ...
 *  Date         Comment
 * ==========   ===============
 * 08/30/2004    Intel pm workaround2
 * 07/21/2005    Update xtime
 * 12/22/2005    GPIO99 should be low	 
 * 03/20/2006    bug fix for wake up
 * 06/20/2006    avoid LCD_SD toggling periodically during wakup
 * 07/24/2006	 restore SPI gpio state after resume
 */
...
int pm_do_suspend(unsigned int mode)
{
	...
	/*restore LCD_SD high after wakeup*/
	GPSR(GPIO_LCD_OFF) = GPIO_bit(GPIO_LCD_OFF);
#endif		
	...
	/*Restore the gpio state of tf CS, PCAP CS and SPI_MOSI to avoid SPI bus chaos caused by
	 conflicting access from PCAP and TF*/
	GPCR(GPIO_SPI_CE) = GPIO_bit(GPIO_SPI_CE);
	GPSR(GPIO20_DREQ0) = GPIO_bit(GPIO20_DREQ0);
