 ...
 */
/* support GPIO 2-120 */
static int GPIO_IRQ_rising_edge[4];
static int GPIO_IRQ_falling_edge[4];
...
/*
 * Demux handler for GPIO 2-120 edge detect interrupts
 */

static int GPIO_2_120_enabled[4];	/* enabled i.e. unmasked GPIO IRQs */
static int GPIO_2_120_spurious[4];	/* GPIOs that triggered when masked */

static void pxa_GPIO_2_120_demux(int irq, void *dev_id,
				    struct pt_regs *regs)
{
	int i, gedr, spurious;
	...
	while ((gedr = (GEDR0 & ~3))) {
		 ...
		 */
		spurious = gedr & ~GPIO_2_120_enabled[0];
		if (spurious) {
			GEDR0 = spurious;
			GRER0 &= ~(spurious & GPIO_2_120_spurious[0]);
			GFER0 &= ~(spurious & GPIO_2_120_spurious[0]);
			GPIO_2_120_spurious[0] |= spurious;
			gedr ^= spurious;
			if (!gedr) continue;
		}
	...
	while ((gedr = GEDR1)) {
		spurious = gedr & ~GPIO_2_120_enabled[1];
		if (spurious) {
			GEDR1 = spurious;
			GRER1 &= ~(spurious & GPIO_2_120_spurious[1]);
			GFER1 &= ~(spurious & GPIO_2_120_spurious[1]);
			GPIO_2_120_spurious[1] |= spurious;
			gedr ^= spurious;
			if (!gedr) continue;
		}
	...
	while ((gedr = GEDR2)) {
		spurious = gedr & ~GPIO_2_120_enabled[2];
		if (spurious) {
			GEDR2 = spurious;
			GRER2 &= ~(spurious & GPIO_2_120_spurious[2]);
			GFER2 &= ~(spurious & GPIO_2_120_spurious[2]);
			GPIO_2_120_spurious[2] |= spurious;
			gedr ^= spurious;
			if (!gedr) continue;
		}
	...
	while ((gedr = (GEDR3 & 0x01ffffff))) {
                spurious = gedr & ~GPIO_2_120_enabled[3];
                if (spurious) {
                        GEDR3 = spurious;
                        GRER3 &= ~(spurious & GPIO_2_120_spurious[3]);
                        GFER3 &= ~(spurious & GPIO_2_120_spurious[3]);
                        GPIO_2_120_spurious[3] |= spurious;
                        gedr ^= spurious;
                        if (!gedr) continue;
                }
...
static struct irqaction GPIO_2_120_irqaction = {
	name:		"GPIO 2-120",
	handler:	pxa_GPIO_2_120_demux,
	flags:		SA_INTERRUPT
};

static void pxa_mask_and_ack_GPIO_2_120_irq(unsigned int irq)
{
	int gpio_nr = IRQ_TO_GPIO_2_120(irq);
	int mask = 1 << (gpio_nr & 0x1f);
	int index = gpio_nr >> 5;
	GPIO_2_120_spurious[index] &= ~mask;
	GPIO_2_120_enabled[index] &= ~mask;
	GEDR(gpio_nr) = mask;
}

static void pxa_mask_GPIO_2_120_irq(unsigned int irq)
{
	int gpio_nr = IRQ_TO_GPIO_2_120(irq);
	int mask = 1 << (gpio_nr & 0x1f);
	int index = gpio_nr >> 5;
...
static void pxa_unmask_GPIO_2_120_irq(unsigned int irq)
{
	int gpio_nr = IRQ_TO_GPIO_2_120(irq);
	int mask = 1 << (gpio_nr & 0x1f);
	int index = gpio_nr >> 5;

	if (GPIO_2_120_spurious[index] & mask) {
		/*
...
void __init pxa_init_irq(void)
{
	...
	for (irq = IRQ_GPIO(2); irq <= IRQ_GPIO(120); irq++) {
		irq_desc[irq].valid	= 0;
		irq_desc[irq].probe_ok	= 1;
		irq_desc[irq].mask_ack	= pxa_mask_and_ack_GPIO_2_120_irq;
		irq_desc[irq].mask	= pxa_mask_GPIO_2_120_irq;
		irq_desc[irq].unmask	= pxa_unmask_GPIO_2_120_irq;
	}
	setup_arm_irq( IRQ_GPIO_2_120, &GPIO_2_120_irqaction );
}
