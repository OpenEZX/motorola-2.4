 ...
 *
 *  2002-Nov-11  Porting to EzX platform
 *  2003-May-12  Make apm devie only  be accessed by one user
 *  2004-Feb-12  Porting wakeup check routine to E680/A780 (Bulverde)
 *  2005-Jun-30  Update pcap interface to spi mode
 *  2006-Apr-10  Add a delay sleep interface
 *  2006-Apr-11  adjust flash suspend function to the bottom of suspend list 
 *               in order to ensure that flash is in read array status before 
 *               Bulverde enter sleep  
 *
 *  2006-Mar-10  Add CONFIG_APM_ISP to support shorting idle time during periodic job.
 *
 *  2006-Jun-20  Improve sleep control of pm ( periodic_jobs_done will notify apmd to go to sleep other than sleep directly ).
 *
 *  2006-Aug-21  Log pm info as interprocessor communication paralysis 
 *
 *  2006-Sep-08  add debugging info  
 *
 *  2006-Oct-09  close debugging info 
...
void dump_pm_info(void) //power management registers and flags
{
	// show flags
        printk(KERN_EMERG " [Power Management flags] \n");
        printk(KERN_EMERG " can_sleep        = %d\n", can_sleep);
        printk(KERN_EMERG " periodic_wake    = %d\n", periodic_wake);
	printk(KERN_EMERG "\n");

}
#endif

#if defined(CONFIG_APM_ISP)
unsigned long periodic_effect_irq_count; /* counter of effect interrupt during RTC wake up */
#endif

static int user_off_available = 0;
static unsigned long delay_sleep_jiffy = 0;
...
#ifdef CONFIG_ARCH_EZX_SUPPORT_EXT_USB20
extern void udc_reflash (void);
#endif
...
static void check_wakeup(void)
{
	...
	{
		printk("Periodic wake up, <periodic_wake>=1.\n");
		periodic_wake = 1;
	}
	else
	{
		printk("No periodic wake up, <periodic_wake>=0.\n");
		periodic_wake = 0;
	}
...
/*
 * we should combine delay_sleep_jiffy, sleep_jiffy, and can_sleep.
 * to a framework in the future.
 */
int pm_delay_sleep(unsigned long tm)
{
        unsigned long flags;
                                                                                                                             
        local_irq_save(flags);
        if (time_after(jiffies + tm, delay_sleep_jiffy)) {
                 delay_sleep_jiffy = jiffies + tm;
        }
        local_irq_restore(flags);
...
static int pm_go_sleep(int arg)
{
	...
	sleep_jiffy = 0;
	if ((can_sleep && (!ipc_is_active()) && (!periodic_wake) && time_after(jiffies, delay_sleep_jiffy)) || arg) {
		printk("apm: pm_go_sleep: \n");
                if (pm_send_all(PM_SUSPEND, (void *)3) == 0) {
                        ezx_i2c_pm_suspend();
                        wmmx_ohci_suspend_call();
			status = mtd_table[0]->suspend(mtd_table[0]);
			if (!irq_pending() && !status) {
                		if(usb_host_suspended)
		                     UP3OCR = 0x3;
                        	pm_do_suspend(CPUMODE_SLEEP);
				clear_dpcsr();
			}
			mtd_table[0]->resume(mtd_table[0]);
                        APM_DPRINTK("resume\n");
                        ezx_i2c_pm_resume();
                        pm_send_all(PM_RESUME, (void *)0);
                        check_wakeup();
			printk("apm: exit from sleep.\n");
                } 
        }
...
int periodic_jobs_done(void)
{
	...
	if (periodic_wake) {
...
#if defined(CONFIG_APM_ISP)
	        printk("periodic jobs completed.\n");

		ignore_irqs  = apm_get_ignore_num(); /* get ignored event number */

		/* periodic_effect_irq_count > ignore_irqs : RTC wakeup and there are other irqs except pcap_atod irq 
		 * periodic_effect_irq_count == ignore_irqs : only pcap_atod irq after RTC wakeup
		 * periodic_effect_irq_count < ignore_irqs : RTC wakeup and there is BP ready irq happens
		 */
		if ( periodic_effect_irq_count == ignore_irqs ) {
		        /* we can go to sleep after finish periodic job if effect irqs equals ignored irqs*/
		        //pm_go_sleep(0);              
			/*
			 * To avoid race condition, only apmd can put phone into sleep mode.
			 */
		...
		periodic_effect_irq_count = 0;       /* reset counter */
		apm_clear_ignore_int();
#else
		//pm_go_sleep(0);
		/*
		 * To avoid race condition, only apmd can put phone into sleep mode.
...
static unsigned int ignored_int_num;  /* counter of Ignored Specific-event in Periodic-job */

/*
 * Interface for other module which will implement CONFIG_APM_ISP
 */
void apm_inc_ignore_int(enum periodic_ignore_event event)
{
  /* only increase counter when periodic jobs are active */
  if ( !periodic_wake )
    return;
...
void apm_clear_ignore_int(void)
{
  ignored_int_num = 0;
}

int apm_get_ignore_num(void)
{
  return ignored_int_num;
}
#endif /* CONFIG_APM_ISP */
...
#ifdef CONFIG_ARCH_EZX_SUPPORT_EXT_USB20
	/* Disconnect and disable ISP1583 so that it will disappear in Windows Device Manager.*/
	udc_reflash();
#endif
