 ...
 *
 *  2006-Apr-11  adjust flash suspend function to the bottom of suspend list in order to ensure that flash is in read array 
 *	         status before Bulverde enter sleep.
 * 
 *  2006-Apr-27  Add a new interface for other components.
 *
 *  2006-May-15  Add an IOCTL to get flip status during pwrup to black screen issue at opening flip during pwrup.
 *
 *  2006-Jun-06  Improve sleep control of pm.
 *
 *  2006-Jun-20	 bug fix for powerdown fail
 */
...
#define WLAN_FIRMWARE_REQ_SLEEP
#endif  
...
int periodic_wake = 0;
#ifdef WLAN_FIRMWARE_REQ_SLEEP
int fix_periodic_wake = 0;
#endif
...
#ifdef WLAN_FIRMWARE_REQ_SLEEP
/*
 * Some drivers can call this function to request sleep. It should be
 * a common API. It should not affect other compoment's behavior. 
 * So in order to avioding critical timing issues, we focus on this case:
 *       |------wireless apps--------------(-----------------)
 *       |                    |(jobs_done) |(fireware sleep) |(1 second)
 *       |                                 |                 
 *  -----|(RTC)                            ------------------|-------
 */ 
void apm_req_sleep(int info)
{
	if (sleep_to && fix_periodic_wake && !periodic_wake) {
		apm_event_notify(APM_EVENT_REQ, EVENT_REQ_SLEEP, info);
	}
}
EXPORT_SYMBOL(apm_req_sleep);
#endif
...
{
	printk("rtc jobs done!, periodic_wake=%d\n", periodic_wake);
	if (periodic_wake) {
		periodic_wake = 0;
		/*
...
#include <linux/power_ic.h>
             ...
             power_ic_periph_set_usb_pull_up(0);
             mdelay(10);
             power_ic_set_reg_bit(POWER_IC_REG_EOC_CONN_CONTROL,19,1);// set USB_CNTRL(bit19) to disable emu control
             mdelay(1000);
#endif
	...
	case APM_IOC_SET_IPM_CONFIG:
		...
		break;
	case APM_IOC_GET_PWRUP_FLIP: /*get flip status during pwrup*/
		flip_status = GPLR(GPIO_FLIP_PIN) & GPIO_bit(GPIO_FLIP_PIN);
		if(!flip_status)		
			APM_DPRINTK("apm_ioctl:default flip: close \n");
		else	
			APM_DPRINTK("apm_ioctl:default flip: open \n");	
		return (copy_to_user((void *)arg, &flip_status,sizeof(int)))? -EFAULT:0;	
		break;		
	default:
		return -EINVAL;
	}
