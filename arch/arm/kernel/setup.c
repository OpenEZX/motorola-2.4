 ...
 *
 *  Copyright (C) 2005-2006 Motorola - Port to EzX.
 ...
 *  Author      Date               Comment
 * =========    ===============    ==================
 * Motorola      2006-10-25        update interface for poweric driver
 * Motorola      2006-10-18        Change on power up reason after reflashing
 * Motorola      2006-01-20        change for powerdown
 * Motorola      2005-12-02        Add power up flag
 * Motorola      2005-08-19        Add power up reason
 *
 */
...
#ifdef CONFIG_ARCH_EZXBASE
/*
 * with charger, phone should reboot to charge only mode after poweroff.
 * with charger, phone should reboot to normal mode after masterclear/reset.
 * So we use APP_REBOOT_FLAG and CHARGE_FLAG to differentiate these case.
 */
#define POWER_ON_MODE_SW_REBOOT (0)
#define POWER_ON_MODE_CHG_FLAG  (1 << 8)
#define POWER_ON_MODE_FACTORY_FLAG (1<<5)

  if (APP_REBOOT_FLAG == *(unsigned long *)(phys_to_virt(FLAG_ADDR))) {
	printk(KERN_WARNING "reboot to full service mode\n");
	/* 
	   RAMOS will set POWER_ON_MODE_SW_REBOOT after reflash finishes, so device
	   will enter full power mode in this case, but due to original design, power
	   up reason from MBM will be reset to 0 in this case, and TCMD will fail to
	   receive factory cable reason after reflash. Factory cable will enter modem
	   mode, instead of desired BLAN mode.
	   
	   To avoid more new issues here, we just check if the reason from MBM is factory 
	   cable when POWER_ON_MODE_SW_REBOOT is set. We will keep factory reason value
	   in such case.
	   
	   Common USB cable is another case, and after reflashing with common usb cable
	   The new design should be same as the old- We just touch factory cable here.
	*/
	if(!(power_up_reason & POWER_ON_MODE_FACTORY_FLAG)) /* if reason is not factory cable */
	power_up_reason = POWER_ON_MODE_SW_REBOOT;
  } else if (CHARGE_FLAG == *(unsigned long *)(phys_to_virt(FLAG_ADDR))) {
	/* 
         * If charger is removed between the accy check point of kernel and MBM,
         * the power_up_reason is 0, so we should correct it.
         */
	printk(KERN_WARNING "change to charge only mode\n");
	power_up_reason = POWER_ON_MODE_CHG_FLAG;
  }
