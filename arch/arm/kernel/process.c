 ...
 *
 *  Copyright (C) 2005-2006 Motorola, Inc. - Port to EzX platform.
 ...
 * Revision History:
  ...
  ------------       -------------------------
   07/14/2005          deep idle 
   11/22/2005          print power off
   04/12/2006          close blue screen
*/
...
void cpu_idle(void)
{
	...
	while (1) {
		while (!need_resched()) {
			...
			if (!idle)
				idle = default_idle;
            /* Since EzX does not enable DPM, so dmp functions are commented out, 6/15/2005  */
