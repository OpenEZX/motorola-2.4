 ...
 * Motorola 2006-Jun-13   modified the panic log bankwidth dependency
 * Motorola 2007-Jan-11   fix mount record
 */
...
int mnt_record_inc_part(struct mnt_record_info *mnt_record_i, int num_part)
{
	...
	ret = mnt_record_write(ofs, sizeof(uint8_t), &retlen, &record);
	if (ret) {
