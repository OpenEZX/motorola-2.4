 ...
 * 2005/01/20		 Calculate the slab free space for EzX
 * 2006/05/12       :  update mmlist_read_proc to use proc_sprintf
 */
				 ...
				 int count, int *eof, void *data)
{
	...
	proc_sprintf(page, &off, &len,
		"active/inactive page list info,\n"
		"D dirty, L lock, C count\n");
	proc_sprintf(page, &off, &len, 
		"ACTIVE list(%d):\n", nr_active_pages);
	...
	while (active_next != active_head)
	{
		...
		if (!this_page->mapping)
			proc_sprintf(page, &off, &len, 
				"PAGE(0x%x) anonymouse, active, "
				"D: %c, "
				"L: %c, "
				"C: %d\n", 
				(unsigned int)this_page,
				PageDirty(this_page) ? 'Y' : 'N', 
				PageLocked(this_page) ? 'Y' : 'N',
				page_count(this_page));
		else
		{
