 ...
 *
 * 2005/10/11     Mount record interface added on summary
 *
 * 2006/03/12	  bug fix for CRC patch 
 *
...
int jffs2_sum_del_mem(struct jffs2_eraseblock *jeb, uint32_t ofs, uint32_t size)
{
	union jffs2_sum_mem *walk, *prev;
	int ret, sumsize = 0;

        if ((ret=jffs2_sum_care_sum_collected(jeb))) return ret;
	
	ofs = ofs - jeb->offset;
	if (jeb->sum_collected && jeb->sum_collected->sum_list) {
		walk = prev = jeb->sum_collected->sum_list;
		while (walk) {
			switch (je16_to_cpu(walk->u.nodetype)) {
				case JFFS2_NODETYPE_INODE :
					if (ofs == je32_to_cpu(walk->i.offset)) {
						sumsize = JFFS2_SUMMARY_INODE_SIZE;
						goto found_mem_node;
					}
					break;
				case JFFS2_NODETYPE_DIRENT :
					if (ofs == je32_to_cpu(walk->d.offset)) {
						sumsize = JFFS2_SUMMARY_DIRENT_SIZE(walk->d.nsize);	
						goto found_mem_node;
					}
					break;
				default:
	...
	D1(printk(KERN_DEBUG "Cannot find the node sum info at jeb offset(0x%x)\n", jeb->offset));
        return 0;

found_mem_node:
	walk->u.nodetype = cpu_to_je16(je16_to_cpu(walk->u.nodetype) & ~JFFS2_NODE_ACCURATE);
	D1(printk(KERN_DEBUG "Mark the sum node on list jeb offset(0x%x)\n", jeb->offset));
/*
	if ((uint32_t)prev == (uint32_t)(jeb->sum_collected->sum_list))
		jeb->sum_collected->sum_list = walk->u.next;
	else
		prev->u.next = walk->u.next;
	D1(printk(KERN_INFO "Delete the sum size in jeb (0x%x) sumsize(%d), type (0x%x) sum_size(%d) sum_padded(%d)\n", jeb->offset, sumsize, je16_to_cpu(walk->u.nodetype), jeb->sum_collected->sum_size, jeb->sum_collected->sum_padded));
	jeb->sum_collected->sum_size -= sumsize;
	jeb->sum_collected->sum_num--;
        jeb->sum_collected->sum_padded += size;
	D1(printk(KERN_INFO "       sum_paded(%d) sum_size(%d) sum_num(%d)\n",jeb->sum_collected->sum_padded, jeb->sum_collected->sum_size, jeb->sum_collected->sum_num));
	kfree(walk);
*/
	return 0;
}
...
#ifdef CONFIG_ARCH_EZXBASE
int jffs2_sum_mark_obsolete(struct jffs2_sb_info *c, uint32_t ofs)
{
	struct jffs2_eraseblock *jeb;
	...
	ret = jffs2_fill_scan_buf(c, (unsigned char *)sm, jeb->offset + c->sector_size - sizeof(struct jffs2_sum_marker), sizeof(struct jffs2_sum_marker));
	if (ret) {
		kfree(sm);
		return ret;
	}
	
	if (je32_to_cpu(sm->magic) == JFFS2_SUM_MAGIC && je32_to_cpu(sm->erase_size) == c->sector_size) {
		summary = (struct jffs2_summary_node *) kmalloc(c->sector_size - je32_to_cpu(sm->offset), GFP_KERNEL);

		if (!summary) {
			kfree(sm);
			return -ENOMEM;
		}
		ret = jffs2_fill_scan_buf(c, (unsigned char *)summary, jeb->offset + je32_to_cpu(sm->offset),c->sector_size - je32_to_cpu(sm->offset));
		if (ret) {
			kfree(summary);
			kfree(sm);
			return ret;
		...
		for(i = 0; i < je16_to_cpu(summary->sum_num); i++) {
			...
			switch (je16_to_cpu(sp->nodetype) | JFFS2_NODE_ACCURATE) {
			case JFFS2_NODETYPE_INODE : {
				struct jffs2_sum_inode_flash *spi;
				spi = (struct jffs2_sum_inode_flash *)sp;
				if ((ofs - jeb->offset)	== je32_to_cpu(spi->offset)) {
					D1(printk("%s: spi->nodetype=%x spi->totlen%d\n",__FUNCTION__, spi->nodetype, spi->totlen));
					DIRTY_SPACE(PAD(je32_to_cpu(spi->totlen)));
					spi->nodetype = cpu_to_je16(je16_to_cpu(spi->nodetype) & ~JFFS2_NODE_ACCURATE);
					goto found_block;
				}
				else{
					temp8ptr = (uint8_t *)sp;
			...
			case JFFS2_NODETYPE_DIRENT : {
				...
				if ((ofs - jeb->offset)	== je32_to_cpu(spd->offset)) {
					D1(printk("%s: spd->nodetype=%x spd->totlen\n",__FUNCTION__, spd->nodetype, spd->totlen));
					DIRTY_SPACE(PAD(je32_to_cpu(spd->totlen)));
					spd->nodetype = cpu_to_je16(je16_to_cpu(spd->nodetype) & ~JFFS2_NODE_ACCURATE);
					goto found_block;
				}
				else{
					temp8ptr = (uint8_t *)sp;
			...
			default : {
				printk(KERN_WARNING "Wrong type of node 0x%x \n", (je16_to_cpu(sp->nodetype)|JFFS2_NODE_ACCURATE));
				return -EIO;
			} 
			}			
		}
	...
	kfree(sm);
	D1(printk(KERN_DEBUG "Can not find node on summary node\n"));
	return 0; 

found_block:
	np = (uint8_t *)sp;
	offs = jeb->offset + je32_to_cpu(sm->offset) + np - (uint8_t *)summary;
	D1(printk(KERN_DEBUG "%s: offs=%x np=0x%p\n",__FUNCTION__, offs, np));
	if (offs & (map_bankwidth(map)-1)) {
		gap = offs - (offs & ~(map_bankwidth(map)-1));
		offs -= gap;
		np -= gap;		
#if 0 //perhaps need for further 
		ret = jffs2_flash_write(c, offs, map_bankwidth(map), &retlen, np);
		if (ret)
			return ret;
#endif
		offs += map_bankwidth(map);
		np += map_bankwidth(map);
	}
	ret = jffs2_flash_write(c, offs, map_bankwidth(map), &retlen, np);
	kfree(summary);
	kfree(sm);
...
uint32_t jffs2_sum_data_crc(uint32_t crc, void *summary_buf, int sum)
{
	union jffs2_sum_flash *sp;
	int i, d_size;

	D1(printk(KERN_DEBUG "%s sum = %d", __FUNCTION__, sum));	
	sp = (union jffs2_sum_flash *)summary_buf;
	for(i = 0; i < sum; i++) {
		uint8_t *temp8ptr = NULL;
		switch (je16_to_cpu(sp->u.nodetype) | JFFS2_NODE_ACCURATE) {
			case JFFS2_NODETYPE_INODE : {
				d_size = JFFS2_SUMMARY_INODE_SIZE - sizeof(jint16_t);
				temp8ptr = (uint8_t *)sp;
				temp8ptr += sizeof(jint16_t);
				crc = crc32(crc, temp8ptr, d_size);
				
				temp8ptr += d_size;
				sp = (union jffs2_sum_flash *)temp8ptr;
				break;
			}
			case JFFS2_NODETYPE_DIRENT : {
				d_size = JFFS2_SUMMARY_DIRENT_SIZE(sp->d.nsize) - sizeof(jint16_t); 
				temp8ptr = (uint8_t *)sp;
				temp8ptr += sizeof(jint16_t);
				crc = crc32(crc, temp8ptr, d_size);
			...
			default : {
				D1(printk(KERN_WARNING "Type node 0x%x doesn't supported in summary!\n", je16_to_cpu(sp->u.nodetype)));
				return -EIO;
			}
		}
	}
...
int jffs2_sum_scan_sumnode(struct jffs2_sb_info *c, struct jffs2_eraseblock *jeb, uint32_t ofs, uint32_t *pseudo_random)
{
...
#ifdef CONFIG_ARCH_EZXBASE
	crc = jffs2_sum_data_crc(0, summary->sum, je16_to_cpu(summary->sum_num));
	if(je32_to_cpu(summary->sum_crc) != crc){
		D1(printk(KERN_DEBUG "jffs2_scan_eraseblock(): Summary node data original crc is change(sum_crc=%x CRC=%x)\n", je32_to_cpu(summary->sum_crc), crc));
	...
	if ((!bad_sum) && (je32_to_cpu(summary->sum_crc) != crc)) {
		printk(KERN_DEBUG "jffs2_scan_eraseblock(): Summary node data is corrupt (sum_crc=%x CRC=%x)\n", je32_to_cpu(summary->sum_crc), crc);
		bad_sum = 1;
	}
...
#ifdef CONFIG_ARCH_EZXBASE
			if (!(je16_to_cpu(sp->nodetype) & JFFS2_NODE_ACCURATE)){
				/* Wheee. This is an obsoleted node on summary*/
				switch (je16_to_cpu(sp->nodetype) | JFFS2_NODE_ACCURATE) {
					case JFFS2_NODETYPE_INODE : {
						struct jffs2_sum_inode_flash *spi;
						spi = (struct jffs2_sum_inode_flash *) sp;
						D1(printk(KERN_DEBUG "jffs2_scan_eraseblock(): Inode at 0x%08x is obsolete\n", jeb->offset + je32_to_cpu(spi->offset)));
						DIRTY_SPACE(PAD(je32_to_cpu(spi->totlen)));
						temp8ptr = (uint8_t *) sp;
						temp8ptr += JFFS2_SUMMARY_INODE_SIZE;
						sp = (struct jffs2_sum_unknown_flash *) temp8ptr;
					...
					default : {
						D1(printk("Kernel doesn't support this type of node in summary!!!\n"));
						return -EIO;
						break;			
					}
				}
...
int jffs2_sum_write_sumnode(struct jffs2_sb_info *c)
{
	...
	D1(printk(KERN_DEBUG "JFFS2_SUMMARY write sum_size(%d) sum_padded(%d) sum_num(%d)\n        datasize(%d) infosize(%d) padsize(%d)\n", jeb->sum_collected->sum_size, jeb->sum_collected->sum_padded, jeb->sum_collected->sum_num, datasize, infosize, padsize));
	...
	D1(printk(KERN_DEBUG "JFFS2_SUMMARY write jeb offset(0x%x)\n", jeb->offset));
		
	while (jeb->sum_collected->sum_num) { // write sum_data 
#ifdef CONFIG_ARCH_EZXBASE
		switch(je16_to_cpu(jeb->sum_collected->sum_list->u.nodetype) |JFFS2_NODE_ACCURATE){
#else
	...
	D1(printk(KERN_DEBUG "JFFS2_SUMMARY write jeb->unchecked_size=%d, jeb->used_size=%d, jeb->dirty_size=%d, jeb->wasted_size=%d, jeb->free_size=%d\n", jeb->unchecked_size, jeb->used_size, jeb->dirty_size, jeb->wasted_size, jeb->free_size)); 
	jeb->sum_collected->sum_size = 0;
	jeb->sum_collected->sum_padded = 0;
	...
	*(wpage++) = magic;
#ifdef CONFIG_ARCH_EZXBASE
	isum.sum_crc = cpu_to_je32(jffs2_sum_data_crc(0, c->summary_buf, 
						je16_to_cpu(isum.sum_num)));
	D1(printk(KERN_DEBUG "JFFS CRC print sum_crc=%x\n", je32_to_cpu(isum.sum_crc)));
#else
