 ...
 * 2005/04/13	Motorola imply summary patch
 *     Fixed summary patch boundary calculating error
 *
 * 2006/03/13	fix bug of CRC patch 
 *
 ...
 restart:
	...
	if (sumsize != JFFS2_SUMMARY_NOSUM_SIZE) {
		...
		if (jeb) {
			...
			D1(printk(KERN_DEBUG "JFFS2:jeb(0x%x) minsize %d , jeb->free(%d), sum_collected->size(%d), sum_collected->padded(%d), sumsize(%d)\n", jeb->offset, minsize,jeb->free_size,jeb->sum_collected->sum_size,jeb->sum_collected->sum_padded,sumsize));
		}
...
#if defined(CONFIG_ARCH_EZXBASE) && defined(CONFIG_JFFS2_SUMMARY)
	D1(printk(KERN_DEBUG "obliterating obsoleted node from summary\n"));
	ret = jffs2_sum_mark_obsolete(c, ref_offset(ref));
	if (ret)
		goto out_erase_sem;
		
	spin_lock(&c->erase_completion_lock);
	ret = jffs2_sum_del_mem(jeb, ref_offset(ref), PAD(ref_totlen(c, jeb, ref)));
	spin_unlock(&c->erase_completion_lock);
	if (ret)
		goto out_erase_sem;
#endif
