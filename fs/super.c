 ...
 *
 *  Modified for EzX peoducts.  15, JAN 2005 
 *
...
restart:
	...
	while (sb != sb_entry(&super_blocks))
		...
		{
			/* Added to trigger GC in kupdate kernel thread */
			if ( (!strcmp(current->comm,"kupdated")) && (!strcmp(sb->s_type->name,"vfm")) ) //we are in kupdate kernel thread //
			{
				...
				{
					// -- not necessory -- lock_super(sb);
					if (sb->s_op->write_super)   // -- this super_block belongs to VFM //
						sb->s_op->write_super(sb);
					// -- not necessory -- unlock_super(sb);
					drop_super(sb);
				}
