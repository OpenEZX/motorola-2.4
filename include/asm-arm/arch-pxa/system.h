 ...
 *    2006-03-20    change reset delay time
*     2006-10-25    update interface for poweric driver
 */
...
static inline void arch_reset(char mode)
{
	if (mode == 's') {
		...
		*(unsigned long *)(phys_to_virt(BPSIG_ADDR)) = NO_FLAG;
		...
		mdelay(1000);// DEC 21 - for restart, disconnect time should be above 2 sec, otherwise unknow device appear on windows
/*		power_ic_periph_set_usb_pull_up(POWER_IC_PERIPH_ON);   -  051226 - it is a step of phone start,other than shutdown*/
