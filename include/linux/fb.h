 ...
 *
 * 09/07/2005  modify EZX platform
 * 05/15/2006  add ioctl for touch screen to set/clear a flag which will indicate whether touch screen 
 *             send ts events to user space when bklight is turned off.  
 */
...
#define TS_BKLIGHTOFF_SENDEVENTSMODE	3

/* added for bklightoff send events */
#define BKLIGHTOFFSENDEVENTS_OFF     0
#define BKLIGHTOFFSENDEVENTS_ON      1
...
/* Added for overlay2 framebuffer mmap */
struct fb_ovl2_mapinfo
{
	unsigned long ovl2_offset;
	unsigned long ovl2_size;
