...
void irlan_ias_register(struct irlan_cb *self, __u8 tsap_sel)
{
	...
	if (!irias_find_object("IrLAN")) {
		...
		irias_add_string_attrib(obj, "Name", sysctl_devname,
					IAS_KERNEL_ATTR);
#else
		irias_add_string_attrib(obj, "Name", "A760", IAS_KERNEL_ATTR);
#endif
		irias_add_string_attrib(obj, "DeviceID",/* "HWP19F0"*/"MOT0002",
					IAS_KERNEL_ATTR);
		irias_add_integer_attrib(obj, "CompCnt", 1, IAS_KERNEL_ATTR);
		if (self->provider.access_type == ACCESS_PEER)
			irias_add_string_attrib(obj, "Comp#01", "PNPC103",
						IAS_KERNEL_ATTR);
		else
			irias_add_string_attrib(obj, "Comp#01", "PNPC103",
						IAS_KERNEL_ATTR);
